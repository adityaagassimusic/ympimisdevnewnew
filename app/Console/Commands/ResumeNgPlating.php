...
use App\MiddlePlatingMonthlyResume;
use App\MiddlePlatingWeeklyResume;
use App\MiddlePlatingDailyResume;
use App\MiddlePlatingCheckLog;
use App\MiddlePlatingNgLog;
use App\MiddlePlatingMonthlyNgResume;

class ResumeNgPlating extends Command{
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'resume:plating';
    ...
    public function handle() {
        ...
        $resume_cek_monthly = db::select("SELECT w.fiscal_year, DATE_FORMAT(w.week_date,'%Y-%m') AS `month`, w.week_name, date(c.created_at) AS date, c.location, m.hpl, SUM(c.quantity) AS cek FROM middle_plating_check_logs c
        ...
        for ($i=0; $i < count($resume_cek_monthly); $i++) {
            $cekMonth = MiddlePlatingMonthlyResume::where('fiscal_year', $resume_cek_monthly[$i]->fiscal_year)
            ...
            if($cekMonth){
                ...
                $insert = new MiddlePlatingMonthlyResume([
            ...
            $cekWeek = MiddlePlatingWeeklyResume::where('fiscal_year', $resume_cek_monthly[$i]->fiscal_year)
            ->where('month', $resume_cek_monthly[$i]->month)
            ...
            if($cekWeek){
                ...
                $insert = new MiddlePlatingWeeklyResume([
                    'fiscal_year' => $resume_cek_monthly[$i]->fiscal_year,
                    'month' => $resume_cek_monthly[$i]->month,
            ...
            $cekDate = MiddlePlatingDailyResume::where('date', $resume_cek_monthly[$i]->date)
            ...
            if($cekDate){
                ...
                $insert = new MiddlePlatingDailyResume([
        ...
        $resume_ng_monthly = db::select("SELECT w.fiscal_year, DATE_FORMAT(w.week_date,'%Y-%m') AS `month`, w.week_name, date(ng.created_at) AS date, ng.location, m.hpl, SUM(ng.quantity) AS ng FROM middle_plating_ng_logs ng
        ...
        for ($i=0; $i < count($resume_ng_monthly); $i++) {
            $ngMonth = MiddlePlatingMonthlyResume::where('fiscal_year', $resume_ng_monthly[$i]->fiscal_year)
            ...
            $ngWeek = MiddlePlatingWeeklyResume::where('fiscal_year', $resume_ng_monthly[$i]->fiscal_year)
            ->where('month', $resume_ng_monthly[$i]->month)
            ...
            $cekDate = MiddlePlatingDailyResume::where('date', $resume_ng_monthly[$i]->date)
        ...
        $resume_ng_key =db::select("SELECT w.fiscal_year, DATE_FORMAT(w.week_date,'%Y-%m') AS `month`, ng.location, m.hpl, m.`key`, ng.ng_name, SUM(ng.quantity) AS ng FROM middle_plating_ng_logs ng
            LEFT JOIN weekly_calendars w ON w.week_date = date(ng.created_at)
            LEFT JOIN materials m ON m.material_number = ng.material_number
            WHERE ng.created_at < '".$datetime."'
            AND ng.sync = 0
        ...
        for ($i=0; $i < count($resume_ng_key); $i++) {
            $ngKey = MiddlePlatingMonthlyNgResume::where('fiscal_year', $resume_ng_key[$i]->fiscal_year)
            ...
            if($ngKey){
                ...
                $insert = new MiddlePlatingMonthlyNgResume([
        ...
        $updateCheck = MiddlePlatingCheckLog::where('created_at', '<', $datetime)
        ->update([
            'sync' => 1
        ]);

        $updateNg = MiddlePlatingNgLog::where('created_at', '<', $datetime)
        ->update([
            'sync' => 1
        ]);
