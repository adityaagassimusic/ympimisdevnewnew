    ...
    <div class="row">
        <div class="col-xs-12" style="height:100%">
            ...
            <table id="assyTable" class="table table-bordered" style="padding: 0px; margin-bottom: 0px; height:100%;">
                ...
                <tr id="stok_all"></tr>
                <tr id="chart"
                style="text-shadow: -1px 0 black, 0 1px black, 1px 0 black, 0 -1px black; font-size: 12px;"></tr>
                <tr id="legend"></tr>
            </table>
        </div>
        <div class="col-xs-12">
            ...
            <form method="GET" action="{{ url('index/display/body/' . $option) }}">
                ...
                <div class="col-xs-2">
                    <select class="form-control select2" id="order" onchange="changeOrder()" placeholder="Order by">
                        ...
                        <option value="1" <?php if ($_GET['order2'] == '1') {
                            echo 'selected';
                        } ?>>Stock Room</option>
                        <option value="2" <?php if ($_GET['order2'] == '2') {
                            echo 'selected';
                        } ?>>Availibility</option>
                    </select>
        ...
        $.get('{{ url('fetch/display/body/' . $option) }}', data, function(result, status, xhr) {
            if (result.status) {
                ...
                $.each(temporary, function(index, value) {
                    ...
                    if (value.model  == 'FL' &&  value.key == 'BODY' && value.surface == 'S.PLT') {
                        retunAcc += "<td class='acc'>" + (value.minus - 339) + "</td>";
                    } else if (value.model  == 'FL282' &&  value.key == 'BODY' && value.surface == 'S.PLT') {
                        retunAcc += "<td class='acc'>" + (value.minus + 4) + "</td>";
                    } else {
                        retunAcc += "<td class='acc'>" + value.minus + "</td>";
                    }

                    var style = '';
                    if (value.key.includes("CROWN")) {
                        style = "color:#f2e127; background-color:#333;";
                    } else {
                        style = "color:#333; background-color:#f2e127;";
                    }

                    model += "<th style='" + style + "'>" + value.model + "<br/>" + value.key +
                    "<br/>" + value
                    .surface + "</th>";
                    ...
                    stk += "<td style='border: 1px solid white; " + color2 + "'>" + value.stockroom +
                    "</td>";

                    stk_als += "<td style='border: 1px solid white; " + colour + "'>" + (value.ava ||
                        '0') + "</td>";

                    max_tmp.push(parseInt(value.barrel) + parseInt(value.lacquering) + parseInt(value
                        .plating) + parseInt(value.stockroom) + parseInt(value.welding) +
                    parseInt(value.buffing));
                ...
                for (var i = 0; i < temporary.length; i++) {
                    ...
                    kosong = (max - (parseInt(temporary[i].barrel) + parseInt(temporary[i].lacquering) +
                        parseInt(temporary[i].plating) + parseInt(temporary[i].stockroom) + parseInt(
                            temporary[i].welding) + parseInt(temporary[i].buffing))) / max * 100;
                    ...
                    if (parseInt(temporary[i].welding) > 0) wld = parseInt(temporary[i].welding);
                    else wld = '';
                    chart +=
                    ...
                    if (parseInt(temporary[i].buffing) > 0) bff = parseInt(temporary[i].buffing);
                    else bff = '';
                    chart +=
                    ...
                    if (parseInt(temporary[i].barrel) > 0) brl = parseInt(temporary[i].barrel);
                    else brl = '';
                    chart +=
                    ...
                    if (parseInt(temporary[i].lacquering) > 0) lcq = parseInt(temporary[i].lacquering);
                    else lcq = '';
                    chart +=
                    ...
                    if (parseInt(temporary[i].plating) > 0) plt = parseInt(temporary[i].plating);
                    else plt = '';
                    chart +=
                    ...
                    if (parseInt(temporary[i].stockroom) > 0) stk = parseInt(temporary[i].stockroom);
                    else stk = '';
                    chart +=
