				...
				<form method="GET" action="{{ url('index/display/sub_assy/'.$option) }}">
					...
					<div class="col-xs-2">
						<select class="form-control select2" id="order" onchange="changeOrder()" placeholder="Order by">
							...
							<option value="3" <?php if($_GET['order2'] == '3' ) echo "selected"; ?> >Specificity</option>
						</select>
			...
			$.get(url, data, function(result, status, xhr){
				if(result.status){
					...
					$.each(temporary, function(index, value){
						...
						if ("{{$option}}".substr(0, 4) != "assy") {
							tmp = (parseInt(value.barrel) + parseInt(value.lacquering) + parseInt(value.plating)+ parseInt(value.stockroom) + parseInt(value.buffing));
							// tmp = (parseInt(value.welding));
							if (tmp >= value.diff) {
