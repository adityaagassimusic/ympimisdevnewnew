...
@section('scripts')
    ...
    <script>
        ...
        jQuery(document).ready(function() {
            fetchChart();
            setInterval(fetchChart, 1000 * 60 * 120);
                ...
                if (loc[1] == 'ALL') {
                    ...
                    $.each(detail_mscr, function(key, value) {
                        if ($.date(value.posting_date) <= $.date(b)) {
                            ...
                            if (value.movement_type == 'XS04' || value.movement_type == '9S2') {
                                items[key] -= value.quantity;
                            } else if (value.movement_type == 'XS03' || value.movement_type == '9S1') {
                                items[key] += value.quantity;
                            }
                        }
                    });
                ...
                if (result.status) {
                        ...
                        tooltip: {
                            formatter: function() {
                                return 'Date: ' + Highcharts.dateFormat('%e - %b - %Y',
                                    new Date(this.x)) + '<br>' + this.series.name + ': USD ' + (this
                                    .y / 1000).toFixed(3) + 'K';
                            }
                        },
                        plotOptions: {
                    ...
                    $.each(result.categories, function(key, value) {
                        if (value.receive_location == 'WSCR') {
                            div_detail = '<div style="height:190px;" class="col-xs-3" id="' + value
                                .receive_location + '_' + value.reason + '"></div>';
                        ...
                        if (value.reason != null) {
                            if (reason_wscr.indexOf(value.reason) === -1) {
                                ...
                                for (var i = 0; i < result.actual_wscr.length; i++) {
                                    if (result.actual_wscr[i].reason == reason || result.actual_wscr[i]
                                        .reason == null) {
                                        ...
                                        new_target_reason.push([Date.parse(result.actual_wscr[i]
                                            .posting_date), parseFloat(new_target)]);
                                ...
                                for (var i = 0; i < new_group.length; i++) {
                                    ...
                                    new_sum.push([Date.parse(new_group[i].posting_date), parseFloat(
                                        new_amount)]);
                                }
                                    ...
                                    plotOptions: {
                                        series: {
                                            ...
                                            point: {
                                                events: {
                                                    click: function() {
                                                        fetchDetail(this.series.name, $.date(
                                                            this.category));
                                                    }
                                                }
                                            }
                        ...
                        if (value.reason != null) {
                            if (reason_mscr.indexOf(value.reason) === -1) {
                                ...
                                for (var i = 0; i < result.actual_mscr.length; i++) {
                                    if (result.actual_mscr[i].reason == reason || result.actual_mscr[i]
                                        .reason == null) {
                                        new_detail_reason.push({
                                        ...
                                        new_target_reason.push([Date.parse(result.actual_mscr[i]
                                            .posting_date), parseFloat(new_target)]);
