...
@section('header')
    <section class="content-header">
        <h1>
            All Stock <span class="text-purple">すべての在庫</span>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box">
                    ...
                    <div class="box-body">
                        <div id="container" style="width:100%; height:77.5vh;"></div>
                    </div>
                </div>
            </div>
            <div class="col-md-12">
                ...
                if (xhr.status == 200) {
                    if (result.status) {
                        ...
                        $.each(result.stockData, function(key, value) {
                            ...
                            tableStockData += '<td style="width: 10%;">' + value.hpl + '</td>';
                        ...
                        for (i = 0; i < data.length; i++) {
                            ...
                            var sum_volume_production = 0;
                            var sum_volume_intransit = 0;
                            var sum_volume_fstk = 0;
                            for (let j = 0; j < result.stockData.length; j++) {

                                var volume = 0;
                                for (let k = 0; k < result.materials.length; k++) {
                                    if (result.materials[k].material_number ==
                                        result.stockData[j].material_number) {
                                        volume = result.stockData[j].quantity / result.materials[k].lot_carton *
                                            result.materials[k].cubic_meter;
                                        break;
                                    }
                                }

                                if (data[i].destination == result.stockData[j].destination &&
                                    result.stockData[j].location == 'Production') {
                                    sum_volume_production += volume;
                                } else if (data[i].destination == result.stockData[j].destination &&
                                    result.stockData[j].location == 'Intransit') {
                                    sum_volume_intransit += volume;
                                } else if (data[i].destination == result.stockData[j].destination &&
                                    result.stockData[j].location == 'FSTK') {
                                    sum_volume_fstk += volume;
                                }

                            }
                            productionCount.push(sum_volume_production);
                            inTransitCount.push(sum_volume_intransit);
                            fstkCount.push(sum_volume_fstk);
                        }
                        ...
                        chart = new Highcharts.Chart({
                            ...
                            yAxis: {
                                ...
                                stackLabels: {
                                    ...
                                    formatter: function() {
                                        return this.total.toFixed(2) + " m&sup3";
                                    }
                                },
                                type: 'logarithmic'
                            },
                            ...
                            tooltip: {
                                formatter: function() {
                                    return '<b>' + this.x + '</b><br/>' +
                                        ...
                                        ' m<sup>3</sup><br/>' +
                                        'Total: ' + this.point.stackTotal.toFixed(2) + ' m<sup>3</sup>';
                                }
                            },
                            series: [{
                                name: 'Production',
            ...
            $.get('{{ url('fetch/tb_stock') }}', data, function(result, status, xhr) {
                ...
                if (xhr.status == 200) {
                    if (result.status) {
                        $('#tableBody').html("");
                        $('.modal-title').html("");
                        $('.modal-title').html('Location <b>' + result.location + '</b> for Destination <b>' +
                            result.title + '</b>');
                        var tableData = '';
                        var totalQty = 0;
                        var totalM3 = 0;
                        $.each(result.table, function(key, value) {
                            ...
                            totalM3 += value.actual / value.lot_carton * value.cubic_meter;
                            tableData += '<tr>';
                            tableData += '<td>' + value.material_number + '</td>';
                            tableData += '<td>' + value.material_description + '</td>';
                            tableData += '<td>' + value.actual + '</td>';
                            tableData += '<td>' + (value.actual / value.lot_carton * value.cubic_meter)
