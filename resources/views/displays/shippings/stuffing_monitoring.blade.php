        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="form-group">
                    ...
                    <div class="col-sm-2">
                        <input type="text" class="form-control datepicker" id="date" placeholder="Enter Date . . ."
                            style="background-color: rgba(0,0,0,0); color: white;font-size: 25px; text-align: center"
                            onchange="fillTable()">
                    </div>
                </div>
                                ...
                                <table id="tabel_detail" class="table table-striped table-bordered" style="width: 100%;">
                                    <thead style="background-color: rgba(126,86,134,.7);">
                                        <tr>
                                            ...
                                            <th style="width: 5%;">Plan Qty</th>
                                            <th style="width: 5%;">Act Qty</th>
...
@section('scripts')
    ...
    <script>
        ...
        jQuery(document).ready(function() {
            fillTable();
            setInterval(fillTable, 1000 * 60 * 5);
            ...
            $.get('{{ url('fetch/display/stuffing_detail') }}', data, function(result, status, xhr) {
                if (result.status) {
                    ...
                    for (var i = 0; i < result.stuffing_detail.length; i++) {
                        detailTableBody += "<tr>";
                        detailTableBody += "<td>" + result.stuffing_detail[i].order_type + "</td>";
                        ...
                        var diff = parseInt(result.stuffing_detail[i].plan_loading) - parseInt(result
                            .stuffing_detail[i].actual_loading);
                        var persen = (parseInt(result.stuffing_detail[i].actual_loading) / parseInt(result
                            .stuffing_detail[i].plan_loading)) * 100;
                    ...
                    var tableQty = $('#tabel_detail').DataTable({
                        'dom': 'Bfrtip',
                        'responsive': true,
                        'lengthMenu': [
                            [-1],
                    ...
                    $.each(result.stuffing_progress, function(index, value) {
                        ...
                        if (value.stats == "DEPARTED") {
                            ...
                            if (value.deleted_at != "-") {
                                ...
                                garis =
                                    "style='text-decoration: line-through; text-decoration-thickness: 3px;text-decoration-color: red;'";

                            }
                            style = "style='background-color:rgb(6, 115, 82); color:white; font-size:" +
                                size + "px'";


                            var d3 = new Date(value.departure);
                        ...
                        if (value.stats == "LOADING") {
                            ...
                            style = "style='background-color:yellow; color: #cc1305; font-size:" + size +
                                "px';";
                        ...
                        if (value.stats == "-") {
                            ...
                            style = "style='background-color:white; color: black; font-size:" + size +
                                "px';";
                        ...
                        stuffingTableBody += "<tr onClick='showModal(\"" + value.id_checkSheet + "\")' " +
                            style + ">";
                        ...
                        stuffingTableBody += "<td " + garis + ">" + value.id_checkSheet.substr(2, 7) +
                            "</td>";
                        ...
                        stuffingTableBody +=
                            "<td style='line-height:5px; padding:5px'><div class='progress " + cls +
                            "' style='height:" + size + "px; border: 1px solid " + color + ";'>";
                        stuffingTableBody += "<div class='progress-bar progress-bar-" + style2 + " " +
                            cls2 + "' role='progressbar' aria-valuenow='" + dif +
                            "' aria-valuemin='0' aria-valuemax='60' style='width: " + progress2 + "%;' >";
                        stuffingTableBody += "<span style='line-height:" + (size - 2) + "px; font-size:" +
                            size + "px'>" + dif + "</span>";
                        stuffingTableBody += "</div></div><span  style='color:" + rs_color +
                            "; font-size:15px; line-height: 80%; font-weight: normal; '>" + reason +
                            "</span></td>";
