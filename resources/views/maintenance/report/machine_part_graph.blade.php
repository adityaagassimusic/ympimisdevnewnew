...
<section class="content" style="padding-left: 0px; padding-right: 0px; padding-top: 0px">
	<div class="row">
		<div class="col-xs-12">
			<div class="col-xs-12">
				<div id="total" style="width: 100%; height: 500px;"></div>
			</div>
						...
						<tr>
							<td width="1%">
								<div class="description-block border-right" style="color: #000">
									<span class="description-text" style="font-size: 32px;">Total Estimasi Biaya : </span>   
								</div>
							</td>
							<td width="1%">
								<div class="description-block border-right text-green" style="color: #7300ab" >
									<span class="description-text" style="font-size: 32px;">Rp <span id="total_biaya"></span></span>   
								</div>
							</td>
						</tr>
					</table>
...
<script>
	...
	function drawChart() {
		$.get('{{ url("fetch/maintenance/machine/part_graph") }}', function(result) {
			if (result.status) {
				var series = [];
				var ctg = [];
				var tot = 0;

				$.each(result.machine_part, function(index, value){
					series.push(parseInt(value.tot_price));
					ctg.push(value.department);
					tot += parseInt(value.tot_price);
				});

				$("#total_biaya").text(tot.toLocaleString('de-DE'));
				...
				Highcharts.chart('total', {
					...
					title: {
						text: 'Machine Part Cost Estimation Chart (Rupiah)'
					},
	...
	function fetchModal(cat){
		...
		$.get('{{ url("fetch/kaizen/resume_detail") }}', data, function(result) {
			if(result.status){
				...
				$('#totalKaizen').text(total.toLocaleString('de-DE'));
