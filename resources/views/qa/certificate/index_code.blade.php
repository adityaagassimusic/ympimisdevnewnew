	...
	<div class="row">
		<div class="col-xs-12" style="padding-bottom: 10px">
			...
			<div class="col-xs-1" style="padding-left: 5px;padding-right: 5px;">
				<button class="btn btn-default pull-left" id="btnSearch" onclick="fillList()" style="font-weight: bold;height:35px;background-color: rgb(126,86,134);color: white;border:1px solid rgb(126,86,134)">
					Search <small>検索</small>
				</button>
			</div>
			<div class="col-xs-3 pull-right" style="padding-left: 5px;padding-right: 5px;">
			<?php if ($role == 'L-QA' || $role == 'S-MIS'): ?>
				<a class="btn btn-success pull-right" style="font-weight: bold;height:35px;color: white;border:1px solid rgb(126,86,134)" href="{{url('approval/qa/certificate/Leader QA')}}">
					Approval Leader <small>リーダーの承認</small>
				</a>
			<?php endif ?>
			<?php if ($role == 'L-QA' || $role == 'S-MIS' || $role == 'QA'): ?>
				<a class="btn btn-primary pull-right" style="font-weight: bold;height:35px;color: white;border:1px solid rgb(126,86,134)" href="{{url('index/qa/qr_code/certificate')}}">
					Print QR Code
				</a>
			<?php endif ?>
			</div>
		...
		<div class="col-xs-5" style="padding-right: 5px;">
			<div id="container2" style="height: 35vh;">
				
			</div>
		</div>
		<div class="col-xs-2" style="margin-bottom: 0px;padding-bottom: 10px;padding-left: 5px">
		...
		<div class="col-xs-12" style="margin-top: 0px;padding-top: 0px" id="div_detail">
							...
							<table id="tableCode" class="table table-bordered table-hover" style="margin-bottom: 0;">
								<thead style="background-color: #605ca8;color: white">
									<tr>
										...
										<th width="1%">Chief QA<br><small>品質保証主任</small></th>
										<th width="1%">Manager Produksi<br><small>組立課長</small></th>
										<th width="1%">Manager In Charge QA<br><small>品質保証課長</small></th>
										<th width="1%">Manager STD<br><small>標準化課長</small></th>
										<!-- <th width="1%">DGM<br><small>副部長</small></th>
										<th width="1%">GM<br><small>部長</small></th> -->
										<th width="1%">Director<br><small>院長</small></th>
...
<script>
	...
	function fillList(){
		...
		$.get('{{ url("fetch/qa/certificate/code") }}',data, function(result, status, xhr){
			if(result.status){
				...
				// var certificate_approval_id = result.approval[0][0].certificate_approval_id;
				// var certificate_approval_id_fix = [];

				// $.each(result.datas, function(key2, value2) {
				// 	for (var i = 0; i < result.approval.length; i++) {
				// 		if (result.approval[i][0].certificate_id == value2.certificate_id) {
				...
				// 	certificate_approval_id_fix.push(certificate_approval_id);
				// });
				// var count = {};
				// certificate_approval_id_fix.forEach(element => {
				...
				$.each(result.datas, function(key, value) {
					tableData += '<tr  id="'+value.certificate_name+'">';
					...
					tableData += '<td style="padding:5px !important">'+ value.description +' - '+value.names+'</td>';
									...
									if (result.approval[i][j].remark == 'Leader QA') {
										if (result.approval[i][j].approver_status == null) {
											if (result.approval[i][j].keutamaan == 'utama') {
												...
												var urls = '{{url("approval/qa/certificate/Leader QA")}}';
							...
							if (result.approval[i][0].certificate_id == value.certificate_id) {
								for (var j = 0; j < result.approval[i].length;j++) {
									if (result.approval[i][j].remark == 'Staff QA') {
										if (result.approval[i][j].approver_status == null) {
											if (result.approval[i][j].keutamaan == 'utama') {
												var urls = '{{url("approval/qa/certificate/Staff QA")}}';
												tableData += '<td style="padding:5px !important;color:white;font-weight:bold;font-size:11px;background-color:#dd4b39;text-align:center;"><a style="text-decoration:none;color:white" target="_blank" href="'+urls+'">'+result.approval[i][j].approver_names+'<br>Waiting<br><small>承認待ち</small></a></td>';
											}else{
												tableData += '<td style="padding:5px !important;background-color:white;"></td>';
											}
									...
									if (result.approval[i][j].remark == 'Foreman QA') {
										if (result.approval[i][j].approver_status == null) {
											if (result.approval[i][j].keutamaan == 'utama') {
												...
												var urls = '{{url("approval_all/qa/certificate/")}}'+'/'+result.approval[i][j].remark+'/'+certificate_id_approve;
												tableData += '<td style="padding:5px !important;color:white;font-weight:bold;font-size:11px;background-color:#dd4b39;text-align:center;"><a style="text-decoration:none;color:white" target="_blank" href="'+urls+'">'+result.approval[i][j].approver_names+'<br>Waiting<br><small>承認待ち</small></a></td>';
											}else{
												tableData += '<td style="padding:5px !important;background-color:white;"></td>';
											}
									...
									if (result.approval[i][j].remark == 'Chief QA') {
										if (result.approval[i][j].approver_status == null) {
											if (result.approval[i][j].keutamaan == 'utama') {
												...
												var urls = '{{url("approval_all/qa/certificate/Chief QA")}}'+'/'+certificate_id_approve;
												tableData += '<td style="padding:5px !important;color:white;font-weight:bold;font-size:11px;background-color:#dd4b39;text-align:center;"><a style="text-decoration:none;color:white" target="_blank" href="'+urls+'">'+result.approval[i][j].approver_names+'<br>Waiting<br><small>承認待ち</small></a></td>';
											}else{
												tableData += '<td style="padding:5px !important;background-color:white;"></td>';
											}
					...
					if (approval_list.indexOf('Manager Produksi') != -1) {
						for (var i = 0; i < result.approval.length; i++) {
							if (result.approval[i][0].certificate_id == value.certificate_id) {
								for (var j = 0; j < result.approval[i].length;j++) {
									if (result.approval[i][j].remark == 'Manager Produksi') {
										if (result.approval[i][j].approver_status == null) {
											if (result.approval[i][j].keutamaan == 'utama') {
												var certificate_id_approve = '';
												for(var u = 0; u < result.utamas.length;u++){
												...
												var urls = '{{url("approval_all/qa/certificate/Manager Produksi")}}'+'/'+certificate_id_approve;
												tableData += '<td style="padding:5px !important;color:white;font-weight:bold;font-size:11px;background-color:#dd4b39;text-align:center;"><a style="text-decoration:none;color:white" target="_blank" href="'+urls+'">'+result.approval[i][j].approver_names+'<br>Waiting<br><small>承認待ち</small></a></td>';
											}else{
												tableData += '<td style="padding:5px !important;background-color:white;"></td>';
											}
					...
					if (approval_list.indexOf('Manager In Charge QA') != -1) {
						for (var i = 0; i < result.approval.length; i++) {
							if (result.approval[i][0].certificate_id == value.certificate_id) {
								for (var j = 0; j < result.approval[i].length;j++) {
									if (result.approval[i][j].remark == 'Manager In Charge QA') {
										if (result.approval[i][j].approver_status == null) {
											if (result.approval[i][j].keutamaan == 'utama') {
												var certificate_id_approve = '';
												for(var u = 0; u < result.utamas.length;u++){
					...
					if (approval_list.indexOf('Manager QA') != -1 || approval_list.indexOf('Manager STD') != -1) {
						for (var i = 0; i < result.approval.length; i++) {
							if (result.approval[i][0].certificate_id == value.certificate_id) {
								for (var j = 0; j < result.approval[i].length;j++) {
									if (result.approval[i][j].remark == 'Manager QA' || result.approval[i][j].remark == 'Manager STD') {
										if (result.approval[i][j].approver_status == null) {
											if (result.approval[i][j].keutamaan == 'utama') {
												var certificate_id_approve = '';
												for(var u = 0; u < result.utamas.length;u++){
					...
					// if (approval_list.indexOf('Deputy General Manager') != -1) {
					// 	for (var i = 0; i < result.approval.length; i++) {
					// 		if (result.approval[i][0].certificate_id == value.certificate_id) {
					// 			for (var j = 0; j < result.approval[i].length;j++) {
					// 				if (result.approval[i][j].remark == 'Deputy General Manager') {
					// 					if (result.approval[i][j].approver_status == null) {
					// 						if (result.approval[i][j].keutamaan == 'utama') {
					// 							var certificate_id_approve = '';
					// 							for(var u = 0; u < result.utamas.length;u++){
					...
					if (approval_list.indexOf('Director') != -1) {
						for (var i = 0; i < result.approval.length; i++) {
							if (result.approval[i][0].certificate_id == value.certificate_id) {
								for (var j = 0; j < result.approval[i].length;j++) {
									if (result.approval[i][j].remark == 'Director') {
										if (result.approval[i][j].approver_status == null) {
											if (result.approval[i][j].keutamaan == 'utama') {
												var certificate_id_approve = '';
												for(var u = 0; u < result.utamas.length;u++){
					...
					if (approval_list.indexOf('President Director') != -1) {
						for (var i = 0; i < result.approval.length; i++) {
							if (result.approval[i][0].certificate_id == value.certificate_id) {
								for (var j = 0; j < result.approval[i].length;j++) {
									if (result.approval[i][j].remark == 'President Director') {
										if (result.approval[i][j].approver_status == null) {
											if (result.approval[i][j].keutamaan == 'utama') {
												var certificate_id_approve = '';
												for(var u = 0; u < result.utamas.length;u++){
					...
					if (value.status == '0') {
						...
						if (value.status == '1') {
							...
							tableData += '<a class="btn btn-primary btn-xs" target="_blank" href="{{url("print/qa/certificate/")}}/'+value.certificate_id+'">Detail <small>詳細</small></a>';
							...
							for (var i = 0; i < result.approval.length; i++) {
								if (result.approval[i][0].certificate_id == value.certificate_id) {
									for (var j = 0; j < result.approval[i].length;j++) {
										if (result.approval[i][j].remark == 'Leader QA') {
											if (result.approval[i][j].approver_status == null) {
												if ('{{$role}}'.match(/QA/gi) || '{{$role}}'.match(/MIS/gi)) {
													tableData += '<a class="btn btn-warning btn-xs" target="_blank" href="{{url("review/qa/certificate/")}}/'+value.certificate_id+'/Leader QA">Edit <small>更新</small></a>';
												}
											}else{
										...
										if (result.approval[i][j].remark == 'President Director' && result.approval[i][j].approver_status != null) {
											fully_approved = 'Full';
										}
							...
							if ('{{$role}}'.match(/QA/gi) || '{{$role}}'.match(/MIS/gi)) {
								tableData += '<a class="btn btn-info btn-xs" target="_blank" href="{{url("renew/qa/certificate/")}}/'+value.certificate_id+'">Renew <small>更新必要</small></a>';
							}
							if ('{{$role}}'.match(/QA/gi) || '{{$role}}'.match(/MIS/gi)) {
				...
				for (var i = 0; i < result.charts.length;i++) {
					if (result.charts[i].product.match(/QA/gi)) {
				...
				const chart2 = new Highcharts.Chart({
				    ...
				    title: {
				        text: '<b>PRODUCTION CERTIFICATE</b>',
						style:{
							fontSize:"12px"
						}
				    },
	...
	function deactivate(certificate_id) {
		if (confirm('Apakah Anda yakin untuk menonaktifkan sertifikat ini?')) {
			$('#loading').show();
			var data = {
				certificate_id:certificate_id
			}
			$.get('{{ url("deactivate/qa/certificate") }}',data, function(result, status, xhr){
				if(result.status){
	...
	function resendEmail(certificate_approval_id,remark) {
		if (confirm('Apakah Anda yakin akan mengirim ulang Email?')) {
			$('#loading').show();
			$.get('{{ url("resend/qa/certificate/code/") }}/'+certificate_approval_id+'/'+remark,  function(result, status, xhr){
				if(result.status){
					fillList();
					$('#loading').hide();
