						...
						<div class="row" style="padding-top: 10px">
							...
							<div class="col-xs-12" style="padding-top: 10px">
								<select class="form-control select3" id="employee_id_select" style="width: 100%" data-placeholder="Pilih Karyawan">
									...
									<option value="{{$employees->employee_id}}_{{$employees->name}}">{{$employees->employee_id}} - {{$employees->name}}</option>
									@endforeach
								</select>
							</div>
						</div>
...
<script>
	...
	function saveCode() {
		...
		for(var i = 0; i < periode.length;i++){
			if (periode[i].code == $('#code_number_select').val().split('-')[0] && periode[i].code_number == $('#code_number_select').val().split('-')[1]) {
			...
			if(result.status){
				...
				for(var i = 0; i < result.point.length;i++){
					...
					if (result.point[i].question == null) {
						...
						tableCheck += '<td style="background-color:white;color:black;font-size:15px;border:2px solid rgb(60, 60, 60);padding:3px;text-align:right"><input type="text" class="form-control" readonly style="width:100%;text-align:right" id="'+index_subject+'_question_'+i+'" onchange="checkSubTotal(this.id)" name="'+index_subject+'_question_'+i+'" value="'+result.point[i].question+'"></td>';
					}
	...
	function checkSubTotalResult(id) {
		...
		// console.log($('#'+id.split('_')[0]+'_question_'+id.split('_')[3]).val());
		// console.log(id.split('_')[0]);
		// console.log(id.split('_')[3]);
		// if ($('#'+id.split('_')[0]+'_question_'+id.split('_')[3]).val() == 'Point & urutan pekerjaan ') {
		// 	var presentase_ik = (parseInt($('#'+id.split('_')[0]+'_question_result_'+id.split('_')[3]).val()) / parseInt($('#'+id.split('_')[0]+'_weight_'+id.split('_')[3]).val()))*100;
		// 	if (presentase_ik < 100) {
		// 		openErrorGritter('Error!','Ada Point yang TIDAK LULUS');
		// 		document.getElementById(id.split('_')[0]+'_question_result_'+id.split('_')[3]).style.backgroundColor = "red";
		// 		document.getElementById(id.split('_')[0]+'_question_result_'+id.split('_')[3]).style.color = "white";
		// 	}else{
		// 		document.getElementById(id.split('_')[0]+'_question_result_'+id.split('_')[3]).style.removeProperty("background-color");
		// 		document.getElementById(id.split('_')[0]+'_question_result_'+id.split('_')[3]).style.removeProperty("color");
		// 	}
		// }
		$('#'+id.split('_')[0]+'_sub_total_question_result').val(total);
		// if ($('#'+id.split('_')[0]+'_sub_total_question').val(total) ) {
	...
	function confirmAll() {
		if (confirm('Apakah Anda yakin menyelesaikan proses?')) {
			...
			$.post('{{ url("input/new/qa/certificate") }}', data, function(result, status, xhr){
				if(result.status){
