...
<section class="content" style="padding-top: 0px">
	...
	<div class="row" style="padding-left: 10px;padding-right: 10px;">
		<div class="col-xs-12" style="padding-bottom: 10px;padding-left: 0px;padding-right: 0px;">
			...
			<div class="col-xs-1" style="padding-left: 5px;padding-right: 5px;">
				<select class="form-control select2" style="width: 100%" data-placeholder="Select Point Check" id="material_number">
					<option value=""></option>
			...
			<div class="col-xs-7 pull-right" style="padding-left: 0px;padding-right: 0px;">
				...
				<a class="btn btn-warning pull-right" style="font-weight: bold;height:35px;color: white;border:1px solid rgb(126,86,134)" href="{{url('index/qa/packing/point_check')}}">
					<i class="fa fa-check-square-o"></i>&nbsp;&nbsp;Point Check WI
				</a>
				<a class="btn btn-success pull-right" style="font-weight: bold;height:35px;color: white;border:1px solid rgb(126,86,134);margin-right: 5px;" href="{{url('index/qa/packing/audit')}}">
					<i class="fa fa-pencil"></i>&nbsp;&nbsp;Audit WI
				</a>
				<a class="btn btn-default pull-right" style="font-weight: bold;height:35px;border:1px solid rgb(126,86,134);margin-right: 5px;" href="{{url('index/qa/packing/report')}}">
					<i class="fa fa-book"></i>&nbsp;&nbsp;Report WI
				</a>
				<!-- <a class="btn btn-info pull-right" style="font-weight: bold;height:35px;color: white;border:1px solid rgb(126,86,134);margin-right: 5px;" href="{{url('index/qa/audit_fg/point_check')}}">
					<i class="fa fa-check-square-o"></i>&nbsp;&nbsp;Point Check FG EI
				</a>
				<a class="btn btn-success pull-right" style="font-weight: bold;height:35px;color: white;border:1px solid rgb(126,86,134);margin-right: 5px;" href="{{url('index/qa/audit_fg/audit')}}">
					<i class="fa fa-pencil"></i>&nbsp;&nbsp;Audit FG EI
				</a>
				<a class="btn btn-danger pull-right" style="font-weight: bold;height:35px;color: white;border:1px solid rgb(126,86,134);margin-right: 5px;" href="{{url('index/qa/audit_fg/report')}}">
					<i class="fa fa-book"></i>&nbsp;&nbsp;Report FG EI
				</a> -->
				@endif
				@endif
...
<script>
	...
	function fillList(){
		...
		$.get('{{ url("fetch/qa/packing") }}',data, function(result, status, xhr){
			if(result.status){
				var categories = [];
				var totals = [];
				var oks = [];
				...
				for(var i = 0; i < result.audit.length;i++){
					date.push(result.audit[i].date_audit);
				...
				for(var i = 0; i < date_unik.length;i++){
					...
					for(var j = 0; j < result.audit.length;j++){
						if (result.audit[j].date_audit == date_unik[i]) {
							total++;
							if (result.audit[j].result_check.match(/NG/gi) && result.audit[j].handled_id == null) {
								ng++;
							}else{
								ok++;
							}
					...
					totals.push({y:parseInt(total),key:date_unik[i]});
				...
				for(var i = 0; i < result.audit.length;i++){
					if (result.audit[i].result_check.match(/NG/gi) && result.audit[i].handled_id == null) {
						tableData += '<tr id="'+result.audit[i].material_number+'">';
						tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+result.audit[i].date_audit_name+'</td>';
						tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+result.audit[i].product+'</td>';
						tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+(result.audit[i].material_audited || '')+'</td>';
						tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+(result.audit[i].serial_number || '')+'</td>';
						tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+result.audit[i].auditor_id+'<br>'+result.audit[i].auditor_name+'</td>';
						tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+result.audit[i].auditee_id+'<br>'+result.audit[i].auditee_name+'</td>';
						tableData += '<td style="padding-left:10px !important;background-color: #ff8f8f;text-align:center;">&#9747;</td>';
						tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff;font-weight:bold;text-align:center"><span class="label label-danger">Open</span><br><span style="font-weight:normal">Due Date : '+result.audit[i].due_date+'</span></td>';
						tableData += '<td style="background-color: #f0f0ff;font-weight:bold;text-align:center">';
						if (result.audit[i].send_status == null && result.audit[i].handled_id == null) {
							if (result.audit[i].product == 'Pianica' || result.audit[i].product == 'Recorder') {
								tableData += '<button class="btn btn-primary btn-xs" onclick="sendEmailEi(\''+result.audit[i].audit_id+'\')" style="margin-right:5px;"><i class="fa fa-envelope-o"></i>&nbsp;&nbsp;Send Email</button>';
							}else{
								tableData += '<button class="btn btn-primary btn-xs" onclick="sendEmail(\''+result.audit[i].audit_id+'\')" style="margin-right:5px;"><i class="fa fa-envelope-o"></i>&nbsp;&nbsp;Send Email</button>';
							}
						}else{
							var url = '{{url("index/qa/packing/send_email")}}/'+result.audit[i].audit_id;
							if (result.audit[i].product == 'Pianica' || result.audit[i].product == 'Recorder') {
								tableData += '<button class="btn btn-primary btn-xs" onclick="sendEmailEi(\''+result.audit[i].audit_id+'\')" style="margin-right:5px;"><i class="fa fa-envelope-o"></i>&nbsp;&nbsp;Re-Send Email</button>';
							}else{
								tableData += '<button class="btn btn-primary btn-xs" onclick="sendEmail(\''+result.audit[i].audit_id+'\')" style="margin-right:5px;"><i class="fa fa-envelope-o"></i>&nbsp;&nbsp;Re-Send Email</button>';
							}
						}
						if (result.audit[i].product == 'Pianica' || result.audit[i].product == 'Recorder') {
							var url = '{{url("index/qa/audit_fg/pdf")}}/'+result.audit[i].audit_id;
							...
							var url = '{{url("index/qa/packing/handling")}}/'+result.audit[i].audit_id;
							tableData += '<a class="btn btn-success btn-xs" href="'+url+'"><i class="fa fa-edit"></i>&nbsp;&nbsp;Penanganan</a>';
						}else{
							var url = '{{url("index/qa/packing/pdf")}}/'+result.audit[i].audit_id;
							tableData += '<a class="btn btn-danger btn-xs" target="_blank" href="'+url+'" style="margin-right:5px;"><i class="fa fa-file-pdf-o"></i>&nbsp;&nbsp;Report PDF</a>';
							var url = '{{url("index/qa/packing/handling")}}/'+result.audit[i].audit_id;
							tableData += '<a class="btn btn-success btn-xs" href="'+url+'"><i class="fa fa-edit"></i>&nbsp;&nbsp;Penanganan</a>';
						}
	...
	function sendEmail(audit_id) {
		$('#loading').show();
		$.get('{{ url("index/qa/packing/send_email") }}/'+audit_id, function(result, status, xhr){
			if(result.status){
				$('#loading').hide();
	...
	function showModal(date_name,condition,date) {
		...
		for(var i = 0; i < packing_detail.length;i++){
			if (condition == 'Audit Sudah Dilakukan (OK & Sudah Ditangani)' && packing_detail[i].date_audit == date) {
				if ((packing_detail[i].result_check.match(/NG/gi) && packing_detail[i].handled_id != null) || packing_detail[i].result_check == 'OK' || packing_detail[i].result_check == 'OK,Tidak Ada') {
					tableData += '<tr id="'+packing_detail[i].material_number+'">';
					tableData += '<td style="background-color: #f0f0ff;text-align:right;padding-right:7px; !important">'+index+'</td>';
					tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+packing_detail[i].product+'</td>';
					tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+(packing_detail[i].material_audited || '')+'</td>';
					tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+(packing_detail[i].session || '')+'</td>';
					tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+(packing_detail[i].serial_number || '')+'</td>';
					tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+(packing_detail[i].qty_lot || '')+'</td>';
					tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+(packing_detail[i].qty_check || '')+'</td>';
					tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+packing_detail[i].auditor_id+'<br>'+packing_detail[i].auditor_name+'</td>';
					tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+(packing_detail[i].qty_auditor || '')+'</td>';
					tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+packing_detail[i].auditee_id+'<br>'+packing_detail[i].auditee_name+'</td>';
					if (packing_detail[i].result_check.match(/NG/gi)) {
						tableData += '<td style="padding-left:10px !important;background-color: #ff8f8f;text-align:center;">&#9747;</td>';
					}else if (packing_detail[i].result_check.match(/OK/gi)) {
					...
					if (packing_detail[i].product == 'Pianica' || packing_detail[i].product == 'Recorder') {
						tableData += '<td style="background-color: #f0f0ff;font-weight:bold;text-align:center">';
						tableData += '<button class="btn btn-info btn-xs" style="margin-right:5px;" onclick="detailFindingEi(\''+packing_detail[i].audit_id+'\')"><i class="fa fa-eye"></i> Detail</button>';
						if ('{{$employee_id}}' == 'PI0904005' || '{{$employee_id}}' == 'PI1910002') {
							var url = '{{url("index/qa/audit_fg/edit")}}/'+packing_detail[i].audit_id;
							tableData += '<a class="btn btn-warning btn-xs" href="'+url+'"><i class="fa fa-edit"></i>&nbsp;&nbsp;Edit</a>';
							var url = '{{url("index/qa/audit_fg/delete")}}/'+packing_detail[i].audit_id;
						...
						var url = '{{url("index/qa/audit_fg/pdf")}}/'+packing_detail[i].audit_id;
						tableData += '<a class="btn btn-danger btn-xs" target="_blank" href="'+url+'" style="margin-right:5px;"><i class="fa fa-file-pdf-o"></i>&nbsp;&nbsp;Report PDF</a>';
					}else{
						tableData += '<td style="background-color: #f0f0ff;font-weight:bold;text-align:center">';
						tableData += '<button class="btn btn-info btn-xs" style="margin-right:5px;" onclick="detailFinding(\''+packing_detail[i].audit_id+'\')"><i class="fa fa-eye"></i> Detail</button>';
						var url = '{{url("index/qa/packing/pdf")}}/'+packing_detail[i].audit_id;
						tableData += '<a class="btn btn-danger btn-xs" target="_blank" href="'+url+'" style="margin-right:5px;"><i class="fa fa-file-pdf-o"></i>&nbsp;&nbsp;Report PDF</a>';
					}
					tableData += '</td>';
					tableData += '</tr>';
			...
			if (condition == 'Audit Sudah Dilakukan (Temuan NG)') {
				if (packing_detail[i].result_check.match(/NG/gi) && packing_detail[i].handled_id == null && packing_detail[i].date_audit == date) {
					tableData += '<tr id="'+packing_detail[i].material_number+'">';
					tableData += '<td style="background-color: #f0f0ff;text-align:right;padding-right:7px; !important">'+index+'</td>';
					tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+packing_detail[i].product+'</td>';
					tableData += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+(packing_detail[i].material_audited || '')+'</td>';
					...
					if (packing_detail[i].send_status == null && packing_detail[i].handled_id == null) {
						if (packing_detail[i].product == 'Pianica' || packing_detail[i].product == 'Recorder') {
							tableData += '<button class="btn btn-primary btn-xs" onclick="sendEmailEi(\''+packing_detail[i].audit_id+'\')" style="margin-right:5px;"><i class="fa fa-envelope-o"></i>&nbsp;&nbsp;Send Email</button>';
						}else{
							tableData += '<button class="btn btn-primary btn-xs" onclick="sendEmail(\''+packing_detail[i].audit_id+'\')" style="margin-right:5px;"><i class="fa fa-envelope-o"></i>&nbsp;&nbsp;Send Email</button>';
						}
					}else{
						if (packing_detail[i].product == 'Pianica' || packing_detail[i].product == 'Recorder') {
							tableData += '<button class="btn btn-primary btn-xs" onclick="sendEmailEi(\''+packing_detail[i].audit_id+'\')" style="margin-right:5px;"><i class="fa fa-envelope-o"></i>&nbsp;&nbsp;Re-Send Email</button>';
						}else{
							var url = '{{url("index/qa/packing/send_email")}}/'+packing_detail[i].audit_id;
							tableData += '<button class="btn btn-primary btn-xs" onclick="sendEmail(\''+packing_detail[i].audit_id+'\')" style="margin-right:5px;"><i class="fa fa-envelope-o"></i>&nbsp;&nbsp;Re-Send Email</button>';
						}
					}
					if (packing_detail[i].product == 'Pianica' || packing_detail[i].product == 'Recorder') {
						var url = '{{url("index/qa/audit_fg/pdf")}}/'+packing_detail[i].audit_id;
						tableData += '<a class="btn btn-danger btn-xs" target="_blank" href="'+url+'" style="margin-right:5px;"><i class="fa fa-file-pdf-o"></i>&nbsp;&nbsp;Report PDF</a>';
						var url = '{{url("index/qa/packing/handling")}}/'+packing_detail[i].audit_id;
						tableData += '<a class="btn btn-success btn-xs" href="'+url+'"><i class="fa fa-edit"></i>&nbsp;&nbsp;Penanganan</a>';
					}else{
						var url = '{{url("index/qa/packing/pdf")}}/'+packing_detail[i].audit_id;
						tableData += '<a class="btn btn-danger btn-xs" target="_blank" href="'+url+'" style="margin-right:5px;"><i class="fa fa-file-pdf-o"></i>&nbsp;&nbsp;Report PDF</a>';
	...
	function detailFinding(audit_id) {
		$('#loading').show();
		data = {
			audit_id:audit_id
		}

		$.get('{{ url("fetch/qa/packing/detail") }}',data, function(result, status, xhr){
			if(result.status){
				$('#data-log-detail-ei').DataTable().clear();
				$('#data-log-detail-ei').DataTable().destroy();
				$('#data-log-detail').DataTable().clear();
				...
				for(var i = 0; i < result.audit.length;i++){
					...
					details += '<td style="background-color: #f0f0ff;text-align:right;padding-right:7px; !important">'+index+'</td>';
					...
					details += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+(result.audit[i].serial_number || '')+'</td>';
					...
					if (result.audit[i].result_check.match(/NG/gi)) {
						details += '<td style="padding-left:10px !important;background-color: #ff8f8f;text-align:center;">&#9747;</td>';
					}else if (result.audit[i].result_check.match(/OK/gi)) {
						details += '<td style="padding-left:10px !important;background-color: #dfffd4;text-align:center;">&#9711;</td>';
					}else{
						details += '<td style="padding-left:10px !important;background-color: #dfffd4;text-align:center;">Tidak Ada</td>';
					}
					if (result.audit[i].point_check_type == 'checklist') {
						...
						if ((result.audit[i].point_check_details != null || result.audit[i].point_check_details != 'null') && (result.audit[i].result_details != null)) {
							var point_check_details = result.audit[i].point_check_details.split(',');
							var result_details = result.audit[i].result_details.split(',');

							if (point_check_details.length > 0 && result_details.length > 0) {
								for(var j = 0; j < point_check_details.length;j++){
									details += point_check_details[j]+' '+result_details[j]+'<br>';
								}
							}
						}
						details += '</td>';
					...
					details += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+(result.audit[i].handled_name || '')+'<br>'+(result.audit[i].handled_at || '')+'</td>';
					details += '<td style="padding-left:10px !important;background-color: #f0f0ff">'+(result.audit[i].handling || '')+'</td>';
					details += '</tr>';
					index++;
				}

				$('#body-detail-detail').append(details);

				var table = $('#data-log-detail').DataTable({
		            'dom': 'Bfrtip',
		            'responsive':true,
		            'lengthMenu': [
		            [ 10, 25, 50, -1 ],
				...
				$('#div_detail').show();
				$('#data-log-detail-ei').hide();
				$('#data-log-detail').show();
				$('#div_resume').hide();
				$('#loading').hide();
			}else{
				$('#loading').hide();
