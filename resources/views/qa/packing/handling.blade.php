		...
		<?php if (count($audit) > 0){ ?>
			<div class="col-xs-6" style="text-align: center;padding-right: 5px">
				<table style="width: 100%;border:1px solid black">
					...
					<tr>
						<td style="background-color: #c7c7c7;font-weight: bold;border:1px solid black;width: 2%;font-size: 15px" colspan="2">Serial Number</td>
					</tr>
					<tr>
						<td colspan="2" style="background-color: #ffadad;border:1px solid black;font-size: 18px" id="serial_number">{{$audit[0]->serial_number}}</td>
					</tr>
				</table>
			</div>
			<div class="col-xs-6" style="text-align: center;padding-right: 5px">
				<table style="width: 100%;border:1px solid black">
					<tr>
						...
						<td colspan="2" style="background-color: #c7c7c7;font-weight: bold;border:1px solid black;width: 2%;font-size: 15px">Material</td>
					</tr>
					<tr>
						<td style="background-color: #abcbff;border:1px solid black;font-size: 18px" id="product">{{$audit[0]->product}}</td>
						<td style="background-color: #abcbff;border:1px solid black;font-size: 18px" id="material_number">{{$audit[0]->material_number}}</td>
						<td style="background-color: #abcbff;border:1px solid black;font-size: 18px" id="material_description">{{$audit[0]->material_description}}</td>
					</tr>
					<tr>
						<td colspan="3" style="background-color: #c7c7c7;font-weight: bold;border:1px solid black;width: 2%;font-size: 15px">Material Audited</td>
					</tr>
					<tr>
						<td colspan="3" style="background-color: #abcbff;border:1px solid black;font-size: 18px" id="material_audited">{{$audit[0]->material_audited}}</td>
					</tr>
				</table>
			</div>
				...
				<table class="table table-responsive" style="width: 100%;border:1px solid black" id="tableCheck">
					<thead>
						<tr>
			 				...
			 				<th style="background-color:rgb(126,86,134);color:#FFD700;border:1px solid black;width:2%;">Result Details</th>
							...
							@foreach($audit as $audit)
								<tr>
									...
									<?php }else if($audit->result_check == 'NG'){ ?>
										<td style="background-color:white;border:1px solid black;width:1%;font-size: 20px">&#9747;</td>
									<?php } ?>
									@if($audit->point_check_type == 'input')
									<td style="background-color:white;border:1px solid black;width:1%;">
										<?php $point_check_details = explode(',', $audit->point_check_details) ?>
										<?php $result_details = explode(',', $audit->result_details) ?>
										<?php $details = []; ?>
										<?php for ($i=0; $i < count($point_check_details); $i++) { 
											array_push($details, $point_check_details[$i].' '.$result_details[$i].'<br>');
										} ?>
										<?php echo join($details) ?>
									</td>
									@else
...
<script>
	...
	function confirmAll() {
		if (confirm('Apakah Anda yakin menyelesaikan penanganan?')) {
			...
			$.post('{{ url("input/qa/packing/handling") }}', data, function(result, status, xhr){
				if(result.status){
					$('#loading').hide();
