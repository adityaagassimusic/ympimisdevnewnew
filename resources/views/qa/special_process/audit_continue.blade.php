				...
				<tr>
					<td colspan="2" style="background-color:#fff;color:#000;border:2px solid rgb(60, 60, 60);font-size: 18px;text-align: left;">
						<select class="form-control" id="select_emp" style="width: 100%;" data-placeholder="Pilih Karyawan">
							<option value=""></option>
							@foreach($emp as $emp)
							<?php if ($point_check[0]->employee_id == $emp->employee_id){ ?>
								<option value="{{$emp->employee_id}}" selected="true">{{$emp->employee_id}} - {{$emp->name}}</option>
							<?php }else{ ?>
			...
			<table class="table table-bordered table-hover" style="width: 100%;border:1px solid black" id="tableCheck">
				<thead id="headCheck" style="background-color: #0073b7;color: white">
					<tr>
						...
						<th style="width: 5%">Evidence & Note</th>
					</tr>
				</thead>
				<tbody id="bodyCheck" style="background-color: #f0f0ff;color: black;">
					...
					<tr style="border: 1px solid black">
						<td style="border: 1px solid black;text-align: right;"><?php echo $index+1 ?>
							...
							<input type="hidden" name="id_{{$index}}" id="id_{{$index}}" value="{{$point->id_hasil}}">
						</td>
						...
						<td style="border: 1px solid black;text-align: center;padding-top: 12px;">
							<div class="col-xs-6">
								<label class="containers">&#9711;
								  <input type="radio" name="condition_{{$index}}" id="condition_{{$index}}" value="OK" <?php if ($point->decision == 'OK') {
								  	echo 'checked';
								  } ?>>
							...
							<div class="col-xs-6">
								<label class="containers">&#9747;
								  <input type="radio" name="condition_{{$index}}" id="condition_{{$index}}" value="NG" <?php if ($point->decision == 'NG') {
								  	echo 'checked';
								  } ?>>
								  <span class="checkmark"></span>
								</label>
							...
							<div class="col-xs-6"  style="text-align: left;padding-left: 0px;padding-right: 5px;">
								<?php if ($point->hasil == null) {
									$bgcolor = 'gold';
									$color = 'black';
								}else{
									if ($point->hasil < $point->lower || $point->hasil > $point->upper){
										$bgcolor = 'red';
										$color = 'white';
									}else{
										$bgcolor = 'lightgreen';
								...
								Hasil<br><input type="text" id="hasil_{{$index}}" class="form-control numpad" style="background-color: {{$bgcolor}};color:{{$color}}" placeholder="Hasil" value="{{$point->hasil}}" readonly onchange="checkValue(this.value,'{{$point->lower}}','{{$point->upper}}','{{$index}}')"><br>
							</div>
							@else
						...
						<td style="border: 1px solid black;"><textarea id="note_{{$index}}"><?php echo $point->note ?></textarea></td>
	...
	function confirmAll() {
		...
		if (confirm(kata_confirm)) {
			...
			for(var i = 0; i < parseInt($('#point_length').val());i++){
				...
				$.ajax({
					url:"{{ url('input/audit/qa/special_process/continue') }}",
					method:"POST",
					data:formData,
					dataType:'JSON',
					contentType: false,
