...
<body class="hold-transition skin-purple layout-top-nav">
     ...
     <div class="content-wrapper" style="background-color: #ecf0f5; padding-top: 10px;">
      <section class="content">
        ...
        <div class="error" style="text-align: center;">
          <h1><i class="fa fa-file-text-o"></i> {{ $head }}<br><small>{{ $sub_head }}</small></h1>
          <p id="text_success">
          </p>
        </div>
  ...
  <script type="text/javascript">
    jQuery(document).ready(function() {
      if (parseInt('{{$cer_no}}') > 0) {
        approve();
      }else{
        $('#text_success').html('<h3 class="text-green"><i class="fa fa-check-circle fa-lg"></i> {{$message}}<br><small>{{$sub_message}}</small></h3>');
      }
    });
    var audio_error = new Audio('{{ url("sounds/error.mp3") }}');
    function approve() {
      $('#loading').show();
      var certificate_id = <?php echo json_encode($certificate_id); ?>;

          var data = {
            certificate_id:certificate_id,
            remark:'{{$remark}}'
          }
          $.get('{{ url("approve/qa/certificate/inprocess") }}',data, function(result, status, xhr){
        if(result.status){
          $('#loading').hide();
          $('#text_success').html('<h3 class="text-green"><i class="fa fa-check-circle fa-lg"></i> {{$message}}<br><small>{{$sub_message}}</small></h3>');
        }else{
          $('#loading').hide();
          audio_error.play();
          $('#text_success').html('<h3 class="text-red"><i class="fa fa-close fa-lg"></i> Approval Failed<br></h3>');
        }
      });
