...
<section class="content" >
	...
	<div class="row">
		...
		<input type="hidden" name="certificate_lama" id="certificate_lama">
			...
			<div class="modal-content">
				...
				<div class="modal-body table-responsive">
					<div class="col-xs-12">
						...
						<div class="row" style="padding-top: 10px" id="divKaryawan">
							<div class="col-xs-12">
								<center><span style="font-weight: bold; font-size: 18px;">Pilih Karyawan</span></center>
							</div>
							<div class="col-xs-12" style="padding-top: 10px">
								<select class="form-control select3" id="certificate_code_select" style="width: 100%" data-placeholder="Pilih Karyawan">
									<option value=""></option>
						...
						<div class="row" id="divEmp">
							<select class="form-control select3" id="employee_id_select" style="width: 100%;display: none;" data-placeholder="Pilih Karyawan" onchange="changeKaryawan(this)">
								<option value=""></option>
								@foreach($employees as $employees)
								<option value="{{$employees->employee_id}}">{{$employees->employee_id}}_{{$employees->name}}_{{$employees->department}}_{{$employees->sub_group}}</option>
								@endforeach
							</select>
						</div>
					</div>
...
<script>
	...
	function changeCode(value) {
		...
		for(var i = 0; i < certificate_code.length;i++){
			if (value === '') {
				...
				if (certificate_code[i].certificate_code.includes(value.split('_')[0]+'-'+value.split('_')[1])) {
	...
	function saveCode() {
		...
		var employee_id = $('#certificate_code_select').val().split('_')[2];

		$('#employee_id').html($('#certificate_code_select').val().split('_')[2]);
		$('#employee_name').html($('#certificate_code_select').val().split('_')[3]);
		$('#certificate_lama').val($('#certificate_code_select').val().split('_')[0]);

		// var employees = $.parseJSON('<?php echo $employees; ?>');

		// for(var i = 0; i < employees.length;i++){
			
		// }

		changeEmp(employee_id);
		...
		$('#periode').html($('#certificate_code_select').val().split('_')[5]);
		var next = new Date(new Date($('#certificate_code_select').val().split('_')[5]).setFullYear(new Date($('#certificate_code_select').val().split('_')[5]).getFullYear() + 1));
		$('#expired_date').html(newDate(next));
	...
	function changeEmp(employee_id) {
		$("#employee_id_select").val(employee_id).trigger('change');
	}

	function changeKaryawan(value) {
		if ($("#employee_id_select option:selected").text().split('_')[2] == 'Woodwind Instrument - Welding Process (WI-WP) Department' && $("#employee_id_select option:selected").text().split('_')[3] == 'FL CL AS KP') {
			$('#code_desc').html($('#code_number_select').val().split('_')[2].split(',')[0]);
		}else if ($("#employee_id_select option:selected").text().split('_')[2] == 'Woodwind Instrument - Welding Process (WI-WP) Department' && $("#employee_id_select option:selected").text().split('_')[3] == 'FL body HTS') {
	...
	function confirmAll() {
		if (confirm('Apakah Anda yakin untuk menyelesaikan proses?')) {
			...
			var certificate_lama = $('#certificate_lama').val();
			...
			var data = {
				...
				certificate_lama:certificate_lama,
			}
			$.post('{{ url("input/renew/qa/certificate/inprocess") }}', data, function(result, status, xhr){
				if(result.status){
					$('#loading').hide();
