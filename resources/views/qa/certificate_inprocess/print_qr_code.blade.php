		...
		<table style="width: 250px;  margin-left: auto; margin-right: auto;text-align: left;">
			<tr>
				<?php for ($j=0; $j < count($cer_array2[$i]); $j++) { 
					QRcode::png(url('print/qa/certificate/inprocess/'.$cer_array2[$i][$j][0]->certificate_id), public_path().'/data_file/qa/certificate_qr/'.$cer_array2[$i][$j][0]->certificate_id.'.png');
					?>
			...
			<tr>
				...
				<td style="text-align: center;border: 1px solid black; width: 120px;font-size: 8px;"><br>
					...
					<br>
					{{url('print/qa/certificate/inprocess/')}}/{{$cer_array2[$i][$j][0]->certificate_id}}
				</td>
				<td>&nbsp;</td>
			<?php } ?>
			</tr>
