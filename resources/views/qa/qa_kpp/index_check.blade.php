	...
	<div class="row" style="padding-left: 10px; padding-right: 10px;">
		<div class="col-xs-6" style="padding-right: 0; padding-left: 0">
			<table class="table table-bordered" style="width: 100%; margin-bottom: 2px;" border="1">
				<tbody>
					...
					<tr>
						...
						<td style="background-color: #14213d; color: white; text-align: center; font-size:15px; width: 2%;" id="op">{{$emp->employee_id}}</td>
						<td style="background-color: #fca311; text-align: center; color: #14213d; font-size: 15px;width: 3%;" id="op2">{{$emp->name}}</td>
						<td style="background-color: #14213d; text-align: center;color: white; font-size:15px;width: 2%;" id="loc">{{$location}}</td>
					</tr>					
				</tbody>
			</table>
			<table class="table table-bordered" style="width: 100%; margin-bottom: 5px;border: 0">
				<tbody>
					<tr>
						<td style="background-color: #d1d1d1; text-align: center; color: #14213d; padding:0;font-size: 20px;font-weight: bold;" colspan="3">
							MATERIAL
						</td>
					</tr>
					<tr>
						<td colspan="3">
							<input type="text" class="pull-right" name="material_number" style="height: 50px;font-size: 2vw;width: 100%;text-align: center;vertical-align: middle;color: #14213d" id="material_number" placeholder="Material Number">
						</td>
					</tr>
					<tr>
						...
						<td style="background-color: #d1d1d1; text-align: center; color: #14213d; padding:0;font-size: 15px; width: 60%">
					...
					<tr>
						<td id="material_number_real" style="background-color: #fca311; text-align: center; color: #14213d; font-size: 20px;">
						</td>
						<td id="material_description" style="background-color: #fca311; text-align: center; color: #14213d; font-size: 20px;">-
						</td>
						<td id="hpl" style="background-color: #fca311; text-align: center; color: #14213d; font-size: 20px;">-
						</td>
					</tr>
				</tbody>
			</table>
			<table class="table table-bordered" style="width: 100%; margin-bottom: 5px;border: 0">
				<tbody>
					...
					<tr>
						<td>
							<input type="text" class="pull-right numpad" name="qty_rec" style="height: 50px;font-size: 2vw;width: 100%;text-align: center;vertical-align: middle;color: #14213d" id="qty_rec" placeholder="Quantity Production">
						</td>
						<td>
			...
			<table class="table table-bordered" style="width: 100%; margin-bottom: 5px;border: 0">
				<tbody>
					<tr>
						<td style="background-color: #d1d1d1; text-align: center; color: #14213d; padding:0;font-size: 20px;font-weight: bold;" colspan="2">
							INSPECTION LEVEL
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="2">
							<select id="note_all" name="note_all" style="height: 50px;width: 100%; font-size: 2vw; text-align: center; vertical-align: middle;">
								<option value="Pilih Note">Pilih Note</option>
					...
					<table class="table table-bordered" id="tableDetail">
						<thead style="border-bottom:3px solid black;border-top:3px solid black;background-color:#cddc39">
							<tr>
								...
								<th style="font-weight: bold;" rowspan="2">Qty Production</th>
...
<script>
	...
	function cancelAll() {		
		...
		if ($('#incoming_check_code').val() != "") {
			...
			$.get('{{ url("delete/qa/kensa/ng_temp") }}', data, function(result, status, xhr){
				if(result.status){
	...
	$('#material_number').keypress(function(e){
		if(e.keyCode==13 || e.keyCode==9) {
			$('#material_number').val($("#material_number").val().toUpperCase());
			var data = {
				material_number:$("#material_number").val(),
				location:'{{$location}}'
			}
			$.get('{{ url("fetch/qa/kpp/check_material") }}', data, function(result, status, xhr){
				if(result.status){
					openSuccessGritter('Success', 'Material Ditemukan');
					$('#material_description').html(result.material[0].product_name);
					$("#material_number_real").text(result.material[0].product_gmc);
					$("#qty_rec").val(result.material[0].product_qty_cs);
					$("#hpl").html(result.material[0].hpl);
					$('#start_time').val(getActualFullDate());
				}
				else{
					$('#material_description').html("-");
	...
	function showModalNg(ng_name) {
		if ($('#material_number').val() == "" || $('#qty_rec').val() == "" || $('#qty_check').val() == "" || $('#inspection_level').val() == "-") {
	...
	function confNgTemp() {
		if ($('#qty_ng').val() == "" || $('#status_ng').val() == "-") {
			...
			var material_number = $('#material_number_real').text();
			...
			$.post('{{ url("input/qa/kensa_check/ng_temp") }}', data, function(result, status, xhr){
				if(result.status){
					openSuccessGritter('Success!', result.message);
					$('#note_ng').val('');
					$('#qty_ng').val('');
	...
	function fetchNgTemp() {
		...
		$.get('{{ url("fetch/qa/kensa/ng_temp") }}', data, function(result, status, xhr){
			if(result.status){
				openSuccessGritter('Success!', result.message);
				var ngTemp = "";
				$('#bodyNgTemp').html("");
	...
	function confirmNgLog() {
		if ($('#material_number').val() == "" || $('#qty_rec').val() == "" || $('#qty_check').val() == "" || $('#inspection_level').val() == "-" || $("#note_all").val() == 'Pilih Note') {
			alert('Isi Semua Data');
		}else{
			$("#loading").show();
			...
			var material_number = $('#material_number_real').text();
			var material_description = $('#material_description').text();
			var qty_production = $('#qty_rec').val();
			var qty_check = $('#qty_check').val();
			var inspection_level = $('#inspection_level').val();
			var inspector = $('#employee_id').val();
			var location = $('#location').val();
			...
			$.post('{{ url("input/qa/kensa/ng_log") }}', data, function(result, status, xhr){
				if(result.status){
					$("#loading").hide();
					openSuccessGritter('Success!', result.message);
					cancelAll();
	...
	function fetchDetailRecord() {
		...
		$.get('{{ url("fetch/qa/kensa/detail_record") }}', data, function(result, status, xhr){
			if(result.status){
				...
				$.each(result.detail, function(key, value) {
					...
					tableData += '<td rowspan="'+jumlah+'">'+ value.qty_production +'</td>';
