						...
						<div class="col-md-8 col-md-offset-2">
							<div class="col-md-12">
								<div class="form-group pull-right">
									<a href="{{ url('index/qa/audit_fg/') }}" class="btn btn-warning">Back</a>
									<a href="{{ url('index/qa/audit_fg/point_check/') }}" class="btn btn-danger">Clear</a>
							...
							<div class="box-body">
								...
								<div class="col-xs-12">
									<div class="form-group row" align="right">
										<label class="col-sm-2">Pilih Data</label>
										<div class="col-sm-10">
											<div class="col-xs-6" style="text-align: center;">
												<label class="containers">NEW
												  <input type="radio" name="data_condition" id="data_condition" value="NEW" onclick="checkCondition(this)">
												  <span class="checkmark"></span>
												</label>
											</div>
											<div class="col-xs-6" style="text-align: center;">
												<label class="containers">EXISTING
												  <input type="radio" name="data_condition" id="data_condition" value="EXISTING" onclick="checkCondition(this)">
												  <span class="checkmark"></span>
												</label>
											</div>
										</div>
									...
									<div id="div_existing">
										<div class="form-group row" align="right">
											...
											<div class="col-sm-10" align="left" id="divAddPointExisting">
												<select class="form-control" name="add_material_existing" id="add_material_existing" data-placeholder="Pilih Material" style="width: 100%;">
													<option value=""></option>
													@foreach($point as $point)
													<option value="{{$point->product}}_{{$point->material_number}}_{{$point->material_description}}">{{$point->product}} - {{$point->material_number}} - {{$point->material_description}}</option>
													@endforeach
												</select>
											</div>
										</div>
						...
						<div class="col-xs-12">
							<div class="box-body">
								...
								<div class="col-xs-12">
									...
									<div class="form-group row" align="right">
										<label class="col-sm-2">Point Check / Urutan</label>
										<div class="col-sm-10" align="left">
											<textarea id="edit_point_check" style="width: 100%"></textarea>
										</div>
									</div>
									<div class="form-group row" align="right">
										<label class="col-sm-2">Point Check Detail</label>
										<div class="col-sm-10" align="left">
											<textarea id="edit_point_check_details" style="width: 100%"></textarea>
										</div>
									</div>
									<div class="form-group row" align="right">
									...
									<div class="form-group row" align="right">
										<label class="col-sm-2">Standard</label>
										<div class="col-sm-10" align="left" id="divEditDefect">
											<select class="form-control" id="edit_defect_category" style="width:100%" data-placeholder="Pilih Defect Category">
											<option value=""></option>
											<option value="Critical">Critical</option>
											<option value="Major">Major</option>
											<option value="Minor">Minor</option>
											<option value="Sesuai Area">Sesuai Area</option>
											</select>
										</div>
									</div>
								</div>
...
<script>
	...
	function cancelAll() {
		...
		$("#qty_point").html(0);
		$('#add_count_point').val(1);
		$("#add_material_existing").val('').trigger('change');
		...
		$("#edit_defect_category").val('').trigger('change');
		$("#edit_point_check").html(CKEDITOR.instances.edit_point_check.setData(''));
		$("#edit_point_check_details").html(CKEDITOR.instances.edit_point_check.setData(''));
	...
	function addPoint() {
		...
		points += '<div class="col-xs-12 point_check_class" id="divPoint_'+id+'" style="border: 1px solid black;padding-left: 0px;padding-right: 0px;margin-bottom:10px;">';
			...
			points += '<div class="form-group col-xs-12" align="right">';
				points += '<label class="col-sm-2">Point Check / Urutan</label>';
				points += '<div class="col-sm-10" align="left">';
					points += '<textarea id="add_point_check_'+id+'" style="width: 100%"></textarea>';
				points += '</div>';
			points += '</div>';
			points += '<div class="form-group col-xs-12" align="right">';
				points += '<label class="col-sm-2">Point Check Detail</label>';
				points += '<div class="col-sm-10" align="left">';
					points += '<textarea id="add_point_check_details_'+id+'" style="width: 100%"></textarea>';
				points += '</div>';
			points += '</div>';
			points += '<div class="form-group col-xs-12" align="right">';
			...
			points += '<div class="form-group col-xs-12" align="right">';
				points += '<label class="col-sm-2">Standard</label>';
				points += '<div class="col-sm-10" align="left" id="divDefectCategory_'+id+'">';
					points += '<select class="form-control" id="add_defect_category_'+id+'" style="width:100%" data-placeholder="Pilih Defect Category">';
					points += '<option value=""></option>';
					points += '<option value="Critical">Critical</option>';
					points += '<option value="Major">Major</option>';
					points += '<option value="Minor">Minor</option>';
					points += '<option value="Sesuai Area">Sesuai Area</option>';
					points += '</select>';
				points += '</div>';
			points += '</div>';
		points += '</div>';
	    ...
	    CKEDITOR.replace('add_point_check_details_'+id ,{
	        filebrowserImageBrowseUrl : '{{ url('kcfinder_master') }}'
	    });
	    ...
	    $('#add_defect_category_'+id).select2({
			allowClear:true,
			dropdownParent: $('#divDefectCategory_'+id),
		});
	...
	$(function () {
		...
		$('#add_material_existing').select2({
			allowClear:true,
			dropdownParent: $('#divAddPointExisting'),
		});
		...
		$('#edit_defect_category').select2({
			allowClear:true,
			dropdownParent: $('#divEditDefect'),
		});
	    ...
	    CKEDITOR.replace('edit_point_check_details' ,{
	        filebrowserImageBrowseUrl : '{{ url('kcfinder_master') }}'
	    });
		...
		$.get('{{ url("fetch/qa/audit_fg/point_check") }}',data, function(result, status, xhr){
			if(result.status){
				...
				$.each(result.point_check, function(key, value) {
					...
					tableData += '<td style="text-align:left;padding-left:7px;">'+ value.point_check +'</td>';
					tableData += '<td style="text-align:left;padding-left:7px;">'+ value.point_check_details +'</td>';
					tableData += '<td style="text-align:left;padding-left:7px;">'+ value.standard +'</td>';
					tableData += '<td style="text-align:left;padding-left:7px;">'+ value.defect_category +'</td>';
	...
	function add() {
		if (confirm('Apakah Anda yakin?')) {
			$("#loading").show();
			var data_condition = '';
			$("input[name='data_condition']:checked").each(function (i) {
				data_condition = $(this).val();
	        });
	        var product = '';
	        var material_number = '';
	        ...
	        if (data_condition == 'NEW') {
				...
				var material_numbers = [];
				var material_descriptions = [];
				for(var i = 0; i < material.length;i++){
					material_numbers.push(material[i].split('_')[0]);
					material_descriptions.push(material[i].split('_')[1]);
				}

				material_number = material_numbers.join(',');
				material_description = material_descriptions.join(',');
	        }else{
	        	product = $('#add_material_existing').val().split('_')[0];
	        	material_number = $('#add_material_existing').val().split('_')[1];
	        	material_description = $('#add_material_existing').val().split('_')[2];
	        }
			...
			for(var i = 1; i <= parseInt(qty_point);i++){
				if ($('#divPoint_'+i).text() != '') {
					...
					defect_category.push($('#add_defect_category_'+i).val());
					point_check_details.push(CKEDITOR.instances['add_point_check_details_'+i].getData());
				}
			}
			...
			$.post('{{ url("input/qa/audit_fg/point_check") }}',data, function(result, status, xhr){
				if(result.status){
	...
	function editPointCheck(id) {
		...
		$.get('{{ url("edit/qa/audit_fg/point_check") }}',data, function(result, status, xhr){
			if(result.status){
				$('#loading').hide();
				...
				$("#edit_point_check_details").html(CKEDITOR.instances.edit_point_check_details.setData(result.point.point_check_details));
				$('#edit_defect_category').val(result.point.defect_category).trigger('change');
			}else{
				openErrorGritter('Error!',result.message);
				audio_error.play();
				$('#loading').hide();
	...
	function deletePointCheck(id) {
		if (confirm('Apakah Anda yakin akan menghapus Point Check?')) {
			...
			$.get('{{ url("delete/qa/audit_fg/point_check") }}',data, function(result, status, xhr){
				if(result.status){
					$('#loading').hide();
					fillList();
	...
	function update() {
		if (confirm('Apakah Anda yakin?')) {
			...
			var defect_category = $('#edit_defect_category').val();
			var point_check_details = CKEDITOR.instances['edit_point_check_details'].getData();
			...
			$.post('{{ url("update/qa/audit_fg/point_check") }}',data, function(result, status, xhr){
				if(result.status){
