...
<script>
	...
	function fillChart(id){
		...
		$.get('{{ url("fetch/daily_production_result_kd") }}', data, function(result, status, xhr){
			if(xhr.status == 200){
				if(result.status){
					...
					for (var i = 1; i < 3; i++) {
						...
						for (i = 0; i < data.length; i++) {
							...
							else if(data[i].hpl == 'SUBASSY-CL'){
								hpl = 'CL KEY';
							}
							else if(data[i].hpl == 'SUBASSY-SX'){
								hpl = 'SX KEY';
							}
							else{
								hpl = "Undefined";
						...
						for (i = 0; i < data2.length; i++) {
							if(jQuery.inArray(data2[i].hpl, ['SUBASSY-FL', 'SUBASSY-CL', 'SUBASSY-SX', 'ASSY-SX', 'MP', 'ZPRO']) !== -1){
								if(data2[i].hpl == 'SUBASSY-FL'){
									hpl = 'FL KEY';
								}
								else if(data2[i].hpl == 'SUBASSY-CL'){
									hpl = 'CL KEY';
								}
								else if(data2[i].hpl == 'SUBASSY-SX'){
									hpl = 'SX KEY';
								}
								else{
									hpl = "Undefined";
...
function modalAccuracy(hpl, name, now, first, last){
	...
	$.get('{{ url("fetch/production_accuracy_modal_kd") }}', data, function(result, status, xhr){
		if(xhr.status == 200){
			if(result.status){
				$('#modalResultTitle').html('');
				$('#modalResultTitle').html('Detail of '+ hpl +' '+ name);
