  ...
  <header class="main-header" >
   ...
   <div class="content-wrapper" style="background-color: #ecf0f5; padding-top: 10px;">
    <section class="content">
     <form id ="importForm" name="importForm" method="post" action="{{ url('save/approval/reject') }}" enctype="multipart/form-data">
      <input type="hidden" value="{{csrf_token()}}" name="_token" />
      <input type="hidden" value="{{$request_id}}" name="request_id" />
      <input type="hidden" value="{{$remark}}" name="position" />
      <div class="box box-solid" id="div_reason">
        <div class="box-body">
         <div style="text-align: center;">
            ...
            <h1><b><i class="fa fa-file-text-o"></i> Pelaporan Kanagata Retak Approval</b></h1>
            <h3><i class="fa fa-file-text-o"></i> 金型割れ報告の承認と</h3>
            <span style="font-weight: bold; color: purple; background-color: #f39c12; font-size: 24px;">REJECTED</span>
            <p>
              <h3 class="text-red">
                <i class="fa fa-times-circle-o"></i> <i class="fa fa-file"></i> {{$request_id}}  <br>
              </h3>
            </p>
          </div>
          ...
          <div style="text-align: center; padding-top: 10px; padding-bottom: : 10px;">
            <button class="btn btn-success" onclick="$('[name=importForm]');">
              CONFIRM (確認)
            </button>
          </div>
        </div>
...
<script type="text/javascript">
  ...
  $("#importForm").submit(function(){
    if (!confirm("Apakah Anda Yakin Ingin Melakukan Rejected??")) {
      return false;
    } else {
      setInterval($("#loading").show(), 4000);
    this.submit();
  }
});
