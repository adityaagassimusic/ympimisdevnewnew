...
<section class="content" style="padding-top: 0;">
	...
	<div class="row">
		<div class="col-xs-12" style="margin-left: 0px;margin-right: 0px;padding-bottom: 10px;">
			<div class="col-xs-12" style="background-color: rgb(126,86,134);padding-left: 5px;padding-right: 5px;height:36px;vertical-align: middle;">
				<center>
					<span style="font-size: 20px;color: white;width: 100%;font-weight: bold;">Monitoring Dokumentasi Packing</span></center>
			</div>
	             	...
	             	<table id="tableDetail" class="table table-striped table-bordered" style="width: 100%;">
				        <thead>
					        <tr style="border-bottom:3px solid black;border-top:3px solid black;background-color:#cddc39">
						        ...
						        <th style="padding: 5px;text-align: left;width: 1%">Detail Karyawan</th>
						        <th style="padding: 5px;text-align: center;width: 1%">Serial Number</th>
						        <th style="padding: 5px;text-align: center;width: 6%">Foto</th>
	...
	<script>
		...
		function fetchChart(){
			$.get('{{ url("fetch/monitoring/packing_documentation") }}',function(result, status, xhr){
				if(result.status){
					...
					$.each(result.doc, function(key, value){
						...
						sax.push(parseInt(value.Saxophone));
			...
			for(var i = 0; i < documentation.length;i++){
				if (getMonthName(new Date(documentation[i].created_at)) === bulan){
					...
					if (documentation[i].location === status) {
						tableDetail += '<tr>';
						tableDetail += '<td width="1%">'+documentation[i].location+'</td>';
						tableDetail += '<td width="1%">'+documentation[i].employee_id+' '+documentation[i].employee_name+'</td>';
						tableDetail += '<td width="1%">'+documentation[i].serial_number+'</td>';
						if (documentation[i].photo != null) {
	           var data = JSON.parse(documentation[i].photo);
	            tableDetail += '<td width="6%" style="text-align:center">'
	            for (var u = 0; u < data.length; u++) { 
	              tableDetail += '<img src="{{ url("images/packing") }}/'+data[u]+'" width="200">&nbsp;';

	              // tableDetail += '<a target="_blank" href="{{ url("images/packing") }}/'+data[u]+'"><i class="fa fa-paperclip"></i></a>';
	            }
	            tableDetail += '</td>'
	          }
	          else{
	            tableDetail += '<td></td>'
						...
						tableDetail += '<td width="1%">'+documentation[i].created_at+'</td>';
						tableDetail += '</tr>';
					}
				}
			}
