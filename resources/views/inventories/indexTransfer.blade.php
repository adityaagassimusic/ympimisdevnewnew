...
<section class="content-header">
	<h1>
		Transfer <span class="text-purple">????</span>
							...
							<table id="transferTable" class="table table-bordered table-striped table-hover">
								<thead style="background-color: rgba(126,86,134,.7);">
									<tr>
										...
										<th style="width:15%;">Transfered At</th>
									</tr>
								</thead>
...
<script>
	...
	function fetchTable(){
		...
		$.get('{{ url("fetch/tr_transfer") }}', data, function(result, status, xhr){
			console.log(status);
			console.log(result);
			console.log(xhr);
			if(xhr.status == 200){
				if(result.status){
					...
					$.each(result.tableData, function(key, value) {
						...
						if(value.transfer != '-'){
							tableData += '<td><a href="javascript:void(0)" id="'+ value.transfer +'" onClick="downloadFile(id)">'+ value.transfer +'</a></td>';
						}
						else{
	...
	function downloadFile(id){
		...
		$.get('{{ url("download/tr_transfer") }}', data, function(result, status, xhr){
			console.log(status);
			console.log(result);
			console.log(xhr);
			if(xhr.status == 200){
