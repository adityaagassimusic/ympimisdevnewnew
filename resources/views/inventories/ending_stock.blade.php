            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myModalLabel">Upload Ending Stock</h4>
                        Format :
                        ...
                        <div class="row">
                            <div class="col-xs-10 col-xs-offset-1">
                                <div class="col-xs-12" style="margin-top: 2%;">
                                    <label>Ending Stock Month :<span class="text-red">*</span></label>
                                </div>
...
@section('scripts')
    ...
    <script>
        ...
        function drawTable() {
            ...
            $.get('{{ url('fetch/ending_stock') }}', data, function(result, status, xhr) {
                ...
                if (result.status) {
                    ...
                    var tableHead = '<tr>';
                    tableHead +=
                        '<th style="vertical-align: middle; text-align: center; background-color:#a488aa;">GMC</th>';
                    tableHead +=
                        '<th style="vertical-align: middle; text-align: center; background-color:#a488aa;">DESC</th>';
                    tableHead +=
                        '<th style="vertical-align: middle; text-align: center; background-color:#a488aa;">CATEGORY</th>';
                    tableHead +=
                        '<th style="vertical-align: middle; text-align: center; background-color:#a488aa;">HPL</th>';
                    for (var i = 0; i < result.interval_month.length; i++) {
                        tableHead +=
                            '<th style="vertical-align: middle; text-align: center; background-color: #b4c6e7;">';
                        tableHead += result.interval_month[i].end_month.toUpperCase() + '</th>';
                    ...
                    tableFoot += '<th style="background-color:#a488aa;"></th>';
                    for (var i = 0; i < result.interval_month.length; i++) {
                        tableFoot += '<th style="background-color:#b4c6e7;"></th>';
                    }
                    tableFoot += '</tr>';
                    ...
                    for (var i = 0; i < result.materials.length; i++) {
                        ...
                        tableBody += '<td style="vertical-align: middle; text-align: center;">';
                        tableBody += result.materials[i].category + '</td>';

                        tableBody += '<td style="vertical-align: middle; text-align: center;">';
                        tableBody += result.materials[i].hpl + '</td>';


                        for (var j = 0; j < result.interval_month.length; j++) {

                            var inserted = false;

                            for (var k = 0; k < result.stock.length; k++) {
                                if ((result.stock[k].material_number == result.materials[i].material_number) &&
                                    (result.stock[k].stock_date.substr(0, 7) == result.interval_month[j].month)) {
                                    ...
                                    tableBody += 'id="' + result.stock[k].id + '" >';
                                    tableBody += result.stock[k].quantity + '</td>';
        ...
        function uploadData() {
            ...
            $('#loading').show();
            $.post('{{ url('input/ending_stock') }}', data, function(result, status, xhr) {
                if (result.status) {
                    ...
                    $('#loading').hide();
                    openSuccessGritter('Success', 'Upload Ending Stock Success');
