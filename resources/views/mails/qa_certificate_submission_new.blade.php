...
<body>
	<div>
		<center>
			...
			<span style="font-weight: bold; color: purple; font-size: 24px;">NEW KENSA CERTIFICATE REQUEST<br></span><br>
			<br>
		...
		<center>
			...
			<div style="width: 60%">
				<table style="border:1px solid black; border-collapse: collapse;">
					<thead style="">
						<tr>
							<th style="background-color: rgb(126,86,134);color: white;font-size: 15px;border:1px solid black;width: 2%">Request ID</th>
							<th style="background-color: rgb(126,86,134);color: white;font-size: 15px;border:1px solid black;width: 5%">Employee ID</th>
							<th style="background-color: rgb(126,86,134);color: white;font-size: 15px;border:1px solid black;width: 5%">Name</th>
						...
						<tr>
							<td style="border:1px solid black; font-size: 15px; width: 20%; height: 20;text-align: left;">{{$data['news'][$i]->request_id}}</td>
							<td style="border:1px solid black; font-size: 15px; width: 20%; height: 20;text-align: left;">
							{{$data['news'][$i]->employee_id}}
							</td>
							<td style="border:1px solid black; font-size: 15px; width: 20%; height: 20;text-align: left;">
							{{$data['news'][$i]->name}}
							</td>
							...
							<?php for ($j=0; $j < count($data['emp']); $j++) { 
								if ($data['news'][$i]->employee_id == $data['emp'][$j]->employee_id) {
						...
						<?php array_push($request_id, $data['news'][$i]->request_id) ?>
						<?php } ?>
					</thead>
					<!-- <tbody align="center">
			...
			<div style="width: 80%">
				<br>
					<?php if (!ISSET($data['complete']) && !ISSET($data['reject'])){ ?>
						...
						(下をクリックしてください)
						<br>
						<a style="background-color: green; width: 50px;text-decoration: none;color: white;font-size:20px;" href="{{ url('approval/submission/qa/certificate/new/'.$data['next_remark'].'/'.join(',',$request_id)) }}">&nbsp;&nbsp;&nbsp; Approve (承認) &nbsp;&nbsp;&nbsp;</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<a style="background-color: red; width: 50px;text-decoration: none;color: white;font-size:20px;margin-left: 50px" href="{{ url('reject/submission/qa/certificate/new/'.$data['next_remark'].'/'.join(',',$request_id)) }}">&nbsp;&nbsp;&nbsp; Reject (却下) &nbsp;&nbsp;&nbsp;</a>
					<?php } ?>
					<br><br>
