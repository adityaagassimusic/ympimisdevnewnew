                    ...
                    <div class="box">
                        <div class="box-body">
                            ...
                            <div class="input-group input-group-lg">
                                ...
                                <input type="text" class="form-control" placeholder="SCAN FLO / KDO / EO" id="slip">
                                <div class="input-group-addon" id="icon-serial"
                                    style="font-weight: bold; border-color: none; font-size: 18px;">
        ...
        <div class="row">
            <div class="col-xs-12 col-md-6 col-lg-6" style="padding-left: 5px; padding-right: 5px;">
                <div class="col-xs-12" style="padding-right: 0; padding-left: 0; margin-bottom: 2%;">
                    <div class="box">
                        <div class="box-body">
                            <h3 style="font-weight: bold; margin-top: 0px; margin-bottom: 2%;">MIRAI vs PI</h3>
...
@section('scripts')
    ...
    <script>
        ...
        function exportResult() {
            ...
            $("#loading").show();
            $.post('{{ url('export/stocktaking/fstk_pi') }}', data, function(result, status, xhr) {
                if (result.status) {
        ...
        function scanSlip(slip) {
            ...
            $.get('{{ url('fetch/stocktaking/check_slip') }}', data, function(result, status, xhr) {
                if (result.status) {
                    $("#loading").hide();
                    ...
                    $.each(result.data, function(key, value) {
                        ...
                        tableData += value.material_number.toUpperCase();
                        tableData += '</td>';

                        var material_description = '';
                        for (let i = 0; i < mpdl.length; i++) {
                            if (mpdl[i].material_number == value.material_number.toUpperCase()) {
                                material_description = mpdl[i].material_description;
                                break;
                            }
                        }

                        tableData += '<td style="text-align: left; width: 50%;">';
                        tableData += material_description;
                        tableData += '</td>';

                        tableData += '<td style="text-align: center; width: 20%;">';
                        tableData += '<input style="text-align: center; width: 100%;" ';
                        tableData += 'id="quantity_' + count_row + '" type="number" ';
                        tableData += 'value="' + value.quantity + '">';
                        tableData += '</td>';
                    ...
                    if (confirm(
                        ...
                        revisePI(result.stocktaking, slip);
                    } else {
                        $("#slip").val("");
                        // $("#slip").focus();
                        setTimeout(function() {
        ...
        function revisePI(stocktaking, slip) {
            ...
            $("#loading").show();
            $.post('{{ url('delete/stocktaking/fstk_pi') }}', data, function(result, status, xhr) {
                if (result.status) {
        ...
        function submitPi() {
            ...
            $("#loading").show();
            $.post('{{ url('input/stocktaking/fstk_pi') }}', data, function(result, status, xhr) {
                if (result.status) {
        ...
        function fetchCountFstk() {
            $("#loading").show();

            $.get('{{ url('fetch/stocktaking/count_fstk') }}', function(result, status, xhr) {
                if (result.status) {
                    ...
                    head_1 += '<th style="vertical-align: middle; text-align: center;">MIRAI</th>';
                    ...
                    $('#body_1').html("");
                    var body_1 = '';
                    $.each(result.slip_new, function(key, value) {
                    ...
                    $.each(result.resume, function(key, value) {
                        ...
                        sum_pi_qty += value.pi;
                        body_2 += '</td>';
                        var diff_qty = value.pi - value.ymes;
                        var bg_color = 'background-color : #ffccff;';
                        if (diff_qty == 0) {
                            bg_color = 'background-color : #ccffff;';
                        }
                        body_2 += '<td style="' + padding + bg_color +
                            ' width : 10%; text-align: right;">';
                        body_2 += diff_qty;
                        body_2 += '</td>';
                        body_2 += '<td style="' + padding + bg_color +
                            ' width : 10%; text-align: right;">';
                        var amt_var = Math.abs(diff_qty) * price;
                        sum_ymes_book += value.ymes * price;
                        sum_pi_book += value.pi * price;
                    ...
                    $('#sum_ymes_book').text(Math.round(sum_ymes_book).toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                        style: "currency",
                        currency: "USD"
