...
@section('scripts')
    ...
    <script>
        ...
        function fillModal(cat, name) {
            ...
            $.get('{{ url('fetch/stocktaking/daily_report_modal') }}', data, function(result, status, xhr) {
                if (result.status) {
                    ...
                    $.each(result.variance, function(key, value) {
                        ...
                        if (value.diff_abs > 0) {
                            ...
                            resultData += '<td style="width: 1%; font-weight: bold;">' + value.diff_qty
                                .toLocaleString() + '</td>';
                            resultData += '<td style="width: 1%; font-weight: bold;">' + value.diff_abs
                                .toLocaleString() + '</td>';
                            resultData += '</tr>';
        ...
        function fillChart() {
            ...
            $.get('{{ url('fetch/stocktaking/daily_report') }}', data, function(result, status, xhr) {
                if (result.status) {
                    ...
                    for (var i = 0; i < result.variances.length; i++) {
                        ...
                        if (result.variances[i].storage_location == 'SX91') {
                            sx91.push(Math.round(parseInt(result.variances[i].variance) / (parseInt(result
                                .variances[i].variance) + parseInt(result.variances[i].ok)) * 100));
                        }

                        if (result.variances[i].storage_location == 'FL91') {
                            fl91.push(Math.round(parseInt(result.variances[i].variance) / (parseInt(result
                                .variances[i].variance) + parseInt(result.variances[i].ok)) * 100));
                        }
                    ...
                    window.chart2 = Highcharts.chart('container2', {
                        ...
                        title: {
                            text: 'Daily Stock Taking Report Trend',
                            style: {
                                fontSize: '30px',
                                fontWeight: 'bold'
                            }
                    ...
                    window.chart = Highcharts.chart('container', {
                        ...
                        title: {
                            text: 'Daily Stock Taking Report',
                            style: {
                                fontSize: '30px',
                                fontWeight: 'bold'
                            }
