...
<section class="content" style="padding-top: 0;">
	...
	<div class="row">
		<center>
		<text x="606" text-anchor="middle" class="highcharts-title" data-z-index="4" style="color:#E0E0E3;font-size:25px;text-transform:uppercase;font-weight:bold;fill:#E0E0E3;" y="25">Resume Peserta Training Code of Conduct (Kode Etik)</text>
		</center>
	...
	<script>
		...
		var kode_etik_detail = [];
		var periode = '';

		function fetchChart(){
		
			$.get('{{ url("fetch/kodeEtik") }}',function(result, status, xhr){
				if(result.status){
					...
					$.each(result.kode_etik, function(key, value){
					...
					$.each(result.kode_etik_detail, function(key2, value2){
						kode_etik_detail.push({
		...
		function ShowModal(department_shortname,status_cek,department) {
			...
			var tableDetail = '';
			for(var i = 0; i < kode_etik_detail.length;i++){
				if (kode_etik_detail[i].department === department) {
					if (kode_etik_detail[i].status_cek === status_cek) {
						tableDetail += '<tr>';
						tableDetail += '<td>'+kode_etik_detail[i].employee_id+'</td>';
						tableDetail += '<td>'+kode_etik_detail[i].name+'</td>';
						tableDetail += '<td>'+(kode_etik_detail[i].department_shortname || '')+'</td>';
						tableDetail += '<td>'+(kode_etik_detail[i].section || '')+'</td>';
						tableDetail += '<td>'+(kode_etik_detail[i].group || '')+'</td>';
						tableDetail += '<td>'+(kode_etik_detail[i].sub_group || '')+'</td>';
						tableDetail += '<td>'+(kode_etik_detail[i].status_cek || '')+'</td>';
						tableDetail += '</tr>';
					}
				}
			}
            ...
            $('#judul_weekly').html('Detail '+status_cek+' Training Post Test Kode Etik Kepatuhan<br>'+(department || 'Management'));
		...
		function ShowModalAll(status_cek) {
            ...
            $('#judul_weekly').html('Detail '+status_cek+' Training Post Test Kode Etik Kepatuhan<br>');
			$('#modalDetail').modal('show');
			$('#loading').hide();
		}
