	...
	<div class="row">
		<div class="col-xs-12" style="margin-top: 0px;">
			<div class="col-xs-2" style="padding-right: 0;">
					...
					<div class="small-box" style="background: #f44336; height: 200px; margin-bottom: 5px;cursor: pointer;" onclick="ShowModalAll('Tidak')">
						<div class="inner" style="padding-bottom: 0px;">
							<h3 style="margin-bottom: 0px;font-size: 2vw;"><b>TIDAK</b></h3>
							<h5 style="font-size: 3vw; font-weight: bold;" id="total_tidak">0</h5>
							<h4 style="font-size: 2.5vw; font-weight: bold;padding-top: 0px" id="persen_tidak">0 %</h4>
						</div>
						<div class="icon" style="padding-top: 40px;">
							<i class="fa fa-remove"></i>
						</div>
					</div>
					<div class="small-box" style="background: #ff9800; height: 200px; margin-bottom: 5px;cursor: pointer;" onclick="ShowModalAll('Belum')">
						<div class="inner" style="padding-bottom: 0px;">
...
<script>
	...
	function fillChart() {
		...
		$.get('{{ url("fetch/hasil_mcu") }}',function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
					...
					total_all = total_sudah + total_belum + total_tidak;
	...
	function ShowModal(dept,answer) {
		...
		$.get('{{ url("fetch/hasil_mcu/detail") }}', data, function(result, status, xhr) {
			if(result.status){
 ...
 function ShowModalAll(answer) {
		...
		$.get('{{ url("fetch/hasil_mcu/detailAll") }}', data, function(result, status, xhr) {
			if(result.status){
				...
				$.each(result.mcu, function(key, value) {
					resultData += '<tr>';
					resultData += '<td>'+ index +'</td>';
					resultData += '<td>'+ value.employee_id +'</td>';
					resultData += '<td>'+ value.name +'</td>';
