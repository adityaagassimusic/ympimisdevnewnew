...
<section class="content" style="padding-top: 0;">
	...
	<div class="row">
		<div class="col-xs-12" style="margin-left: 0px;margin-right: 0px;padding-bottom: 10px;">
			...
			<div class="col-xs-2" style="padding-left: 10px;padding-right: 10px">
					<select style="width: 100%;text-align: left;" class="form-control select2" id="periode_pkb" data-placeholder="Pilih Periode">
						<option value=""></option>
	...
	<script>
		...
		function fetchChart(){
			$('#loading').show();
			var data = {
				periode:$('#periode_pkb').val()
			}
			$.get('{{ url("fetch/pkb") }}', data,function(result, status, xhr){
				if(result.status){
		...
		function ShowModal(department_shortname,status_cek,department) {
			...
			var tableDetail = '';
			for(var i = 0; i < pkb_detail.length;i++){
				if (pkb_detail[i].department === department) {
					if (pkb_detail[i].status_cek === status_cek) {
						tableDetail += '<tr>';
						tableDetail += '<td>'+pkb_detail[i].employee_id+'</td>';
						tableDetail += '<td>'+pkb_detail[i].name+'</td>';
						tableDetail += '<td>'+(pkb_detail[i].department_shortname || '')+'</td>';
						tableDetail += '<td>'+(pkb_detail[i].section || '')+'</td>';
						tableDetail += '<td>'+(pkb_detail[i].group || '')+'</td>';
						tableDetail += '<td>'+(pkb_detail[i].sub_group || '')+'</td>';
						tableDetail += '<td>'+(pkb_detail[i].status_cek || '')+'</td>';
						tableDetail += '</tr>';
					}
				}
			}
            ...
            $('#judul_weekly').html('Detail '+status_cek+' Menyetujui Surat Pernyataan PKB<br>'+(department || 'Management')+'<br>Periode '+periode);
