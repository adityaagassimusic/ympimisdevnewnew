...
<script>
	...
	function DataResume(){
		...
		$.get('{{ url("fetch/data/pencabutan_tunjangan") }}', function(result, status, xhr){
			if (result.status) {
				$.each(result.data_resumes, function(key, value) {
					var anak = value.tahun_anak.split(',');
					var tahun_sekarang = result.tahun_sekarang;

					var test = [];
					var b = [];
					for (var i = 0; i < anak.length; i++) {
						test.push(parseInt(tahun_sekarang-anak[i]));
						if (test[i] == tahun_sekarang) {
							// b.push(parseInt(0));
						}else{
							b.push('(Anak Ke '+[i+1]+') '+parseInt(tahun_sekarang-anak[i]) + ' Th');
						}
					}

					var jumlah_anak = b.filter(obj=>!Object.values(obj).includes(0));
					...
					tableData += '<td>'+ (jumlah_anak.length || "") +'</td>';
					tableData += '<td style="text-align: left">'+ (b[0] || "") +'<br>'+ (b[1] || "") +'<br>'+ (b[2] || "") +'<br>'+ (b[3] || "") +'<br>'+ (b[4] || "") +'<br>'+ (b[5] || "") +'<br>'+ (b[6] || "") +'</td>';
					tableData += '</tr>';
				});
