		...
		<?php if (count($audit) > 0){ ?>
			<div class="col-xs-6" style="text-align: center;padding-right: 5px">
				<table style="width: 100%;border:1px solid black">
					...
					<tr>
						<td colspan="2" style="background-color: #c7c7c7;font-weight: bold;border:1px solid black;width: 2%;font-size: 15px">Claim Title</td>
					</tr>
					<tr>
						<td colspan="2" style="background-color: #abcbff;border:1px solid black;font-size: 18px" id="audit_title">{{$audit[0]->audit_title}}</td>
					</tr>
				</table>
			</div>
			<div class="col-xs-6" style="text-align: center;padding-right: 5px">
				<table style="width: 100%;border:1px solid black">
					<tr>
						...
						<td style="background-color: #c7c7c7;font-weight: bold;border:1px solid black;width: 2%;font-size: 15px" >Claim Date</td>
						<td style="background-color: #c7c7c7;font-weight: bold;border:1px solid black;width: 2%;font-size: 15px" >Claim Origin</td>
					</tr>
					<tr>
						<td  style="background-color: #ffadad;border:1px solid black;font-size: 18px" id="product">{{$audit[0]->product}}</td>
						<td  style="background-color: #ffadad;border:1px solid black;font-size: 18px" id="incident_date_title">{{$audit[0]->incident_date}}</td>
						<td  style="background-color: #ffadad;border:1px solid black;font-size: 18px" id="origin">{{$audit[0]->origin}}</td>
					</tr>
					<tr>
						<td colspan="2" style="background-color: #c7c7c7;font-weight: bold;border:1px solid black;width: 2%;font-size: 15px" >Dept</td>
						<td style="background-color: #c7c7c7;font-weight: bold;border:1px solid black;width: 2%;font-size: 15px">Area</td>
					</tr>
					<tr>
						<td colspan="2" style="background-color: #abcbff;border:1px solid black;font-size: 18px" id="department_name">{{$audit[0]->department_shortname}}</td>
						<td  style="background-color: #abcbff;border:1px solid black;font-size: 18px" id="area">{{$audit[0]->area}}</td>
					</tr>
				</table>
			</div>
				...
				<table class="table table-responsive" style="width: 100%;border:1px solid black" id="tableCheck">
					<thead>
						<tr>
			 				...
			 				<th style="background-color:rgb(126,86,134);color:#FFD700;border:1px solid black;width:3%;">Image Reference</th>
			 				<th style="background-color:rgb(126,86,134);color:#FFD700;border:1px solid black;width:2%;">Condition</th>
			 				<th style="background-color:rgb(126,86,134);color:#FFD700;border:1px solid black;width:3%;">Image Evidence</th>
						...
						<?php if (count($audit) > 0): ?>
							@foreach($audit as $audit)
								<tr>
									...
									<td style="background-color:white;border:1px solid black;width:1%;">{{$audit->audit_point}}</td>
									<td style="background-color:white;border:1px solid black;width:1%;"><img style="width:100%;cursor:pointer" src="{{url('data_file/qa/ng_jelas_point/')}}/{{$audit->audit_images}}" class="user-image" alt="User image" onclick="modalImage('{{url("data_file/qa/ng_jelas_point/")}}/{{$audit->audit_images}}')"></td>
									<?php if ($audit->result_check == 'OK'){ ?>
										<td style="background-color:white;border:1px solid black;width:1%;font-size: 20px">&#9711;</td>
									<?php }else if($audit->result_check == 'NS'){ ?>
										<td style="background-color:white;border:1px solid black;width:1%;font-size: 20px">&#8420;</td>
									<?php }else if($audit->result_check == 'NG'){ ?>
										<td style="background-color:white;border:1px solid black;width:1%;font-size: 20px">&#9747;</td>
									<?php } ?>
									<td style="background-color:white;border:1px solid black;width:1%;">
										<?php if (str_contains($audit->result_image,',')){ ?>
					                      <?php $imagesss = explode(',', $audit->result_image) ?>
					                      ...
					                      <?php } ?>
					                    <?php }else { ?>
					                      <img style="width:100%;cursor:pointer" src="{{url('data_file/qa/ng_jelas/')}}/{{$audit->result_image}}" class="user-image" alt="User image" onclick="modalImage('{{url("data_file/qa/ng_jelas/")}}/{{$audit->result_image}}')">
					                    <?php } ?>
									</td>
									<td style="background-color:white;border:1px solid black;width:1%;">{{$audit->note}}</td>
	...
	function confirmAll() {
		if (confirm('Apakah Anda yakin menyelesaikan proses?')) {
			...
			for(var i = 1; i <= parseInt('{{count($audit)}}');i++){
				...
				handled_by.push($('#operator_id').text());
			}
			...
			$.post('{{ url("input/qa/audit_ng_jelas/handling") }}', data, function(result, status, xhr){
				if(result.status){
					$('#loading').hide();
					openSuccessGritter('Success!',result.message);
					window.location.replace("{{url('index/qa/audit_ng_jelas_monitoring')}}");
				}else{
					audio_error.play();
