          ...
          <div class="col-xs-4" style="padding-right: 0; padding-left: 0; margin-bottom: 2%; border-radius: 5px" id="this_area">
            ...
            <table class="table" width="100%">
              <tr>
                ...
                <input type="hidden" id="id_sosialisasi" class="form-control" value="{{$id_sosil}}">
...
function checkCode(param) {
  ...
  $.get('{{ url("scan/sds/participant") }}', data, function(result, status, xhr){
    if (result.status) {
      $("#op_name").val(result.employee.name);
      $("#op_id").val(result.employee.employee_id);
      audio_ok.play();
      save();
    } else {
      audio_error.play();
...
function save() {
  ...
  $.post('{{ url("post/sds/sosialisasi") }}', data, function(result, status, xhr){
    if (result.status) {
      $("#loading").hide();
      audio_ok.play();
...
function getHistory() {
  ...
  $.get('{{ url("fetch/employee/sosialisasi") }}', data, function(result, status, xhr){
    $("#body_history").empty();
