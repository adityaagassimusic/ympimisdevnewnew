...
@section('content')
    <section class="content" style="padding-top: 0;">
        ...
        <div class="row">
            ...
            <div class="col-xs-6 col-sm-6 col-xs-offset-3 col-sm-offset-3" id="field_kanban">
                <div class="input-group-addon" id="icon-serial" style="font-weight: bold">
                    <span style="font-size: 3vw; background-color: #00c0ef; padding-top: 6px;">
                        &nbsp;
                ...
                <input type="text" style="text-align: center; font-size: 3vw; height: 100px;" class="form-control"
                    id="order_id" placeholder="Scan Order ID">
            </div>


            <div class="col-xs-12" id="picking">
                <input id="qr_item" type="text"
                    style="border:0; width: 100%; text-align: center; height: 20px; color: white; background-color: #3c3c3c; height: 50px;">
                <div class="row" style="margin-top: 1%">
                    <div class="col-xs-4" style="">
                            ...
                            <div class="box-body">
                                <table class="table table-bordered table-stripped" style="margin-bottom: 0px;">
                                    <thead>
                                        <tr>
                                            <th colspan="2"
                                                style="font-size: 2vw; background-color: orange; border-bottom: 1px solid black;">
                                                INJECTION PROCESS</th>
                                        </tr>
                                        <tr>
                                            <th colspan="2"
                                                style="font-size: 2vw; background-color: orange; border-bottom: 1px solid black;"
                                        ...
                                        <tr>
                                            <th style="font-size:1.5vw; background-color:#f5f5f5; vertical-align:middle;">
                                                <span id="material"></span>
                                            </th>
                                            <th style="font-size:1.5vw; background-color:#f5f5f5; vertical-align:middle;">
                                                <span id="data_op"></span>
                                            </th>
                                        </tr>
                                    </thead>
                                </table>
                        ...
                        <div class="box">
                            <div class="box-body">
                                <button onclick="approval()" class="btn btn-primary"
                                    style="font-weight: bold; font-size: 2vw; width: 100%;">CHECK DIMENSI MATERIAL</button>
                            </div>
                        </div>
                        <div class="box">
                            <div class="box-body">
                                <button onclick="printLabel()" class="btn btn-danger"
                                    style="font-weight: bold; font-size: 2vw; width: 100%;"><i
                                        class="fa fa-print"></i>&nbsp;&nbsp;&nbsp;PRINT LABEL</button>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <thead style="background-color: orange;">
                                        <tr>
                                            ...
                                            <th width="50%" style="font-size: 2vw;">
                                                <button id="start" onclick="startInj()" class="btn btn-success"
                                                    style="font-weight: bold; font-size: 2vw; width: 100%;">START</button>
                                                <button id="finish" onclick="finishInj()" class="btn btn-danger"
                                                    style="font-weight: bold; font-size: 2vw; width: 100%;">FINISH</button></span>
                                            </th>
                                        </tr>
                                    </thead>
                                </table>
        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header no-padding">
                    <h3 style="background-color: #00c0ef; text-align: center; font-weight: bold; padding-top: 3%; padding-bottom: 3%;"
                        class="modal-title">
                ...
                <div class="modal-body table-responsive">
                    <div class="form-group">
                        ...
                        <br><br>
                        <a href="{{ url('/index/reed') }}" class="btn btn-warning"
                            style="width: 100%; font-size: 1vw; font-weight: bold;"><i class="fa fa-arrow-left"></i> Ke
                            Halaman Reed</a>
                    </div>
                </div>
            </div>
...
@section('scripts')
    ...
    <script>
                ...
                if ($("#operator").val().length == 9) {
                    ...
                    $.get('{{ url('scan/reed/operator') }}', data, function(result, status, xhr) {
                        if (result.status) {
                            ...
                            $('#data_op').html(result.employee.employee_id + " <br> " + result.employee
                                .name)
            ...
            $.get('{{ url('fetch/reed/injection_picking_list') }}', data, function(result, status, xhr) {
                if (result.status) {
                    ...
                    $.each(result.data, function(key, value) {
                        ...
                        pickingData +=
                            '<td style="font-size: 1.8vw; height:2%; vertical-align:middle; height:40px;">' +
                            value.picking_list + '</td>';
                        pickingData +=
                            '<td style="font-size: 1.8vw; height:2%; vertical-align:middle; height:40px;">' +
                            value.picking_description + '</td>';
                        pickingData +=
                            '<td style="font-size: 1.8vw; height:2%; vertical-align:middle; height:40px;">' +
                            value.quantity + '</td>';
                        pickingData +=
                            '<td style="font-size: 1.8vw; height:2%; vertical-align:middle; height:40px;">' +
                            value.actual_quantity + '</td>';
                    ...
                    if (total_quantity == total_actual) {
                        if (result.order.start_injection != null) {
                            time = new Date(result.order.start_injection);
                            document.getElementById("hours").innerHTML = pad(parseInt(diff_seconds(new Date(),
                                time) / 3600));
                            document.getElementById("minutes").innerHTML = pad(parseInt((diff_seconds(new Date(),
                                time) % 3600) / 60));
                            ...
                            if (result.order.setup_molding == 1) {
        ...
        function startInj() {
            ...
            if (confirm("Apakah anda yakin memulai proses injeksi?")) {
                $.post('{{ url('fetch/reed/start_injection') }}', data, function(result, status, xhr) {
                    if (result.status) {

                        time = new Date(result.start);

                        document.getElementById("hours").innerHTML = pad(parseInt(diff_seconds(new Date(), time) /
                            3600));
                        document.getElementById("minutes").innerHTML = pad(parseInt((diff_seconds(new Date(),
                            time) % 3600) / 60));
                        document.getElementById("seconds").innerHTML = pad(diff_seconds(new Date(), time) % 60);
        ...
        function finishInj() {
            ...
            if (confirm("Apakah anda yakin mengakhiri proses injeksi?")) {
                $.post('{{ url('fetch/reed/finish_injection') }}', data, function(result, status, xhr) {
                    if (result.status) {
                        time = undefined;
                        $('#finish').prop('disabled', true);
        ...
        function approval() {
            ...
            var employee_id = $('#employee_id').val();
            var order_id = $('#order_id').val();
            window.open('{{ url('index/reed/approval/') }}' + '/' + location + '/' + order_id + '/' + employee_id,
                '_blank');
        }
