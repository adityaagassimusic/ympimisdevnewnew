		...
		<?php if ($audit){ ?>
			<div class="col-xs-6" style="text-align: center;padding-right: 5px">
				<table style="width: 100%;border:1px solid black">
					...
					<tr>
						<td  style="background-color: #ffadad;border:1px solid black;font-size: 18px" id="operator_id">{{$audit->auditor_effectivity_id}}</td>
						<td  style="background-color: #ffadad;border:1px solid black;font-size: 18px" id="operator_name">{{$audit->auditor_effectivity_name}}</td>
					</tr>
				</table>
			</div>
			<div class="col-xs-6" style="text-align: center;padding-right: 5px">
				...
				<table class="table table-responsive" style="width: 100%;border:1px solid black" id="tableCheck">
					<thead>
						<tr>
			 				...
			 				<th style="background-color:rgb(126,86,134);color:#FFD700;border:1px solid black;width:3%;">Penanganan</th>
					...
					<tbody>
						...
						<tr>
							...
							<td style="background-color:white;border:1px solid black;width:1%;"><?php echo $audit->handling_result ?></td>
							<td style="background-color:white;border:1px solid black;width:1%;">
							<div class="col-xs-6">
								<label class="containers">&#9711;
								  <input type="radio" name="audit_effectivity" id="audit_effectivity" value="OK">
								  <span class="checkmark"></span>
								</label>
							</div>
							<div class="col-xs-6">
								<label class="containers">&#9747;
								  <input type="radio" name="audit_effectivity" id="audit_effectivity" value="NG">
								  <span class="checkmark"></span>
								</label>
							</div>
							</td>
							<td style="background-color:white;border:1px solid black;"><textarea id="audit_effectivity_note"></textarea></td>
							<script type="text/javascript">
							CKEDITOR.replace('audit_effectivity_note' ,{
	...
	function confirmAll() {
		if (confirm('Apakah Anda yakin menyelesaikan verifikasi?')) {
			...
			for(var i = 1; i <= parseInt('{{$count_audit}}');i++){
				ids.push($('#id').val());
				var decision = '';
				$("input[name='audit_effectivity']:checked").each(function (i) {
					decision = $(this).val();
		        });
		        verification.push(decision);
				verification_note.push(CKEDITOR.instances['audit_effectivity_note'].getData());
			...
			$.post('{{ url("input/audit_ik_monitoring/cek_efektifitas") }}', data, function(result, status, xhr){
				if(result.status){
					$('#loading').hide();
