		...
		<div class="col-xs-12">
			<div class="box no-border" style="margin-bottom: 5px;">
				<div class="box-header" style="margin-top: 10px">
					<h3 class="box-title">Outstanding PR Kantin Belum PO</span></h3>
...
<form id="importForm" name="importForm" method="post" action="{{ url('create/purchase_order_canteen') }}" enctype="multipart/form-data">
	<input type="hidden" value="{{csrf_token()}}" name="_token" />
		...
		<div class="modal-dialog modal-lg" style="width: 1300px">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">Create Purchase Order Canteen</h4>
					<br>
									...
									<div class="col-md-4">
										<div class="col-md-6" style="padding:0">
											<div class="form-group">
												...
												<input type="hidden" class="form-control" id="remark" name="remark" value="Kantin">
											</div>
										</div>
										<div class="col-md-6" style="padding:0">
											<div class="form-group">
												<label>&nbsp;</label>
												<input type="text" class="form-control" id="no_po2" name="no_po2" placeholder="E.g. : 08001-IT">
											</div>
										</div>
										<div class="form-group">
						...
						<div class="tab-pane" id="tab_2">
							<div class="row">
								<div class="col-md-12">
									...
									<div class="col-xs-1" style="padding:5px;">
										<input type="text" class="form-control" id="qty1" name="qty1" placeholder="Qty" required="" onkeyup="getkonversi(this)">
									</div>
									...
									<div class="col-xs-1" style="padding:5px;">
										...
										<select class="form-control select2" data-placeholder="GL Number" name="gl_number1" id="gl_number1" style="width: 100% height: 35px;" required=''>
											<option value="51400100">51400100 - Meal Allowance</option>
...
<div class="modal fade in" id="modalEdit">
	<form id ="importFormEdit" name="importFormEdit" method="post" action="{{ url('update/purchase_order_canteen') }}">
		<input type="hidden" value="{{csrf_token()}}" name="_token" />
		<div class="modal-dialog modal-lg" style="width: 1300px">
			<div class="modal-content">
				<div class="modal-header">
...
<div class="modal fade in" id="modalEditPR">
    <form id ="importFormEditPR" name="importFormEditPR" method="post" action="{{ url('update/purchase_requisition_canteen/po') }}">
      <input type="hidden" value="{{csrf_token()}}" name="_token" />
      <div class="modal-dialog modal-lg" style="width: 1300px">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">Edit Purchase Requisition Kantin</h4>
            <br>
	...
	<div class="modal modal-danger fade" id="modalDeletePR" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	  <div class="modal-dialog">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	        Apakah anda yakin ingin cancel Form PR Ini ?
	      </div>
	      <div class="modal-footer">
...
<script>
	...
	function deleteConfirmationPROutstanding(id) {
	...
	function deletePR(id){
		...
		$.post('{{ url("canteen/delete/purchase_requisition") }}', data, function(result, status, xhr){
			if (result.status == true) {
	        	openSuccessGritter("Success","Data Berhasil Di Cancel");
	        	$("#loading").hide();
	...
	function fillTableOutstanding(){
		...
		var table = $('#outstandingTable').DataTable({
			...
			"ajax": {
				"type" : "get",
				"url" : "{{ url("fetch/po_canteen_outstanding") }}",
			},
			"columns": [
			{ "data": "no_pr" },
	...
	function fillTable(){
		...
		var table = $('#poTable').DataTable({
			...
			"ajax": {
				"type" : "get",
				"url" : "{{ url("fetch/purchase_order_canteen") }}",
				"data" : data
			},
			"columns": [
			{ "data": "no_po" },
	...
	function getPRList() {
		$.get('{{ url("fetch/purchase_order/kantinlist") }}', function(result, status, xhr) {
	...
	function pilihPR(elem)
	{
		...
		$.ajax({
			url: "{{ url('fetch/purchase_order/pilih_prkantin') }}?no_pr="+elem.value,
			method: 'GET',
			success: function(data) {
				var json = data,
				obj = JSON.parse(json);
	...
	function pilihItem(elem)
	{
		...
		$.ajax({
			url: "{{ url('purchase_order/get_item_kantin') }}?item_code="+elem.value+"&no_pr="+no_pr,
			method: 'GET',
			success: function(data) {
				var json = data,
				obj = JSON.parse(json);
    ...
    function editPO(id){
    	...
    	$.get('{{ url("edit/purchase_order_canteen") }}', data, function(result, status, xhr){
	...
	function delete_item(id) {
		...
		$.post('{{ url("delete/purchase_order_canteen_item") }}', data, function(result, status, xhr){
	...
	function cancel_po(id){
		...
		$.post('{{ url("cancel/purchase_order_canteen") }}', data, function(result, status, xhr){
			if (result.status == true) {
	...
	function sendEmail(id) {
      ...
      $.get('{{ url("purchase_order_canteen/sendemail") }}', data, function(result, status, xhr){
    ...
    function ResendEmail(id) {
      ...
      $.get('{{ url("purchase_order_canteen/resendemail") }}', data, function(result, status, xhr){
   ...
   function pilihItemPR(elem)
   {
    ...
    function detailPR(id){
        ...
        $.get('{{ url("canteen/detail/purchase_requisition/po") }}', data, function(result, status, xhr){  
