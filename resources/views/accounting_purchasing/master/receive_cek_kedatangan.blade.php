...
<section class="content-header">
	<h1>
		Receive Report List <span class="text-purple">{{ $title_jp }}</span>
	</h1>
	<ol class="breadcrumb">
	</ol>
</section>
					...
					<div class="col-xs-12">
						...
						<div class="col-md-2">
							<div class="form-group">
								...
								<select class="form-control select2" id="no_pr" name="no_pr" data-placeholder='Nomor PR' style="width: 100%">
					              <option value="">&nbsp;</option>
					              @foreach($pr_detail as $pr)
					              <option value="{{$pr->no_pr}}">{{$pr->no_pr}}</option>
					              @endforeach
					            </select>
							</div>
						</div>
						...
						<div class="col-md-2">
							<div class="form-group">
								...
								<input type="text" class="form-control pull-right" id="surat_jalan" name="surat_jalan" placeholder="Masukkan Surat Jalan">
							</div>
						</div>
...
<script>
	...
	$('#surat_jalan').keydown(function(event) {
		if (event.keyCode == 13 || event.keyCode == 9) {
			fetchTable();
		}
	});
	...
	function fetchTable(){
		...
		var table = $('#itemtable').DataTable({
			...
			rowCallback: function(row, data, index) {
		      if (data.pic_receive == "-") {
		        $("td:eq(11)", row).addClass("bg-red");
		      }else{
		      	$("td:eq(11)", row).addClass("bg-green");
		      }
		    }
		});
