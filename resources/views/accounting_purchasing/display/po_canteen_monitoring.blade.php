...
<script>
  ...
  function drawChart() {
    ...
    $.get('{{ url("fetch/canteen/purchase_order/monitoring") }}', data, function(result, status, xhr) {
      if(xhr.status == 200){
        if(result.status){
  ...
  function fetchTable(){
    ...
    $.get('{{ url("canteen/purchase_order/table") }}', data, function(result, status, xhr){
      if(xhr.status == 200){
        if(result.status){
          ...
          $.each(result.datas, function(key, value) {
            ...
            //CPAR
            var urldetail = '{{ url("purchase_order_canteen") }}';
            var urlreport = '{{ url("canteen/purchase_order/report/") }}';
            var urlverifikasi = '{{ url("canteen/purchase_order/verifikasi/") }}';
  ...
  function ShowModalPO(tanggal, status, tglfrom, tglto) {
    ...
    var table = $('#example2').DataTable({
      ...
      "ajax": {
           "type" : "get",
          "url" : "{{ url("canteen/purchase_order/detail") }}",
          "data" : {
            tanggal : tanggal,
            status : status,
            tglfrom : tglfrom,
