...
<script>
	...
	function fillAudit() {
		...
		$.get('{{ url("fetch/maintenance/compressor") }}', function(result, status, xhr){
			if(result.status){
				...
				for(var i = 0; i < result.audit.length;i++){
					...
					var url = '{{url("data_file/daily_audit/compressor/")}}/'+result.audit[i].evidence;
				...
				for(var i = 0; i < result.audit_kerjakan.length;i++){
					...
					var url = '{{url("data_file/daily_audit/compressor/")}}/'+result.audit_kerjakan[i].evidence;
	...
	function startDoing(id) {
			...
			$.get('{{ url("doing/maintenance/compressor") }}',data, function(result, status, xhr){
				if(result.status){
					$('#loading').hide();
	...
	function finishDoing(id) {
			...
			$.ajax({
				url:"{{ url('input/maintenance/compressor') }}",
				method:"POST",
				data:formData,
				dataType:'JSON',
				contentType: false,
