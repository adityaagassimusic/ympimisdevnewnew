            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" style="text-transform: uppercase; text-align: center;"><b>Operator NG Rate
                                Details</b></h4>
        ...
        function showNGDetail(bulan, param) {
            ...
            $.get('{{ url('fetch/middle/bff_op_ng_monthly_detail') }}', data, function(result, status, xhr) {
                if (result.status) {
                    ...
                    $('#judul1').append().empty();
                    $('#judul1').append('<b>' + result.ng[0].operator_id + ' - ' + result.ng[0].name + ' on ' +
                        bulanText(bulan) + '</b>');
        ...
        function showTimeDetail(bulan, param) {
            ...
            $.get('{{ url('fetch/middle/bff_op_work_monthly_detail') }}', data, function(result, status, xhr) {
                if (result.status) {
                    ...
                    $('#judul2').append().empty();
                    $('#judul2').append('<b>' + result.emp[0].employee_id + ' - ' + result.emp[0].name + ' on ' +
                        bulanText(bulan) + '</b>');
            ...
            $.get('{{ url('fetch/middle/bff_op_ng_monthly/resume') }}', data, function(result, status, xhr) {
                if (xhr.status == 200) {
                    if (result.status) {
                        ...
                        $('#last_update').html('<p><i class="fa fa-fw fa-clock-o"></i> Last Updated: ' +
                            getActualFullDate() + '</p>');
                        ...
                        for (var i = 0; i < result.op_ng.length; i++) {
                            ...
                            if (name_temp[0] == 'M.' || name_temp[0] == 'Muhammad' || name_temp[0] == 'Muhamad' ||
                                name_temp[0] == 'Mokhammad' || name_temp[0] == 'Mokhamad' || name_temp[0] ==
                                'Mukhammad' || name_temp[0] == 'Mochammad' || name_temp[0] == 'Akhmad' || name_temp[
                                    0] == 'Achmad' || name_temp[0] == 'Moh.' || name_temp[0] == 'Moch.' ||
                                name_temp[0] == 'Mochamad') {
                        ...
                        Highcharts.chart('container1', {
                            ...
                            title: {
                                text: '<span style="font-size: 18pt;">Highest NG Rate by OP on ' +
                                    bulanText(result.bulan) + '</span>',
                                useHTML: true
                            },
                            xAxis: {
                                categories: name,
                            ...
                            yAxis: {
                                ...
                                plotLines: [{
                                    color: '#FF0000',
                                    value: result.target_ng,
                                    dashStyle: 'shortdash',
                                    width: 2,
                                    zIndex: 5,
                                    label: {
                                        align: 'right',
                                        text: 'Target ' + result.target_ng + '%',
                                        x: -7,
                                        style: {
                                            fontSize: '12px',
                                            color: '#FF0000',
                ...
                if (xhr.status == 200) {
                    if (result.status) {
                        ...
                        for (var i = 0; i < result.act.length; i++) {
                            ...
                            name.push(xAxis);
                            series.push([xAxis, parseInt(result.act[i].std.toFixed(2))]);
                        ...
                        for (var i = 0; i < series.length; i++) {
                            ...
                            if (series[i][1] > target) {
                                // categories.push(series[i][0]);
                                // data.push({y: series[i][1], color: 'rgb(144,238,126)'});
                            } else {
                                categories.push(series[i][0]);
                                data.push({
                                    y: series[i][1],
                        ...
                        Highcharts.chart('container3', {
                            ...
                            title: {
                                text: '<span style="font-size: 18pt;">Highest Productivity Average on ' +
                                    bulanText(result.bulan) + '</span>',
                                useHTML: true
                            },
                            xAxis: {
