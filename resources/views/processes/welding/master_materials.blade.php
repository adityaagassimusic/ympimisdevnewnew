					...
					<div class="row">
						...
						<div class="col-md-6 col-md-offset-3">
							<div class="form-group pull-right">
								...
								<a href="{{ url('index/welding/master_material/'.$location) }}" class="btn btn-danger">Clear</a>
...
<script>
	...
	function fillList(){
		...
		$.get('{{ url("fetch/welding/master_material") }}',data, function(result, status, xhr){
			if(result.status){
				...
				$.each(result.material, function(key, value) {
					...
					tableData += '<td style="text-align:left;padding-left:7px;">'+ (value.work_station || 'Belum Diset') +'</td>';
	...
	function update() {
		if (confirm('Apakah Anda yakin?')) {
			...
			$.post('{{ url("update/welding/master_material") }}',data, function(result, status, xhr){
				if(result.status){
