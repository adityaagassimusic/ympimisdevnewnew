...
<script>
	...
	function getData() {
		$.get('{{ url("fetch/welding/jig_bom_proses") }}', function(result, status, xhr){
			if(result.status){
	...
	function addJigBom() {
		if ($('#jig_parent').val() == "" || $('#jig_child').val() == "" || $('#usage').val() == "") {
			...
			$.post('{{ url("input/welding/jig_bom_proses") }}', data,function(result, status, xhr){
				if(result.status){
	...
	function editJigBom(id) {
		...
		$.get('{{ url("edit/welding/jig_bom_proses") }}', data,function(result, status, xhr){
			if(result.status){
				$.each(result.jig_bom, function(key, value) {
	...
	function updateJigBom() {
		...
		$.post('{{ url("update/welding/jig_bom_proses") }}', data,function(result, status, xhr){
			if(result.status){
	...
	function deleteJigBom(id,jig_parent,jig_child) {
		var url = "{{ url('delete/welding/jig_bom_proses') }}";
