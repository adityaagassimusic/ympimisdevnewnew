...
<script>
	...
	function getData() {
		$('#loading').show();
		$.get('{{ url("fetch/welding/kensa_point_proses") }}', function(result, status, xhr){
			if(result.status){
	...
	function addJigPoint() {
		if ($('#jig_parent').val() == "" || $('#jig_child').val() == "" || $('#check_name').val() == "" || $('#lower_limit').val() == "" || $('#upper_limit').val() == "") {
			...
			$.post('{{ url("input/welding/kensa_point_proses") }}', data,function(result, status, xhr){
				if(result.status){
	...
	function editJigPoint(id) {
		...
		$.get('{{ url("edit/welding/kensa_point_proses") }}', data,function(result, status, xhr){
			if(result.status){
				// $.each(result.jig_point, function(key, value) {
	...
	function updateJigPoint() {
		...
		$.post('{{ url("update/welding/kensa_point_proses") }}', data,function(result, status, xhr){
			if(result.status){
	...
	function deleteJigPoint(id,jig_parent,jig_child) {
		var url = "{{ url('delete/welding/kensa_point_proses') }}";
