...
<section class="content">
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<div class="box box-solid">
				<div class="box-header">
					<center><h2 style="background-color: #1ac200;color: white;font-weight: bold;padding-top: 5px;padding-bottom: 5px">RESUME JIG SELESAI REPAIR</h2></center>
				</div>
					...
					<table id="jigTableResume" class="table table-bordered table-striped table-hover">
						<thead style="background-color: rgb(126,86,134); color: #FFD700;">
							<tr>
								...
								<th style="width: 1%">Before</th>
								<th style="width: 1%">After</th>
		...
		<div class="col-xs-12">
			<div class="box box-solid">
				<div class="box-header">
					<center><h2 style="background-color: #ffb300;color: white;font-weight: bold;padding-top: 5px;padding-bottom: 5px">RESUME JIG BELUM REPAIR</h2></center>
				</div>
				<div class="box-body" style="padding-top: 0;">
					<table id="jigTableNoRepair" class="table table-bordered table-striped table-hover">
						<thead style="background-color: rgb(126,86,134); color: #FFD700;">
							<tr>
								<th style="width: 1%">#</th>
								<th style="width: 1%">Jig ID</th>
...
<script>
	...
	function getData() {
		...
		$.get('{{ url("fetch/welding/repair_jig_report_proses") }}',data, function(result, status, xhr){
			if(result.status){
				...
				$.each(result.jig_repaired, function(key, value) {
					jigtable += '<tr>';
					...
					jigtable += '<td>'+result.operator2[(index-1)].split('_')[0]+'<br>'+result.operator2[(index-1)].split('_')[1]+'</td>';
					var url_before = "{{ url('data_file/jig//') }}"+'/'+value.image_before;
					jigtable += '<td style="padding:2px"><img width="100px" src="'+url_before+'"></td>';
					var url_after = "{{ url('data_file/jig//') }}"+'/'+value.image_after;
					jigtable += '<td style="padding:2px"><img width="100px" src="'+url_after+'"></td>';
				...
				$('#jigTableNoRepair').DataTable().clear();
				$('#jigTableNoRepair').DataTable().destroy();
				$('#bodyJigTableNoRepair').empty();
				var jigtable = '';
				var index = 1;

				var stts = 'No Repair';

				$.each(result.jig_no_repair, function(key, value) {
					jigtable += '<tr>';
					jigtable += '<td>'+index+'</td>';
					jigtable += '<td>'+value.jig_id+'</td>';
					jigtable += '<td>'+value.jig_name+'</td>';
					jigtable += '<td>'+value.started_at+'</td>';
					jigtable += '<td>'+value.finished_at+'</td>';
					jigtable += '<td>'+result.operator[(index-1)].split('_')[1]+'</td>';
					var url_before = "{{ url('data_file/jig//') }}"+'/'+value.image_before;
					jigtable += '<td style="padding:2px"><img width="100px" src="'+url_before+'"></td>';
					var url_after = "{{ url('data_file/jig//') }}"+'/'+value.image_after;
					jigtable += '<td style="padding:2px"><img width="100px" src="'+url_after+'"></td>';
					jigtable += '<td>'+value.status+'</td>';
					jigtable += '<td>'+value.action+'</td>';
					jigtable += '<td><button onclick="showModal(\''+stts+'\',\''+value.jig_id+'\',\''+value.started_at+'\',\''+value.finished_at+'\')" class="btn btn-primary btn-sm">Detail</button></td>';
					index++;
				});

				$('#bodyJigTableNoRepair').append(jigtable);

				var table = $('#jigTableNoRepair').DataTable({
					'dom': 'Bfrtip',
					'responsive':true,
					'lengthMenu': [
					[ 10, 25, 50, -1 ],
	...
	function showModal(stts,jig_id,started_at,finished_at) {
		$('#loading').show();
		var data = {
			stts:stts,
			jig_id:jig_id,
		...
		$.get('{{ url("fetch/welding/detail_repair_jig_report_proses") }}',data, function(result, status, xhr){
			if(result.status){
				var detail = "";
				var jig_judul = "";
				$('#bodyDetail').empty();
				...
				$.each(result.detail, function(key2, value2) {
					...
					detail += '<td style="border:1px solid black;padding:2px">'+result.operator[(index2-1)].split('_')[1]+'</td>';
					detail += '</tr>';
					detail += '</tbody>';
					index2++;
