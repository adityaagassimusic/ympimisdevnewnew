...
<script>
	...
	function getData() {
		$.get('{{ url("fetch/welding/jig_data_proses") }}', function(result, status, xhr){
			if(result.status){
	...
	function addJigData() {
		if ($('#jig_parent').val() == "" || $('#jig_id').val() == "" || $('#jig_index').val() == "" || $('#jig_name').val() == "" || $('#jig_alias').val() == "" || $('#category').val() == "" || $('#drawing').val() == "") {
			...
			$.ajax({
				url:"{{ url('input/welding/jig_data_proses') }}",
				method:"POST",
				data:formData,
				dataType:'JSON',
				contentType: false,
	...
	function editJigData(id) {
		...
		$('#drawing_now').html("");
		$.get('{{ url("edit/welding/jig_data_proses") }}', data,function(result, status, xhr){
			if(result.status){
				$.each(result.jigs, function(key, value) {
	...
	function updateJigData() {
		...
		$.ajax({
			url:"{{ url('update/welding/jig_data_proses') }}",
			method:"POST",
			data:formData,
			dataType:'JSON',
			contentType: false,
	...
	function deleteJigData(id,jig_id,jig_name,jig_parent) {
		var url = "{{ url('delete/welding/jig_data_proses') }}";
