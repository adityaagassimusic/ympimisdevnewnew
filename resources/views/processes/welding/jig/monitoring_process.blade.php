				...
				<table class="table table-bordered table-striped" id="tableOutstanding" style="width: 100%;color: white">
					<thead style="background-color: rgb(255,255,255); font-size: 12px;font-weight: bold">
						<tr style="background-color: #2d2d2e;border-bottom: 3px solid white;border-top: 1px solid #2d2d2e">
							<th rowspan="2" style="vertical-align: middle;padding: 4px">No.</th>
							<th rowspan="2" style="vertical-align: middle;padding: 4px">Jig ID</th>
							<th rowspan="2" style="vertical-align: middle;padding: 4px">Jig Name</th>
							<th colspan="3" style="border-right: 2px solid red;border-left: 2px solid red;padding: 4px">Kensa</th>
							<th colspan="3" style="border-right: 2px solid red;padding: 4px">Repair</th>
							<th rowspan="2" style="vertical-align: middle;padding: 4px">Jig Status</th>
							<th rowspan="2" style="vertical-align: middle;padding: 4px">Due Date</th>
							<th rowspan="2" style="vertical-align: middle;padding: 4px">Action</th>
						</tr>
						<tr style="background-color: #2d2d2e;border-bottom: 3px solid white;border-top: 1px solid #2d2d2e">
...
<script>
	...
	function fillList(){
		...
		$.get('{{ url("fetch/welding/monitoring_jig_proses") }}',data, function(result, status, xhr){
			if(result.status){
				var date = [];
				var series = [];
				var before_kensa = [];
				...
				for (var i = 0; i < result.monitoring.length; i++) {
					date.push(result.monitoring[i][0].week_date);
					before_kensa.push(parseInt(result.monitoring[i][0].before_kensa));
					after_kensa.push(parseInt(result.monitoring[i][0].after_kensa));
					before_repair.push(parseInt(result.monitoring[i][0].before_repair));
					waiting_part.push(parseInt(result.monitoring[i][0].waiting_part));
				}
				...
				Highcharts.chart('container', {
				    ...
				    title: {
				        text: 'KENSA JIG PROCESS MONITORING',
				        style: {
			                fontSize: '20px',
			                fontWeight: 'bold'
			              }
				...
				for(var i = 0; i < result.resume.length;i++){
					before_kensa = before_kensa + parseInt(result.resume[i][0].before_kensa);
					before_repair = before_repair + parseInt(result.resume[i][0].before_repair);
					waiting_part = waiting_part + parseInt(result.resume[i][0].waiting_part);
					if (parseInt(result.resume[i][0].before_kensa) > 0) {
						tgl_before_kensa.push(result.resume[i][0].week_date);
					}
				}
				...
				$.each(result.outstanding, function(key, value) {
					var kerjakan = '';
					var url = '{{url("index/welding/kensa_jig_proses")}}'+'/'+value.jig_id+'/monitoring';
					if (index % 2 == 0) {
						if (tgl_before_kensa.includes(value.schedule_date)) {
							kerjakan += '<td><a class="btn btn-success" href="'+url+'"><b>Kerjakan</b></a></td>';
					...
					outstanding += '<td style="text-align:right;vertical-align:middle;padding:3px;">'+index+'</td>';
					outstanding += '<td style="text-align:left;vertical-align:middle;padding:3px;">'+value.jig_id+'</td>';
					outstanding += '<td style="text-align:left;vertical-align:middle;padding:3px;">'+value.jig_name+'</td>';
					outstanding += '<td style="text-align:right;vertical-align:middle;padding:3px;">'+value.kensa_time+'</td>';
					outstanding += '<td style="text-align:left;vertical-align:middle;padding:3px;">'+result.operator_outstanding[(index-1)].split('_')[0]+'</td>';
					if (value.kensa_status != "") {
						outstanding += '<td style="vertical-align:middle;padding:3px;"><span class="label label-success">'+value.kensa_status+'</span></td>';
					}else{
						outstanding += '<td style="vertical-align:middle;padding:3px;">'+value.kensa_status+'</td>';
					}
					outstanding += '<td style="text-align:right;vertical-align:middle;padding:3px;">'+value.repair_time+'</td>';
					outstanding += '<td style="text-align:left;vertical-align:middle;padding:3px;">'+result.operator_outstanding[(index-1)].split('_')[1]+'</td>';
					outstanding += '<td style="text-align:left;vertical-align:middle;padding:3px;">'+value.repair_status+'</td>';
					if (value.schedule_status == 'Close') {
						outstanding += '<td style="vertical-align:middle;padding:3px;"><span class="label label-success" style="border:1px solid white">'+value.schedule_status+'</span></td>';
					}else{
						outstanding += '<td style="vertical-align:middle;padding:3px;"><span class="label label-danger" style="border:1px solid white">'+value.schedule_status+'</span></td>';
					}
					outstanding += '<td style="text-align:right;vertical-align:middle;padding:3px;">'+value.schedule_date+'</td>';
					outstanding += kerjakan;
					outstanding += '</tr>';
					index++;
				});
	...
	function ShowModal(date,kondisi) {
		...
		$.get('{{ url("fetch/welding/detail_monitoring_jig_proses") }}',data, function(result, status, xhr){
			if(result.status){
				...
				$.each(result.schedule, function(key, value) {
			        ...
			        if (kondisi === 'Sudah Kensa' || kondisi === 'Belum Repair' || kondisi === 'Menunggu Part') {
						...
						$.each(result.detail[value.jig_id], function(key2, value2) {
							...
							detail += '<td style="border:1px solid black;padding:2px">'+result.operator[value.jig_id]+'</td>';
							detail += '</tr>';
							detail += '</tbody>';
							index2++;
						});
	...
	function ShowModalPeriode(date,kondisi) {
		...
		$.get('{{ url("fetch/welding/detail_monitoring_jig_periode_proses") }}',data, function(result, status, xhr){
			if(result.status){
				$('#modalDetail').modal('show');
