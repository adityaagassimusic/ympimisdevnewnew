					...
					<table id="jigTableResume" class="table table-bordered table-striped table-hover">
						<thead style="background-color: rgb(126,86,134); color: #FFD700;">
							<tr>
								...
								<th style="width: 2%">Before</th>
								<th style="width: 2%">After</th>
...
<script>
	...
	function getData() {
		...
		$.get('{{ url("fetch/welding/kensa_jig_report_proses") }}',data, function(result, status, xhr){
			if(result.status){
	...
	function showModal(jig_id,started_at,finished_at) {
		...
		$.get('{{ url("fetch/welding/detail_kensa_jig_report_proses") }}',data, function(result, status, xhr){
			if(result.status){
				var detail = "";
				var jig_judul = "";
				$('#bodyDetail').empty();
