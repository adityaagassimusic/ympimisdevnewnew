...
<script>
	...
	function getData() {
		$('#loading').show();
		$.get('{{ url("fetch/welding/jig_schedule") }}', function(result, status, xhr){
			if(result.status){
				...
				$.each(result.jig_schedule, function(key, value) {
					...
					jigtable += '<td>';
					jigtable += '<button class="btn btn-warning btn-sm" onclick="editJigSchedule(\''+value.id_jig_schedule+'\')" style="margin-right: 5px"><i class="fa fa-edit"></i></button>';
					if (value.schedule_status == 'Open') {
						jigtable += '<button class="btn btn-danger btn-sm" onclick="deleteSchedule(\''+value.id_jig_schedule+'\')"><i class="fa fa-trash"></i></button>';
					}
					jigtable += '</td>';
					jigtable += '</tr>';
