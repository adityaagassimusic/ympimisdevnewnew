...
<script>
	...
	function getData() {
		$('#loading').show();
		$.get('{{ url("fetch/welding/jig_part_proses") }}', function(result, status, xhr){
			if(result.status){
				...
				$.each(result.jig_part, function(key, value) {
					...
					if (value.quantity_order != null) {
						...
						jigtable += '<td>'+result.wjo_date[(index-1)]+'</td>';
					}else{
						jigtable += '<td></td>';
						jigtable += '<td></td>';
						jigtable += '<td></td>';
	...
	function addJigPart() {
		if ($('#jig_id').val() == "" || $('#quantity').val() == "" || $('#min_stock').val() == "" || $('#min_order').val() == "" || $('#material').val() == "") {
			...
			$.post('{{ url("input/welding/jig_part_proses") }}', data,function(result, status, xhr){
				if(result.status){
	...
	function editJigPart(id) {
		...
		$.get('{{ url("edit/welding/jig_part_proses") }}', data,function(result, status, xhr){
			if(result.status){
				// $.each(result.jig_part, function(key, value) {
	...
	function updateJigBom() {
		...
		$.post('{{ url("update/welding/jig_part_proses") }}', data,function(result, status, xhr){
			if(result.status){
	...
	function deleteJigPart(id,jig_id) {
		var url = "{{ url('delete/welding/jig_part_proses') }}";
