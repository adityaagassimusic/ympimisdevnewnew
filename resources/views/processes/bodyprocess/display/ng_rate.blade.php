...
<section class="content" style="padding-top: 0;">
	...
	<div class="row">
		<div class="col-xs-12" style="padding-bottom: 5px;">
			<div class="row">
				<form method="GET" action="{{ url('index/body_parts_process/ng_rate') }}/{{$id}}">
...
<script>
	...
	function fetchChart(){
		...
		$.get('{{ url("fetch/body_parts_process/ng_rate") }}', data, function(result, status, xhr) {
			if(result.status){
					...
					plotOptions: {
						series:{
							...
							point: {
								events: {
									click: function () {
										showModalKey(this.category,result.date);
									}
								}
							},
							dataLabels: {
...
function showModal(ng_name,date) {
	...
	$.get('{{ url("fetch/body_parts_process/ng_rate/detail") }}', data, function(result, status, xhr) {
		if(result.status){
			...
			for(var i = 0; i < result.detail.length;i++){
				...
				if (result.detail[i].operator_id == 'Operator Not Found') {
					tableBody += '<td></td>';
				}else{
					var name = '';
					for(var j = 0; j < result.emp.length;j++){
						if (result.emp[j].employee_id == result.detail[i].operator_id) {
							name = result.emp[j].name;
						}
					}
					tableBody += '<td>'+result.detail[i].operator_id+' - '+name+'</td>';
				}
