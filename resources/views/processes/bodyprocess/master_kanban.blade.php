                        ...
                        <div class="row">
                                    ...
                                    <div class="form-group">
                                        <select class="form-control select2" name="material_category" id="material_category" data-placeholder="Pilih Category" style="width: 100%;">
                                            ...
                                            @foreach ($material_category as $material_category)
                                                <option value="{{ $material_category->material_category }}">
                                                    {{ $material_category->material_category }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        <select class="form-control select2" name="material_type" id="material_type" data-placeholder="Pilih Type" style="width: 100%;">
                                            ...
                                            @foreach ($material_type as $material_type)
                                                <option value="{{ $material_type->material_type }}">
                                                    {{ $material_type->material_type }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-6 col-md-offset-3">
                                <div class="form-group pull-right">
                                    ...
                                    <a href="{{ url('index/body_parts_process/master_kanban/' . $location) }}" class="btn btn-danger">Clear</a>
                            ...
                            <div class="row">
                                <table id="tableMaster" class="table table-bordered table-striped table-hover" style="margin-bottom: 0;">
                                    <thead style="background-color: rgb(126,86,134); color: #FFD700;">
                                        <tr>
                                            <th width="1%">#</th>
                                            {{-- <th style="text-align: center;width: 2%" style="d">Tag</th> --}}
                                        ...
                                        <div class="col-sm-7" align="left" id="divEditMaterial">
                                            <select class="form-control select2" data-placeholder="Select Material" name="edit_material" id="edit_material" style="width: 100%">
                                                ...
                                                @foreach ($material2 as $material2)
                                                    <option value="{{ $material2->material_number }}_{{ $material2->material_category }}_{{ $material2->material_type }}">
                                                        {{ $material2->material_number }} -
                                                        {{ $material2->material_description }}
                                                        ({{ $material2->material_category }} -
                                        ...
                                        <div class="col-sm-7" align="left" id="divAddMaterial">
                                            <select class="form-control select2" data-placeholder="Select Material" name="add_material" id="add_material" style="width: 100%" onchange="getNoKanban()">
                                                ...
                                                @foreach ($material3 as $material3)
                                                    <option value="{{ $material3->material_number }}_{{ $material3->material_category }}_{{ $material3->material_type }}">
                                                        {{ $material3->material_number }} -
                                                        {{ $material3->material_description }}
                                                        ({{ $material3->material_category }} -
                                                        {{ $material3->material_type }})
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        function fillList() {
            ...
            $.get('{{ url('fetch/body_parts_process/kanban') }}', data, function(result, status, xhr) {
                if (result.status) {
                    ...
                    $.each(result.kanban, function(key, value) {
                        ...
                        // tableData += '<td style="text-align:right;padding-right:7px;">' + value.tag +
                        //     '</td>';
                        tableData += '<td style="text-align:left;padding-left:7px;">' + (value.barcode || '') + '</td>';
                        tableData += '<td style="text-align:center;padding-right:7px;">' + (value.no_kanban || '') + '</td>';
                        ...
                        tableData += '<button class="btn btn-sm btn-warning" onclick="editKanban(\'' + value.id + '\',\'' + value.material_number + '\',\'' + value.tag + '\',\'' + value.barcode + '\',\'' + value.material_category + '\',\'' + value.material_type + '\',\'' + value.no_kanban + '\')"><i class="fa fa-edit"></i>EDIT</button>';
                        tableData += '<button class="btn btn-sm btn-danger" onclick="deleteKanban(\'' + value.id + '\')"><i class="fa fa-trash"></i>DELETE</button>';
                        tableData += '</div>'
                        tableData += '</td>';
                        tableData += '</tr>';
                        index++;
        ...
        function getNoKanban() {
            ...
            for (var i = 0; i < all_kanban.length; i++) {
                if (all_kanban[i].material_number == material.split('_')[0] && all_kanban[i].material_category == material
                    .split('_')[1] && all_kanban[i].material_type == material.split('_')[2]) {
            ...
            if (material.split('_')[1] == 'HSA') {
                $('#add_barcode').val('{{ strtoupper($location) }}' + '21' + material.split('_')[0] + (parseInt(last_no) +
                    1));
            }
        ...
        function editKanban(id, material_number, tag, barcode, material_category, material_type, no_kanban) {
            cancelAll();
            $('#id').val(id);
        ...
        function update() {
            if (confirm('Apakah Anda yakin?')) {
                ...
                $.post('{{ url('update/body_parts_process/kanban') }}', data, function(result, status, xhr) {
                    if (result.status) {
        ...
        function add() {
            if (confirm('Apakah Anda yakin?')) {
                ...
                $.post('{{ url('input/body_parts_process/kanban') }}', data, function(result, status, xhr) {
                    if (result.status) {
        ...
        function deleteKanban(id) {
            if (confirm('Apakah Anda yakin akan menghapus data?')) {
                ...
                $.get('{{ url('delete/body_parts_process/kanban') }}', data, function(result, status, xhr) {
                    if (result.status) {
