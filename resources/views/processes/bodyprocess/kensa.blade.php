...
<section class="content" style="padding-top: 0;">
	...
	<input type="hidden" id="processing_time">
	<div class="row" style="margin-left: 0px; margin-right: 0px;">
		<div class="col-xs-7" style="padding-right: 0; padding-left: 0">
			<div class="col-xs-12" style="padding-bottom: 5px;">
				<?php 
					if ($loc == 'fbody-kensa-fl-pipe' || $loc == 'ffoot-kensa-fl-pipe' || $loc == 'fbody-kensa-fl-onko' || $loc == 'ffoot-kensa-fl-onko') {
				?>
						<div class="row" style="margin-bottom: 5px;">
							<div class="col-xs-3" style="padding: 0px 0px 0px 0px;">
								<a class="btn btn-md {{ $loc == 'fbody-kensa-fl-pipe'? 'btn-primary' : 'btn-warning' }}" href="{{ route("bodyPartsProcessKensa", "fbody-kensa-fl-pipe") }}">FLUTE BODY PIPE</a>								
							</div>
							<div class="col-xs-3" style="padding: 0px 0px 0px 0px;">
								<a class="btn btn-md {{ $loc == 'ffoot-kensa-fl-pipe'? 'btn-primary' : 'btn-warning' }}" href="{{ route("bodyPartsProcessKensa", "ffoot-kensa-fl-pipe") }}">FLUTE FOOT PIPE</a>								
							</div>
							<div class="col-xs-3" style="padding: 0px 0px 0px 0px;">
								<a class="btn btn-md {{ $loc == 'fbody-kensa-fl-onko'? 'btn-primary' : 'btn-warning' }}" href="{{ route("bodyPartsProcessKensa", "fbody-kensa-fl-onko") }}">FLUTE BODY ONKO</a>								
							</div>
							<div class="col-xs-3" style="padding: 0px 0px 0px 0px;">
								<a class="btn btn-md {{ $loc == 'ffoot-kensa-fl-onko'? 'btn-primary' : 'btn-warning' }}" href="{{ route("bodyPartsProcessKensa", "ffoot-kensa-fl-onko") }}">FLUTE FOOT ONKO</a>								
							</div>
						</div>
				<?php
					} else if ($loc == 'asneck-kensa-sx' || $loc == 'tsneck-kensa-sx') {
				?>
					<div class="row" style="margin-bottom: 5px;">
						<div class="col-xs-6" style="padding: 0px 0px 0px 0px;">
							<a class="btn btn-md {{ $loc == 'asneck-kensa-sx'? 'btn-primary' : 'btn-warning' }}" href="{{ route("bodyPartsProcessKensa", "asneck-kensa-sx") }}" style="width: 100%;">AS NECK</a>								
						</div>
						<div class="col-xs-6" style="padding: 0px 0px 0px 0px;">
							<a class="btn btn-md {{ $loc == 'tsneck-kensa-sx'? 'btn-primary' : 'btn-warning' }}" href="{{ route("bodyPartsProcessKensa", "tsneck-kensa-sx") }}" style="width: 100%;">TS NECK</a>								
						</div>						
					</div>
				<?php
				}
				...
				<div class="row">
					<div class="col-xs-2" style="padding: 0px 5px 0px 0px;">
						{{-- js button history back, with fa fa left --}}
						<button onclick="history.back()" class="btn btn-danger btn-lg" style="width: 100%; font-weight: bold; padding-top: 10px; padding-bottom: 10px; margin-bottom: 5px;">
				...
				<table style="width: 100%; margin-top: 5px;" border="1">
					<tbody>
						<tr>
							...
							<td style="width: 1%; font-weight: bold; font-size: 16px; background-color: rgb(220,220,220);">Type</td>
							...
							<td id="opbpro" style="width: 5%; font-weight: bold; font-size: 16px; background-color: rgb(100,100,100); color: yellow; border: 1px solid black"></td>
				...
				<table class="table table-bordered" style="width: 100%; margin-bottom: 2px;" border="1">
					<thead>
						<tr>
							...
							<th style="width: 10%; background-color: rgb(220,220,220); padding:0;font-size: 20px;" >Count</th>
					...
					<tr <?php echo $color ?>>
                        ...
                        <td style="width:15%"><input style="font-weight: bold; font-size: 45px; background-color: rgb(100,100,100); color: yellow;width: 100%;text-align: center;" id="count{{$nomor}}" value="0" readonly=""></td>
						<td id="ng{{$nomor}}" class="ngPlus" style="font-size: 20px;" onclick="plus({{$nomor}})">{{ $ng_list->ng_name }} </td>
		...
		<div>
			<center style="margin: 10px 0px">								
					<button onclick="canc()" class="btn btn-danger col-xs-5" style=" margin:0px; font-size: 3vw; padding: 2px 50px; font-weight: bold; border-color: black; color: white;">CANCEL</button>
				{{-- <button onclick="rework()" id="rework" class="btn btn-warning col-xs-6" style=" margin:0px; font-size: 2vw; padding: 2px 50px; font-weight: bold; border-color: black; color: white;">REWORK</button> --}}
				<button onclick="conf()" id="conf1" class="btn btn-success col-xs-7" style=" margin-top:0px; font-size: 3vw; padding: 2px 50px; font-weight: bold; border-color: black; color: white;" >CONFIRM</button>				
			</center>
		</div>
	</div>
</div>
				...
				<div class="row">
					<div class="col-xs-12">
						<table style="width: 100%;" border="1">
							<tbody>
								<tr>
									<td style="width: 1%; font-weight: bold; font-size: 16px; background-color: #ffff66;">FL HEAD 4XX</td>
									<td style="width: 1%; font-weight: bold; font-size: 16px; background-color: #1565c0;color: white;">HEAD 2XX</td>
									<td style="width: 1%; font-weight: bold; font-size: 16px; background-color: lightgreen;">HEAD 2XXU</td>
									<td style="width: 1%; font-weight: bold; font-size: 16px; background-color: #f3a;">U CON</td>
								</tr>
								<tr>
...
<script>
	...
	$('#operator').keydown(function(event) {
		if (event.keyCode == 13 || event.keyCode == 9) {
			...
			// if($('#operator').val().length > 9 ){
				...
				$.get('{{ url("scan/body_parts_process/operator") }}', data, function(result, status, xhr){
					if(result.status){
	...
	function fetchResult(){
		...
		$.get('{{ url("fetch/body_parts_process/kensa_result") }}', data, function(result, status, xhr){
			if (result.status) {
	...
	function getHeader(tag) {
		...
		$.get('{{ url("scan/body_parts_process/kensa") }}', data, function(result, status, xhr){
			if(result.status){
				$("#model").text(result.material.model);
				$("#key").text(result.material.key);				
				if(result.opbpro){
					if(result.opbpro.last_check){
						if(result.opbpro.last_check.includes('PI')){
							...
							// }
							var name = result.opbpro.name;
							if (result.opbpro.name == null) {
								for(var i = 0; i < result.emp.length;i++){
									if (result.emp[i].employee_id == result.opbpro.last_check) {
										name = result.emp[i].name;
										break;
									}
								}
							}
							$("#opbpro").text(result.opbpro.last_check+" - "+(name || ''));	
							$('#processing_time').val(result.opbpro.created_at);
						}else{
							$("#opbpro").text("Operator Not Found");
							$('#processing_time').val("");
						}	
					}else{
						$("#opbpro").text("Operator Not Found");
	...
	// 		ng: ng,
	// 		operator_id:$('#opbpro').text()
	// 		// total_ng: total,
	// 	}		
	// 	disabledButtonRework();

	// 	$.post('{{ url("input/body_parts_process/rework") }}', data, function(result, status, xhr){
	// 		if(result.status){
	// 			var btn = document.getElementById('rework');
	// 			btn.disabled = false;
	...
	function conf(){
		...
		var data = {
			...
			quantity: $('#material_quantity').val(),
			employee_id: $('#op').text(),
			operator_id: $('#opbpro').text().split(' - ')[0],
			started_at: $('#started_at').val(),
			processing_time: $('#processing_time').val(),
		...
		$.post('{{ url("input/body_parts_process/kensa") }}', data, function(result, status, xhr){
			if(result.status){
				var btn = document.getElementById('conf1');
				btn.disabled = false;
