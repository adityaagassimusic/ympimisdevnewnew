		...
		$.get('{{ url("fetch/assembly/group_balance") }}', data, function(result, status, xhr) {
			if(result.status){
				...
				for (var j = 0; j < result.key.length; j++) {
					key.push(result.key[j].location.toUpperCase());
				}
				
				for (var i = 0; i < result.data.length; i++) {
					for (var j = 0; j < result.key.length; j++) {

						if(result.data[i].location == result.key[j].location){
				...
				for (var j = 0; j < result.key.length; j++) {
					...
					for(var i = 0; i < result.data_plan.length;i++){
						if(result.data_plan[i].location == result.key[j].location){
							
							key_value_plan.push(Math.ceil(result.data_plan[i].plan_time / parseInt(result.key[j].jml)));
							
							total_value_plan += Math.ceil(result.data_plan[i].plan_time / parseInt(result.key[j].jml));
							
							if(Math.ceil(result.data_plan[i].plan_time / result.key[j].jml) > max_plan){
								max_plan = Math.ceil(result.data_plan[i].plan_time / parseInt(result.key[i].jml));
							}
						}
					}
				}
				console.log(key_value_plan);
