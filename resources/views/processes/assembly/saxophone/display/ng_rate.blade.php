...
<script>
	...
	function fetchChart(){
		...
		$.get('{{ url("fetch/assembly/saxophone/ng_rate") }}', data, function(result, status, xhr) {
			if(result.status){
...
// 		$.get('{{ url("fetch/assembly/saxophone/ng_rate/line") }}', data, function(result, status, xhr) {
// 			if(result.status){
...
function ShowModal(cat,date,type) {
	...
	$.get('{{ url("fetch/assembly/saxophone/ng_rate_detail") }}', data, function(result, status, xhr) {
		if(result.status){
			...
			if (type === 'ng_name') {
				...
				$('#modalDetailTitle').html('Detail NG '+cat.split('-')[0]+' Kunci '+cat.split('-')[1]+'<br>Tanggal '+result.dateTitleFirst+' - '+result.dateTitleLast);
			}else if (type === 'key') {
