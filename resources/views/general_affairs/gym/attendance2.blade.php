...
.activedada, .dotdada:hover {
  background-color: #717171;
}

.mySlidespunggung {display: none}
.dotpunggung {
  cursor: pointer;
  height: 15px;
  width: 15px;
...
.activepunggung,.dotpunggung:hover {
  background-color: #717171;
}

.mySlidesbahu {display: none}
.dotbahu {
  cursor: pointer;
  height: 15px;
  width: 15px;
...
.activebahu,.dotbahu:hover {
  background-color: #717171;
}

.mySlidestangan {display: none}
.dottangan {
  cursor: pointer;
  height: 15px;
  width: 15px;
...
.activetangan,.dottangan:hover {
  background-color: #717171;
}

.mySlideskaki {display: none}
.dotkaki {
  cursor: pointer;
  height: 15px;
  width: 15px;
...
<script>
	...
	function getVideo(param) {
		...
		clearInterval(intervaldada);
		clearInterval(intervalpunggung);
		clearInterval(intervalbahu);
		clearInterval(intervaltangan);
		clearInterval(intervalkaki);
		...
		if (param == 'dada') {
			...
			for(var i = 1; i <= count;i++){
				...
				video += '<div class="mySlidesdada">';
				  video += '<div class="numbertext">'+i+' / '+count+'</div>';
				  video += '<img style="width:100%" src="'+url+'">';
				video += '</div>';
			}
			video += '<a class="prev" onclick="plusSlidesdada(-1)">❮</a>';
			video += '<a class="next" onclick="plusSlidesdada(1)">❯</a>';
			...
			for(var i = 1; i <= count;i++){
				video += '<span class="dotdada" onclick="currentSlidedada(\''+i+'\')"></span> ';
			}
			video += '</div>';

			$('#video').append(video);
			showSlidesdada(slideIndexdada);
			intervaldada =  setInterval( function() { plusSlidesdada(1); }, 10000 );
		}else if(param == 'punggung'){
			video += '<div class="slideshow-container">';

			for(var i = 1; i <= count;i++){
				var url = "{{asset('data_file/gym/')}}/"+param+'_'+i+'.gif';
				video += '<div class="mySlidespunggung">';
				  video += '<div class="numbertext">'+i+' / '+count+'</div>';
				  video += '<img style="width:100%" src="'+url+'">';
				video += '</div>';
			}
			video += '<a class="prev" onclick="plusSlidespunggung(-1)">❮</a>';
			video += '<a class="next" onclick="plusSlidespunggung(1)">❯</a>';
			...
			for(var i = 1; i <= count;i++){
				video += '<span class="dotpunggung" onclick="currentSlidepunggung(\''+i+'\')"></span> ';
			}
			video += '</div>';

			$('#video').append(video);
			showSlidespunggung(slideIndexpunggung);
			intervalpunggung =  setInterval( function() { plusSlidespunggung(1); }, 10000 );
		}else if(param == 'bahu'){
			video += '<div class="slideshow-container">';

			for(var i = 1; i <= count;i++){
				var url = "{{asset('data_file/gym/')}}/"+param+'_'+i+'.gif';
				video += '<div class="mySlidesbahu">';
				  video += '<div class="numbertext">'+i+' / '+count+'</div>';
				  video += '<img style="width:100%" src="'+url+'">';
				video += '</div>';
			}
			video += '<a class="prev" onclick="plusSlidesbahu(-1)">❮</a>';
			video += '<a class="next" onclick="plusSlidesbahu(1)">❯</a>';
			...
			for(var i = 1; i <= count;i++){
				video += '<span class="dotbahu" onclick="currentSlidebahu(\''+i+'\')"></span> ';
			}
			video += '</div>';

			$('#video').append(video);
			showSlidesbahu(slideIndexbahu);
			intervalbahu =  setInterval( function() { plusSlidesbahu(1); }, 10000 );
		}else if(param == 'tangan'){
			video += '<div class="slideshow-container">';

			for(var i = 1; i <= count;i++){
				var url = "{{asset('data_file/gym/')}}/"+param+'_'+i+'.gif';
				video += '<div class="mySlidestangan">';
				  video += '<div class="numbertext">'+i+' / '+count+'</div>';
				  video += '<img style="width:100%" src="'+url+'">';
				video += '</div>';
			}
			video += '<a class="prev" onclick="plusSlidestangan(-1)">❮</a>';
			video += '<a class="next" onclick="plusSlidestangan(1)">❯</a>';
			...
			for(var i = 1; i <= count;i++){
				video += '<span class="dottangan" onclick="currentSlidetangan(\''+i+'\')"></span> ';
			}
			video += '</div>';

			$('#video').append(video);
			showSlidestangan(slideIndextangan);
			intervaltangan =  setInterval( function() { plusSlidestangan(1); }, 10000 );
		}else if(param == 'kaki'){
			video += '<div class="slideshow-container">';

			for(var i = 1; i <= count;i++){
				var url = "{{asset('data_file/gym/')}}/"+param+'_'+i+'.gif';
				video += '<div class="mySlideskaki">';
				  video += '<div class="numbertext">'+i+' / '+count+'</div>';
				  video += '<img style="width:100%" src="'+url+'">';
				video += '</div>';
			}
			video += '<a class="prev" onclick="plusSlideskaki(-1)">❮</a>';
			video += '<a class="next" onclick="plusSlideskaki(1)">❯</a>';
			...
			for(var i = 1; i <= count;i++){
				video += '<span class="dotkaki" onclick="currentSlidekaki(\''+i+'\')"></span> ';
			}
			video += '</div>';

			$('#video').append(video);
			showSlideskaki(slideIndexkaki);
			intervalkaki =  setInterval( function() { plusSlideskaki(1); }, 10000 );
		}else if(param == 'warmup'){
			video += '<div class="slideshow-container">';

			for(var i = 1; i <= count;i++){
				var url = "{{asset('data_file/gym/')}}/"+param+'_'+i+'.gif';
				video += '<div class="mySlideswarmup">';
				  video += '<div class="numbertext">'+i+' / '+count+'</div>';
				  video += '<img style="width:100%" src="'+url+'">';
				video += '</div>';
			}
			video += '<a class="prev" onclick="plusSlideswarmup(-1)">❮</a>';
			video += '<a class="next" onclick="plusSlideswarmup(1)">❯</a>';
			...
			for(var i = 1; i <= count;i++){
				video += '<span class="dotwarmup" onclick="currentSlidewarmup(\''+i+'\')"></span> ';
			}
			video += '</div>';

			$('#video').append(video);
			showSlideswarmup(slideIndexwarmup);
			intervalwarmup =  setInterval( function() { plusSlideswarmup(1); }, 10000 );
		}
	...
	function changeTab() {
		...
		for(var i = 0; i < 6;i++){
			...
			document.getElementById("btn_"+paramss[indexParams]).click();
			$('#titles').html('TUTORIAL '+$('#btn_'+paramss[indexParams]).text());
			indexParams++;
		}
	}
	...
	let slideIndexwarmup = 1;
	let slideIndexdada = 1;
	let slideIndexpunggung = 1;
	let slideIndexbahu = 1;
	let slideIndextangan = 1;
	let slideIndexkaki = 1;

		function plusSlideswarmup(n) {
		  showSlideswarmup(slideIndexwarmup += n);
		}

		function currentSlidewarmup(n) {
		  showSlideswarmup(slideIndexwarmup = n);
		}

		function showSlideswarmup(n) {
		  let i;
		  let slides = document.getElementsByClassName("mySlideswarmup");
		  let dots = document.getElementsByClassName("dotwarmup");
		  if (n > slides.length) {slideIndexwarmup = 1}    
		  if (n < 1) {slideIndexwarmup = slides.length}
		  for (i = 0; i < slides.length; i++) {
		    slides[i].style.display = "none";  
		  }
		  for (i = 0; i < dots.length; i++) {
		    dots[i].className = dots[i].className.replace(" activewarmup", "");
		  }
		  slides[slideIndexwarmup-1].style.display = "block";  
		  dots[slideIndexwarmup-1].className += " activewarmup";
		}

		function plusSlidesdada(n) {
		  showSlidesdada(slideIndexdada += n);
		}

		function currentSlidedada(n) {
		  showSlidesdada(slideIndexdada = n);
		}

		function showSlidesdada(n) {
		  let i;
		  let slides = document.getElementsByClassName("mySlidesdada");
		  let dots = document.getElementsByClassName("dotdada");
		  if (n > slides.length) {slideIndexdada = 1}    
		  if (n < 1) {slideIndexdada = slides.length}
		  for (i = 0; i < slides.length; i++) {
		    slides[i].style.display = "none";  
		  }
		  for (i = 0; i < dots.length; i++) {
		    dots[i].className = dots[i].className.replace(" activedada", "");
		  }
		  slides[slideIndexdada-1].style.display = "block";  
		  dots[slideIndexdada-1].className += " activedada";
		}

		function plusSlidespunggung(n) {
		  showSlidespunggung(slideIndexpunggung += n);
		}

		function currentSlidepunggung(n) {
		  showSlidespunggung(slideIndexpunggung = n);
		}

		function showSlidespunggung(n) {
		  let i;
		  let slides = document.getElementsByClassName("mySlidespunggung");
		  let dots = document.getElementsByClassName("dotpunggung");
		  if (n > slides.length) {slideIndexpunggung = 1}    
		  if (n < 1) {slideIndexpunggung = slides.length}
		  for (i = 0; i < slides.length; i++) {
		    slides[i].style.display = "none";  
		  }
		  for (i = 0; i < dots.length; i++) {
		    dots[i].className = dots[i].className.replace(" activepunggung", "");
		  }
		  slides[slideIndexpunggung-1].style.display = "block";  
		  dots[slideIndexpunggung-1].className += " activepunggung";
		}

		function plusSlidesbahu(n) {
		  showSlidesbahu(slideIndexbahu += n);
		}

		function currentSlidebahu(n) {
		  showSlidesbahu(slideIndexbahu = n);
		}

		function showSlidesbahu(n) {
		  let i;
		  let slides = document.getElementsByClassName("mySlidesbahu");
		  let dots = document.getElementsByClassName("dotbahu");
		  if (n > slides.length) {slideIndexbahu = 1}    
		  if (n < 1) {slideIndexbahu = slides.length}
		  for (i = 0; i < slides.length; i++) {
		    slides[i].style.display = "none";  
		  }
		  for (i = 0; i < dots.length; i++) {
		    dots[i].className = dots[i].className.replace(" activebahu", "");
		  }
		  slides[slideIndexbahu-1].style.display = "block";  
		  dots[slideIndexbahu-1].className += " activebahu";
		}

		function plusSlidestangan(n) {
		  showSlidestangan(slideIndextangan += n);
		}

		function currentSlidetangan(n) {
		  showSlidestangan(slideIndextangan = n);
		}

		function showSlidestangan(n) {
		  let i;
		  let slides = document.getElementsByClassName("mySlidestangan");
		  let dots = document.getElementsByClassName("dottangan");
		  if (n > slides.length) {slideIndextangan = 1}    
		  if (n < 1) {slideIndextangan = slides.length}
		  for (i = 0; i < slides.length; i++) {
		    slides[i].style.display = "none";  
		  }
		  for (i = 0; i < dots.length; i++) {
		    dots[i].className = dots[i].className.replace(" activetangan", "");
		  }
		  slides[slideIndextangan-1].style.display = "block";  
		  dots[slideIndextangan-1].className += " activetangan";
		}

		function plusSlideskaki(n) {
		  showSlideskaki(slideIndexkaki += n);
		}

		function currentSlidekaki(n) {
		  showSlideskaki(slideIndexkaki = n);
		}

		function showSlideskaki(n) {
		  let i;
		  let slides = document.getElementsByClassName("mySlideskaki");
		  let dots = document.getElementsByClassName("dotkaki");
		  if (n > slides.length) {slideIndexkaki = 1}    
		  if (n < 1) {slideIndexkaki = slides.length}
		  for (i = 0; i < slides.length; i++) {
		    slides[i].style.display = "none";  
		  }
		  for (i = 0; i < dots.length; i++) {
		    dots[i].className = dots[i].className.replace(" activekaki", "");
		  }
		  slides[slideIndexkaki-1].style.display = "block";  
		  dots[slideIndexkaki-1].className += " activekaki";
		}
