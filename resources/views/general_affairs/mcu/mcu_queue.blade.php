...
<script>
	...
	function fetchQueue() {
		$.get('{{ url("fetch/ga_control/mcu/queue") }}', function(result, status, xhr) {
			if(xhr.status == 200){
				if(result.status){
					...
					$.each(result.data_registrasi, function(key,value){
						...
						for(var i = 0; i < result.employee.length;i++){
							if (result.employee[i].employee_id == value.employee_id) {
								department_shortname = result.employee[i].department_shortname;
								section = result.employee[i].section;
							}
						}
						for(var i = 0; i < result.shift_schedule.length;i++){
							if (result.shift_schedule[i].employee_id == value.employee_id) {
								shiftdaily_code = result.shift_schedule[i].shiftdaily_code;
								attend_code = result.shift_schedule[i].attend_code;
							}
						}
						if (shiftdaily_code != null) {
							if (shiftdaily_code.match(/Shift_1/gi) && getActualFullDate() > result.now+" 05:00:00" && getActualFullDate() < result.now+" 16:00:00") {
								if (attend_code != null && attend_code.match(/CUTI/gi) == null && attend_code.match(/SAKIT/gi) == null && attend_code.match(/CK/gi) == null) {
									...
									table_registrasi += "<td "+color+">"+department_shortname+"</td>";
									table_registrasi += "</tr>";
									total_registrasi++;
								}
							}else if (shiftdaily_code.match(/Shift_2/gi) && getActualFullDate() > result.now+" 15:00:00" && getActualFullDate() < result.now+" 23:00:00") {
								if (attend_code != null && attend_code.match(/CUTI/gi) == null && attend_code.match(/SAKIT/gi) == null && attend_code.match(/CK/gi) == null) {
									if (result.section.length > 0) {
										if (section == result.section) {
											var color = "style='background-color:#d4f05b'";
							...
							}else if (shiftdaily_code.match(/Shift_3/gi) && getActualFullDate() > result.now+" 05:00:00" && getActualFullDate() < result.now+" 16:00:00") {
								if (attend_code != null && attend_code.match(/CUTI/gi) == null && attend_code.match(/SAKIT/gi) == null && attend_code.match(/CK/gi) == null) {
									if (result.section.length > 0) {
										if (section == result.section) {
											var color = "style='background-color:#d4f05b'";
					...
					$.each(result.data_clinic, function(key,value){
						...
						table_clinic += "<td "+color+">"+department_shortname+"</td>";
						table_clinic += "</tr>";
						total_clinic++;
					});
					...
					$.each(result.data_thorax, function(key,value){
						...
						table_thorax += "<td "+color+">"+department_shortname+"</td>";
						table_thorax += "</tr>";
						total_thorax++;
					});
					...
					$.each(result.data_audiometri, function(key,value){
						...
						table_audiometri += "<td "+color+">"+department_shortname+"</td>";
						table_audiometri += "</tr>";
						total_audiometri++;
					});
