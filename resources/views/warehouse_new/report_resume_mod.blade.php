...
<section class="content-header">
	<h1>
		History Resume Request Mod<span class="text-purple">??</span>
						...
						<div class="col-md-6 col-md-offset-2">
							<div class="col-md-10">
								<div class="form-group pull-right">
									<a href="{{ url('history/resume/mod') }}" class="btn btn-danger">Clear</a>
...
<script>
		...
		$.get('{{ url("fetch/history/detail") }}', function(result, status, xhr){
			if (result.status) {
				...
				for (var i = 0; i < result.detail_request_history.length; i++) {
					detail_history.push({id: result.detail_request_history[i].id,kode_request:result.detail_request_history[i].kode_request, gmc: result.detail_request_history[i].gmc,description: result.detail_request_history[i].description,lot: result.detail_request_history[i].lot,sloc_name: result.detail_request_history[i].sloc_name,quantity_request1: result.detail_request_history[i].quantity_request, quantity_check: result.detail_request_history[i].quantity_check,status_aktual: result.detail_request_history[i].status_aktual,no_hako: result.detail_request_history[i].no_hako,uom: result.detail_request_history[i].uom});
				}
			}
		});
	}
	...
	function fillList(){
		...
		$.get('{{ url("fetch/resume/mod") }}',data, function(result, status, xhr){
			if(result.status){
				...
				$.each(result.datas, function(key, value) {
					...
					tableData += '<tr>';
					tableData += '<td>'+ value.date_request +'</td>';
					...
					tableData += '<td>'+ value.quantity_total +'</td>';
					tableData += '<td>'+ value.loc +'</td>';
	...
	function detail_historys(kode_request){
		...
		$.each(detail_history, function(key, value){
			if (value.kode_request == kode_request) {
				...
				tableData += '<td>'+ value.quantity_request1 +'</td>';
				tableData += '<td>Selesai</td>';
				tableData += '</tr>';
			// no++;
		}
	});
