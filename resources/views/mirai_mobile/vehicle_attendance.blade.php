...
<section class="content-header" >
	<h1>
		VEHICLE STIKER ATTENDANCE<span class="text-purple"> </span>
	</h1>
</section>
@stop
@section('content')
				...
				<div class="box box-solid">
					<div class="box-body">
						...
						<div style="width: 100%;height: 100%;vertical-align: middle;padding-top: 20px">
							...
							<div class="col-xs-2">
								<div class="row">
									<center style="background-color: lightsalmon;border-bottom: 3px solid black;border-top:0px;border-left:0px;border-right:0px;"><span style="font-size: 15px;text-align: center;font-weight: bold;">STICKER</span></center>
									<center style="border: 1px solid black">
										<span style="font-size: 50px;font-weight: bold;" id="qty"></span>
									</center>
								</div>
							</div>
							<!-- <div class="col-xs-4">
...
<script>
	...
	$('#tag').keyup(function(event) {
		if (event.keyCode == 13 || event.keyCode == 9) {
			...
			if($("#tag").val().length >= 7){
				...
				$.get('{{ url("fetch/vehicle/attendance") }}', data, function(result, status, xhr){
					if(result.status){
						openSuccessGritter('Success','Scan Berhasil');
						$('#loading').hide();
						$('#name').html(result.emp[0].name);
						$('#qty').html(result.emp.length);
						audio_ok.play();
						$('#tag').removeAttr('disabled');
						$('#tag').val("");
						$('#tag').focus();
	...
	function fetchQueue() {
		...
		$.get('{{ url("fetch/vehicle/queue") }}', data, function(result, status, xhr){
			if(result.status){
				fetchHeadFoot();
