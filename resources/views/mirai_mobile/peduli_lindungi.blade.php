...
<script>
	...
	function initiateTable() {
		...
		tableData += '<th width="5%">Employee ID</th>';
		tableData += '<th width="5%">Name</th>';
		tableData += '<th width="5%">Department</th>';
		tableData += '<th width="2%">Result</th>';
		tableData += '</tr>';
		tableData += '</thead>';
	...
	function fillList(){
			...
			$.get('{{ url("fetch/peduli_lindungi/report") }}', function(result, status, xhr){
			if(result.status){
				...
				$.each(result.peduli_lindungi, function(key, value) {
					...
					if (value.result_survey == "Sudah Instal") {
						var keterangan = 'Already Installed';
						var color = '#4fff9e';
					}else {
						var keterangan = 'Not Installed';
						var color = '#ffadad';
					}
