...
<section class="content-header" >
	<h1>
		FAMILY DAY ATTENDANCE<span class="text-purple"> </span>
	</h1>
				...
				<div class="box box-solid">
					<div class="box-body">
						...
						<div style="width: 100%;height: 100%;vertical-align: middle;padding-top: 20px">
							...
							<div class="col-xs-4">
								<div class="row">
									<center style="background-color: #ffd333;border-bottom: 3px solid black;border-top:0px;border-left:0px;border-right:0px;"><span style="font-size: 15px;text-align: center;font-weight: bold;">JUMLAH TIKET</span></center>
									<center style="border: 1px solid black">
										<span style="font-size: 50px;font-weight: bold;" id="jumlah_tiket"></span>
									</center>
								</div>
							</div>
							<div class="col-xs-12">
								...
								<table id="tableAttendance" class="table table-bordered table-striped table-hover">
									<thead style="background-color: rgb(126,86,134);" id="headTableAttendance">
										<tr>
											...
											<th style="color:white;width:1%">Status</th>
											<th style="color:white;width:7%">Destinasi</th>
											<th style="color:white;width:1%">Jumlah</th>
...
<script>
	...
	$('#tag').keyup(function(event) {
		if (event.keyCode == 13 || event.keyCode == 9) {
			...
			if($("#tag").val().length >= 7){
				...
				$.get('{{ url("fetch/family_day/attendance") }}', data, function(result, status, xhr){
					if(result.status){
						openSuccessGritter('Success','Scan Berhasil');
						$('#loading').hide();
						$('#name').html(result.emp.data.name);
						$('#jumlah_tiket').html(result.emp.data.jumlah_tiket);

						var desti = "";
						if (result.emp.data.destinasi == "Opsi 1") {
							desti += 'Jatim Park 1 + Museum Angkut';
						}else if(result.emp.data.destinasi == "Opsi 2"){
							desti += 'Jatim Park 3 : Dino Park + Fun Tech + Museum Musik Dunia + Circus Magic';
						}else if(result.emp.data.destinasi == "Opsi 3"){
							desti += 'Jatim Park 3 : Dino Park + Predator Fun Park';
						}else if(result.emp.data.destinasi == "Opsi 4"){
							desti += 'Taman Safari Indonesia II Prigen';
						}else{
							desti += '';
						}

						$('#destinasi').html(desti);
						audio_ok.play();
						$('#tag').removeAttr('disabled');
						$('#tag').val("");
						$('#tag').focus();
	...
	function fetchHeadFoot() {
		...
		headTable += '<th style="color:white;width:1%">Status</th>';
	...
	function fetchQueue() {
		$.get('{{ url("fetch/family_day/queue") }}',  function(result, status, xhr){
			if(result.status){
				fetchHeadFoot();
				...
				for(var i = 0; i < result.emp.length; i++){
					...
					datas += '<td>'+result.emp[i].status+'</td>';

					datas += '<td style="text-align:left">';
					if (result.emp[i].destinasi == "Opsi 1") {
						datas += 'Jatim Park 1 + Museum Angkut';
					}else if(result.emp[i].destinasi == "Opsi 2"){
						datas += 'Jatim Park 3 : Dino Park + Fun Tech + Museum Musik Dunia + Circus Magic';
					}else if(result.emp[i].destinasi == "Opsi 3"){
						datas += 'Jatim Park 3 : Dino Park + Predator Fun Park';
					}else if(result.emp[i].destinasi == "Opsi 4"){
						datas += 'Taman Safari Indonesia II Prigen';
					}else{
						datas += '';
					}
					datas += '</td>';
					datas += '<td>'+result.emp[i].jumlah_tiket+'</td>';
