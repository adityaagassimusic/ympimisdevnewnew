				...
				<div class="box-body">
					...
					<div class="row">
						<div class="col-xs-12" style="overflow-x: scroll;">
							<table id="tableReportBox" class="table table-bordered table-striped table-hover">
								<thead style="background-color: rgba(126,86,134,.7);">
									<tr>
										...
										<th width="1%">Box</th>
										<th width="1%">Head</th>
										<th width="2%">Middle / Body</th>
										<th width="1%">Foot / Stopper</th>
										<th width="2%">Block</th>
									</tr>
								</thead>
								<tbody id="bodyReportBox">
								</tbody>
								<tfoot>
									<th></th>
									<th></th>
...
<script>
	...
	function fetchReportCdm() {
		...
		$.get('{{ url("fetch/recorder/report/ng_box") }}', function(result, status, xhr){
			if(result.status){
				$('#tableReportBox').DataTable().clear();
				$('#tableReportBox').DataTable().destroy();
				$('#bodyReportBox').html("");
				var tableData = "";
				var count = 1;
				$.each(result.ng_box, function(key, value) {
					tableData += '<tr>';
					tableData += '<td>'+ count +'</td>';
					tableData += '<td>'+ value.date +'</td>';
					tableData += '<td>'+ value.tray +'</td>';
					tableData += '<td>'+ value.ng_head +'</td>';
					tableData += '<td>'+ value.ng_middle +'</td>';
					tableData += '<td>'+ value.ng_foot +'</td>';
					tableData += '<td>'+ value.ng_block +'</td>';
					tableData += '</tr>';
					count++;
				});
				$('#bodyReportBox').append(tableData);

				$('#tableReportBox tfoot th').each( function () {
					var title = $(this).text();
					$(this).html( '<input style="text-align: center;" type="text" placeholder="Search '+title+'" />' );
				});
				var table = $('#tableReportBox').DataTable({
					'dom': 'Bfrtip',
					'responsive':true,
					'lengthMenu': [
					[ 10, 25, 50, -1 ],
				...
				$('#tableReportBox tfoot tr').appendTo('#tableReportBox thead');
				$("#loading").hide();
			}
			else{
				audio_error.play();
