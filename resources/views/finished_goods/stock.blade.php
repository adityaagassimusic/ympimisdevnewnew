			...
			if(xhr.status == 200){
				if(result.status){
					...
					$.each(result.stocks, function(key, value) {
						...
						tableStockData += '<td style="width: 4%;">'+ value.material_description +'</td>';
						...
						tableStockData += '<td style="width: 1%; text-align: right;">'+ value.quantity +'</td>';
					...
					array.reduce(function(res, value) {
						if (!res[value.destination]) {
							res[value.destination] = { destination: value.destination, production: 0, intransit: 0, warehouse: 0, production_m3: 0, intransit_m3: 0, warehouse_m3: 0, total_stock: 0, total_volume: 0 };
