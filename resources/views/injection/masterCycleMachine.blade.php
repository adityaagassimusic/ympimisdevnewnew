...
function fillexample1(){
  ...
  var table = $('#example1').DataTable({
    ...
    "ajax": {
      "type" : "get",
      "url" : "{{ url("fetch/fillMasterCycleMachine") }}",
    },
    "columns": [    
    { "data": "part"},
    { "data": "model"},
...
function percenMesin() {
  ...
  $.get('{{ url("fetch/chartMasterCycleMachine") }}',  function(result, status, xhr) {
      console.log(status);
      console.log(result);
      console.log(xhr);
      ...
      if(xhr.status == 200){
        if(result.status){
          ...
          for (var i = 0; i < result.part.length; i++) {
              part.push(result.part[i].part+' - '+result.part[i].color);
              mesin.push(parseInt(result.part[i].total));
              cycle.push(parseInt(result.part[i].cycle));
              shoot.push(parseInt(result.part[i].shoot));
              hako.push(parseInt( result.part[i].qty_hako));
          }
