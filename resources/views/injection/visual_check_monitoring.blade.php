					...
					<table class="table table-hover table-bordered table-responsive no-padding" id="tableTrendDetail">
						<thead style="background-color: rgba(126,86,134,.7)">
							<tr>
								...
								<th style="width: 1%;">Cav</th>
								<th style="width: 1%;">Mesin</th>
								<th style="width: 1%;">Jam Pengecekan</th>
...
<script>
	...
	function fetchChart(){
		...
		$.get('{{ url("fetch/injection/visual/monitoring") }}', data, function(result, status, xhr) {
			if(result.status){
				...
				var not_yet = [];
				var done = [];
				for(var k = 0; k < result.visual_check.length; k++){
					categories.push(result.visual_check[k].date);
					if (result.visual_check[k].hour_check != null) {
						var hour = result.visual_check[k].hour_check.split(',');
						done.push(hour.length);
						not_yet.push(8-hour.length);
					}else{
						done.push(0);
						not_yet.push(8);
					}
				...
				// 	for(var k = 0; k < result.visual_check.length; k++){
				// 		// if (result.visual_check[k].date == categories[i]) {
				// 			if (result.visual_check[k].hour_check != null) {
				// 				var hour = result.visual_check[k].hour_check.split(',');

				// 				for(var l = 0; l < hour.length;l++){
				// 					if (hour[l] == '06:00:00 - 08:00:00') {
				// 						hour0608.push({y:1,className:'highcharts-color-done'});
				// 					}else{
				// 						hour0608.push({y:1,className:'highcharts-color-notyet'});
				// 					} 
				// 					if (hour[l] == '08:00:00 - 10:00:00') {
				// 						hour0810.push({y:1,className:'highcharts-color-done'});
				// 					}else{
				// 						hour0810.push({y:1,className:'highcharts-color-notyet'});
				// 					} 
				// 					if (hour[l] == '10:00:00 - 12:00:00') {
				// 						hour1012.push({y:1,className:'highcharts-color-done'});
				// 					}else{
				// 						hour1012.push({y:1,className:'highcharts-color-notyet'});
				// 					} 
				// 					if (hour[l] == '12:00:00 - 14:00:00') {
				// 						hour1214.push({y:1,className:'highcharts-color-done'});
				// 					}else{
				// 						hour1214.push({y:1,className:'highcharts-color-notyet'});
				// 					} 
				// 					if (hour[l] == '14:00:00 - 16:00:00') {
				// 						hour1416.push({y:1,className:'highcharts-color-done'});
				// 					}else{
				// 						hour1416.push({y:1,className:'highcharts-color-notyet'});
				// 					} 
				// 					if (hour[l] == '16:00:00 - 18:00:00') {
				// 						hour1618.push({y:1,className:'highcharts-color-done'});
				// 					}else{
				// 						hour1618.push({y:1,className:'highcharts-color-notyet'});
				// 					} 
				// 					if (hour[l] == '18:00:00 - 20:00:00') {
				// 						hour1820.push({y:1,className:'highcharts-color-done'});
				// 					}else{
				// 						hour1820.push({y:1,className:'highcharts-color-notyet'});
				// 					}
				// 					if (hour[l] == '20:00:00 - 22:00:00') {
				// 						hour2022.push({y:1,className:'highcharts-color-done'});
				// 					}else{
				// 						hour2022.push({y:1,className:'highcharts-color-notyet'});
				// 					} 
				// 					if (hour[l] == '22:00:00 - 00:00:00') {
				// 						hour2200.push({y:1,className:'highcharts-color-done'});
				// 					}else{
				// 						hour2200.push({y:1,className:'highcharts-color-notyet'});
				// 					} 
				// 					if (hour[l] == '00:00:00 - 02:00:00') {
				// 						hour0002.push({y:1,className:'highcharts-color-done'});
				// 					}else{
				// 						hour0002.push({y:1,className:'highcharts-color-notyet'});
				// 					} 
				// 					if (hour[l] == '02:00:00 - 04:00:00') {
				// 						hour0204.push({y:1,className:'highcharts-color-done'});
				// 					}else{
				// 						hour0204.push({y:1,className:'highcharts-color-notyet'});
				// 					} 
				// 					if (hour[l] == '04:00:00 - 06:00:00') {
				// 						hour0406.push({y:1,className:'highcharts-color-done'});
				// 					}else{
				// 						hour0406.push({y:1,className:'highcharts-color-notyet'});
				// 					}
				// 				}
				// 			}
				// 			else{
				// 				hour0608.push({y:0,className:'highcharts-color-notyet'});
				// 				hour0810.push({y:0,className:'highcharts-color-notyet'});
				// 				hour1012.push({y:0,className:'highcharts-color-notyet'});
				// 				hour1214.push({y:0,className:'highcharts-color-notyet'});
				// 				hour1416.push({y:0,className:'highcharts-color-notyet'});
				// 				hour1618.push({y:0,className:'highcharts-color-notyet'});
				// 				hour1820.push({y:0,className:'highcharts-color-notyet'});
				// 				hour2022.push({y:0,className:'highcharts-color-notyet'});
				// 				hour2200.push({y:0,className:'highcharts-color-notyet'});
				// 				hour0002.push({y:0,className:'highcharts-color-notyet'});
				// 				hour0204.push({y:0,className:'highcharts-color-notyet'});
				// 				hour0406.push({y:0,className:'highcharts-color-notyet'});
				// 			}
				// 		// }
				// 	}
				// // }

				// console.log(hour0608);
				// console.log(hour0810);
				// console.log(hour1012);
				// console.log(hour1214);
				// console.log(hour1416);
				// console.log(hour1618);
				// console.log(hour1820);
				// console.log(hour2022);
				// console.log(hour2200);
				// console.log(hour0002);
				// console.log(hour0204);
				// console.log(hour0406);
				...
				Highcharts.chart('container', {
					...
					title: {
						text: "INJECTION VISUAL CHECK MONITORING",
						style: {
							fontSize: '30px',
							fontWeight: 'bold'
						}
					...
					tooltip: {
						headerFormat: '<span>Sampling Check</span><br/>',
...
function showHighlight(name,date) {
		...
		if (name === 'Sudah Dilakukan') {
			...
			$.get('{{ url("fetch/injection/visual/monitoring/detail") }}', data, function(result, status, xhr) {
				if(result.status){
					...
					for (var i = 0; i < result.details.length; i++) {
						...
						// for(j = 0; j < ng_name.length; j++){
							detailBody += '<tr>';
							detailBody += '<td>'+date+'</td>';
							detailBody += '<td>'+result.details[i].material_number+'<br>'+result.details[i].material_description.split(' ').slice(0,2).join(' ')+'</td>';
							detailBody += '<td>'+result.details[i].part_name+' '+result.details[i].part_type+'</td>';
							detailBody += '<td>'+result.details[i].color+'</td>';
							detailBody += '<td>'+result.details[i].cavity+'</td>';
							detailBody += '<td>'+result.details[i].machine+'</td>';
							detailBody += '<td>'+result.details[i].hour_check+'</td>';
							detailBody += '<td>'+result.details[i].ok+' Point(s)</td>';
							detailBody += '<td>'+result.details[i].ns+' Point(s)</td>';
							detailBody += '<td>'+result.details[i].ng+' Point(s)</td>';
							detailBody += '<td>'+result.details[i].pic_check+'<br>'+result.details[i].name.split(' ').slice(0,2).join(' ')+'</td>';
							detailBody += '<td>'+result.details[i].created+'</td>';
							detailBody += '</tr>';
						// }
					}
					...
					$('#modalDetailTitle').html('DETAIL VISUAL CHECK ON '+result.dateTitle.toUpperCase());
					$('#loading').hide();
			...
			for(var i = 0; i < schedules_belum.length;i++){
				if (date === schedules_belum[i].date) {
					belum += '<tr>';
					belum += '<td style="font-size:16px">'+schedules_belum[i].date+'</td>';
					belum += '<td style="font-size:16px">'+schedules_belum[i].hour+'</td>';
					belum += '</tr>';
				}
			}
			$('#tableBelumBody').append(belum);
			$('#modalDetailTitle').html('DETAIL VISUAL NOT CHECK ON '+date.toUpperCase());
			$('#loading').hide();
			$('#modalDetail').modal('show');
