            ...
            <div class="box-body">
              <div class="form-group row" align="right">
                ...
                <div class="col-sm-6" align="left">
                  <input type="text" class="form-control" style="width: 100%" name="reason" id="reason" placeholder="Reason">
                </div>
                <div class="col-sm-4">
                  <button class="btn btn-success pull-left" onclick="confirmRepair()">
                    <b>CONFIRM</b>
                  </button>
                  <a class="btn btn-danger pull-left" href="{{url('index/lifetime/'.$category.'/'.$location)}}" style="margin-left: 10px;">
                    <b>CANCEL</b>
                  </a>
                </div>
              </div>
...
<script type="text/javascript">
  ...
  function confirmRepair() {
    ...
    if ($('#reason').val() == '') {
      $('#loading').hide();
      openSuccessGritter('Error!','Masukkan Reason');
      return false;
    }
    ...
    $.post('{{ url("input/repair/lifetime/".$category."/".$location) }}',data, function(result, status, xhr){
      if(result.status){
        $('#loading').hide();
        openSuccessGritter('Success!',result.message);
        alert('Sukses');
        window.location.replace("{{url('index/lifetime/')}}/{{$category}}/{{$location}}");
      }else{
        $('#loading').hide();
