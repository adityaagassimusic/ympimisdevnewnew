...
<section class="content" style="padding-top: 0;">
	...
	<div class="row">
		<div class="col-xs-12">
			<div class="row">
				<div class="col-xs-10" style="color: white; font-size: 20px; font-weight: bold">
					TOTAL MANPOWER YMPI : <span id="tot_mp">0 Person</span><br>
					{{-- TOTAL MANPOWER OUTSOURCING : <span id="tot_mp_os">0 Person</span> --}}
				</div>
				<div class="col-xs-2">
				...
				<div class="row">
					<div class="col-xs-3">
						<div id="chartStatus" style="width: 100%; height: 300px;"></div>
					</div>
					<div class="col-xs-3">
						<div id="chartDirect" style="width: 100%; height: 300px;"></div>
					</div>
					<div class="col-xs-3">
						<div id="chartUnion" style="width: 100%; height: 300px;"></div>
					</div>
					<div class="col-xs-3">
			...
			<div class="col-xs-12" style="padding-top: 10px;">
				...
				{{-- <div id="chartAttendanceRate" style="width: 100%; padding-top: 10px;"></div> --}}
			</div>
		</div>
	</section>
	...
	<script>
		...
		function fetchModalCategory(cat, status){
			var tableData = "";
			...
			$.each(manpowers, function(key, value){
				if(value.category == cat && value.employ_code == status){
					tableData += "<tr>";
		...
		function fetchModal(filter, cat){
			...
			$.each(departments, function(key, value){
				if (cat == value.department_shortname) {
					stat = value.department_name;
				}
			})
			var data = {
				filter:filter,
		...
		function fetchChart(){
			...
			$.get('{{ url("fetch/report/manpower") }}', data, function(result, status, xhr) {
				if(result.status){
					...
					$.each(xDepartment, function(key, value){
						$.each(result.departments, function(key2, value2){
							// str = value;

							if (value == value2.department_name) {
								xDepartment[key] = value2.department_shortname;
							}
							// str = str.replace("Woodwind Instrument", "WI");
							// str = str.replace("Educational Instrument (EI)", "Edin");
						});
					});					
				...
				$.each(result.manpowers, function(key, value) {
					...
					if(!countDirect[catStatus]) {
						countDirect[catStatus] = 0;
					}
					if(value.category == 'DIRECT'){
						countDirect[catStatus]++;
					}
					...
					if (value.employ_code == 'OUTSOURCING') {
						// tot_person_os++;
					} else {
						tot_person++;
					}
				...
				// $("#tot_mp_os").text(tot_person_os+" Person");

				for(var prop in countStatus){
					seriesStatus.push(countStatus[prop]);
				}
				for(var prop in countDirect){
					seriesDirect.push(countDirect[prop]);
				}
				...
				var xPosition = [];
				var seriesPosition = [];
				var array = result.by_positions;
				var result = [];
				array.reduce(function(res, value) {
					if (!res[value.pos_name_en]) {
						res[value.pos_name_en] = { pos_name_en: value.pos_name_en, count: 0 };
						result.push(res[value.pos_name_en])
					}
					res[value.pos_name_en].count += 1;
					return res;
				}, {});
				...
				$.each(result, function(key, value){
					...
					seriesPosition.push(parseInt(value.count));
				});

				Highcharts.chart('chartDirect', {
					title: {
						text: 'Direct MP (直接人工)',
						style: {
							fontSize: '16px',
					...
					plotOptions: {
						series:{
							...
							point: {
								events: {
									click: function () {
										fetchModalCategory('DIRECT', this.category);
									}
								}
							}
						}
				...
				Highcharts.chart('chartGender', {
					...
					plotOptions: {
						pie: {
							...
							dataLabels: {
								enabled: true,
								format: '<b style="font-size: 16px;">{point.name}: {point.y} </b>',
								distance: -50,
								style:{
									fontSize:'16px',
									textOutline:0
