		...
		$.get("{{ url('fetch/kaizen/') }}", data, function(result, status, xhr){
			...
			$.each(result.kaizen, function(key, value) {
				// if(jQuery.inArray(value.employee_id.toUpperCase(), result.employee) !== -1) {
					...
					//Foreman Status
					if (value.status == '-1') {
						if (pos == 'Foreman' || pos == 'Manager' || pos == 'Chief' || pos == 'Deputy General Manager' || pos == 'General Manager' || '{{ Auth::id() }}' == 53 || '{{ Auth::id() }}' == 80 ||  '{{ Auth::id() }}' == 2580 ||  '{{ Auth::id() }}' == 81 || '{{ Auth::id() }}' == 2177 || '{{ Auth::id() }}' == 83) {
						...
						if (value.foreman_point_1 != null || value.foreman_point_2 != null || value.foreman_point_3 != null) {
					...
					// MANAGER STATUS
					if (value.foreman_point_1 != null && value.foreman_point_2 != null && value.foreman_point_3 != null) {
						if (value.manager_point_1 != 0 && value.manager_point_2 != 0 && value.manager_point_3 != 0) {
							body += '<td><span class="label bg-green"><i class="fa fa-check"></i> Verified</span></td>';
						} else {
							if (value.status == 2) {
								body += '<td><span class="label bg-red"><i class="fa fa-close"></i> NOT Kaizen</span></td>';
								...
								if (pos == 'Manager' || pos == 'Deputy General Manager' || pos == 'General Manager') {
...
$(window).on('pageshow', function(){
	<?php if (isset($filter)) { $fil = $filter; session(['kz_filter' => $fil, 'kz_stat' => '']); } else $fil = "";?>
	...
	var user2 = <?php echo json_encode($user); ?>;
		...
		fill_table('{{ $position->position }}', '{{ $fil }}', stat, '', user2);
	});
