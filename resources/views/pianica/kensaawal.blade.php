...
<section class="content-header">
  <h1>
    Detail {{ $page }}
    <span class="text-purple"> 最初検査詳細</span>
  </h1>
    ...
    <div class="col-xs-6">
      <div class="info-box">        
        <div class="info-box-content" style="margin:0px">
         <button class="btn btn-warning btn-lg pull-right" onclick="openmodal()">Change Operator Kensa Awal</button>         
         <span class="info-box-text" style="font-size: 25px">OPERATOR Kensa Awal</span>
         <span class="info-box-number" id="p_pureto_nama" style="font-size: 25px">[ ]</span><b id="p_pureto" hidden></b> <b id="p_pureto_nik" hidden></b> 
       </div>
     </div>
     <div class="info-box">        
      <div class="info-box-content" style="margin:0px">
        <b>Tag RFID</b> <b id="textmodel" style="color:red"> [ Model ] - </b><b class="destroy" id="modelb"></b><br>
...
<div class="row">
  <div class="col-xs-4">
    <div class="info-box"> 
      <div class="info-box-content" style="margin:0px">
        <b>OP Fixing Plate - </b>  <b id="opkaret" style="color:red"> [ - ]  </b>
      </div>
    </div>
  </div>
  ...
  <div class="col-xs-6">
    <div class="info-box"> 
      <div class="info-box-content" style="margin:0px">
        <b>OP Tuning - </b>  <b id="optuning" style="color:red"> [ - ]  </b>
        <div class="table-responsive">
          <table width="100%"  class="table  table-striped" border="0" style="margin :0px" id="tuningtabel">
          </table>  
        </div>
      </div>
    </div>
  ...
  <div class="col-xs-2">
    <div class="info-box">
      <button class="btn btn-lg btn-warning" style="font-size:30px; width:100%; height: 100px" onclick="simpan()"><i class="fa fa-save"></i>&nbsp;Save</button>
    </div>
  </div>
      ...
      <div class="box box-body"> 
       ...
       <div class="nav-tabs-custom">
        <ul class="nav nav-tabs" style="font-size: 25px">
          <li class="active" style="width: 14%"><a href="#tab_1" data-toggle="tab">Biri</a></li>
          <li style="width: 14%"><a href="#tab_2" data-toggle="tab">Oktaf</a></li>
          <li style="width: 14%"><a href="#tab_3" data-toggle="tab">T. Rendah</a></li>
          <li style="width: 14%"><a href="#tab_4" data-toggle="tab">T. Tinggi</a></li>
...
<script >
  ...
  function simpan() {
    ...
    var optuning = $('#optuningInput').val();
    var opfixing = $('#opkaretInput').val();
    ...
    if(tag == '' || $('#opkaret').text() == "[ - ]" || $('#optuning').text() == "[ - ]"){
      ...
      $.post('{{ url("index/SaveKensaAwal") }}', data, function(result, status, xhr){
        if(xhr.status == 200){
          ...
          if(result.status){
           ...
           $('#opkaret').text("[ - ]");
           $('#opkaret').css({'color':'red'});

           $('#optuning').text("[ - ]");
           $('#optuning').css({'color':'red'});
    ...
    $.get('{{ url("fetch/opTunning") }}', data, function(result, status, xhr){
      if(xhr.status == 200){
        if(result.status){
          ...
          var planData = '';
          planData += '<tr>';
          $.each(result.opTunning, function(key, value) {
            planData += '<td > <button class="btn btn-md btn-danger" style="font-size:30px; width:90%" id="'+value.nik+'"'+'name="'+value.nama+'" onclick="optun(this.id,this.name,this)">'+ value.id_number +'</td>';
          }); 
          planData += '</tr>';  
          $('#tuningtabel').append(planData); 
        }
        else{
  ...
  function optun(id,nik,kode) {            
    $('#optuning').text(" "+nik+""); 
    $('#optuningInput').val(" "+id+""); 
    $('#optuning').css({'color':'black'})           
  ...
  $.get('{{ url("index/model") }}', data, function(result, status, xhr){
    if(xhr.status == 200){
      if(result.status){
        ...
        $('#opkaret').text(" "+result.op_fixing[0].nama+""); 
        $('#opkaretInput').val(" "+result.op_fixing[0].operator+""); 
        $('#opkaret').css({'color':'black'});
    ...
    if(result.status){
      ...
      if (result.model.length > 0) {
        $.each(result.model, function(index, value) {
          if (value.ng_name == 'Biri') {
            $('#biri').text(value.total);
          } else if (value.ng_name == 'Oktaf') {
            $('#oktaf').text(value.total);
          } else if (value.ng_name == 'T. Tinggi') {
            $('#tinggi').text(value.total);
          } else if (value.ng_name == 'T. Rendah') {
            $('#rendah').text(value.total);
          }
        })
      }
      // openSuccessGritter('Success!', result.message);
