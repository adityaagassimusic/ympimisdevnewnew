...
<script>
	...
	jQuery(document).ready(function() {
		...
		fetchBoardResult();
		setInterval(fetchBoardResult,10000);
	...
	function fetchTable(){
		...
		$.get('{{ url("fetch/pn/board") }}', data, function(result, status, xhr){
			if(xhr.status == 200){
				if(result.status){
					...
					$.each(result.board, function(index, value){
							...
							if (value.sedang != '<span style="font-size:50px;"></span><br>') {
								...
								var tanggal_fix = value.sedang_time.replace(/-/g,'/');
								started_at = new Date(tanggal_fix);
								timeada = countUpFromTime(started_at);
							...
							if (value.employee_id == null) {
								...
								assemblyTableBody += '<td height="5%" style="font-size:40px">'+value.location+'</td>';
								assemblyTableBody += '<td style="font-size:40px"></td>';
								assemblyTableBody += '<td '+color3+'>'+sedang2+'</td>';
								assemblyTableBody += '<td></td>';
								operators.push('');
							}else{
								assemblyTableBody += '<tr '+color+'>';
								assemblyTableBody += '<td height="5%" style="font-size:40px">'+value.location+'</td>';
								assemblyTableBody += '<td style="font-size:40px">'+value.name.split(' ').slice(0,2).join(' ')+'</td>';
								operators.push(value.employee_id);
								// if (percent >= 100) {
								...
								// }else{
									assemblyTableBody += '<td '+color3+'>'+sedang2+' - '+timeada;
									assemblyTableBody += '</td>';
									if (perolehan_all != null && perolehan_all.length > 0) {
										assemblyTableBody += '<td style="font-size:45px;text-align:center;" id="perolehan_'+i+'">'+perolehan_all[i];
										assemblyTableBody += '</td>';
										perolehans = perolehans + parseInt(perolehan_all[i]);
									}else{
										assemblyTableBody += '<td style="font-size:45px;text-align:center;" id="perolehan_'+i+'">0';
										assemblyTableBody += '</td>';
									}
								// }
							}
	...
	function fetchBoardResult() {
		var data = {
			operators:operators
		}
		$.get('{{ url("fetch/pn/board/result") }}', data, function(result, status, xhr){
			if(xhr.status == 200){
				if(result.status){
					perolehan_all = result.results;
				}else{
					openErrorGritter('Error!',result.message);
				}
			}else{
