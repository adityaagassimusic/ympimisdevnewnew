...
<script>
    ...
    function fetchTable() {
        ...
        $.get('{{ url("fetch/material/plan_usage/monthly") }}', data,  function(result, status, xhr){
            if(result.status){
                ...
                for (var i = 0; i < result.calendar.length; i++) {
                    headUsage += '<th style="vertical-align: middle; text-align: center;">'+result.calendar[i].text_month.substr(0, 3)+'-'+result.calendar[i].text_year+'</th>';
                }
                for (var i = 0; i < result.interval.length; i++) {
                    headUsage += '<th style="vertical-align: middle; text-align: center;">'+result.interval[i].text_month.substr(0, 3)+'-'+result.interval[i].text_year+'</th>';
                }
                headUsage += '</tr>';
                ...
                for (var i = 0; i < result.material.length; i++) {
                    ...
                    bodyUsage += '<td style="vertical-align: middle; text-align: left;">'+result.material[i].material_description+'</td>';
                    ...
                    for (var j = 0; j < result.calendar.length; j++) {
                        ...
                        for (var k = 0; k < result.usage.length; k++) {
                            if((result.calendar[j].month == result.usage[k].month) && (result.material[i].material_number == result.usage[k].raw_material)){
                    ...
                    for (var j = 0; j < result.interval.length; j++) {
                        ...
                        for (var k = 0; k < result.avg_usage.length; k++) {
                            if(result.material[i].material_number == result.avg_usage[k].raw_material){
                                bodyUsage += '<td style="vertical-align: middle; text-align: center;">'+result.avg_usage[k].usage.toFixed(4)+'</td>';
                                inserted = true;
                            }
                        }
