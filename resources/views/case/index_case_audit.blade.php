			...
			<table class="table table-bordered" id="store_table">
				<thead>
					...
					<tr>
						...
						<th style="background-color: rgb(204,255,255); text-align: center; color: yellow; background-color: rgb(50, 50, 50); font-size:18px;">QTY Audit</th>
					...
					<div class="modal-body table-responsive no-padding">
						<div class="form-group">
							...
							<select class="form-control select2" name="lokasi" id='lokasi' data-placeholder="Pilih Lokasi" style="width: 100%;">
								...
								<option value="FLFG">Flute</option>
								<option value="ASFG">Saxophone</option>
								<option value="CLFG">Clarinet</option>
							</select>
						</div>
...
<script>
	...
	function selectData(){
		...
		else if (lokasi == "ASFG" || lokasi == "TSFG") {
			lok = "Saxophone";
		}
		$('#location_name').text('');
		$('#location_name').text('Lokasi : ' + lok);
	...
	function fillStore(loc){
		...
		$.get('{{ url("fetch/case/audit") }}', data, function(result, status, xhr){
			if (result.status) {
				...
				$("#audit_body").html("");
				$("#audit_title").text("");
				$("#audit_title").text("Audit Case Assy");
				...
				for (var i = 0; i < result.lists.length; i++) {
					body += '<tr>';
					body += '<td style="width:15%;padding: 0px; background-color: rgb(250,250,210); text-align: left; color: #000000; font-size: 20px;padding-left:5px" id="number_'+result.lists[i].id+'">'+result.lists[i].material_number+'</td>';
					body += '<td style="width:50%;padding: 0px; background-color: rgb(250,250,210); text-align: left; color: #000000; font-size: 20px;padding-left:5px" id="description_'+result.lists[i].id+'">'+result.lists[i].material_description+'</td>';
					body += '<td style="width:10%;padding: 0px; background-color: rgb(250,250,210); text-align: center; color: #000000; font-size: 20px;padding-left:5px" id="qty_'+result.lists[i].id+'">'+result.lists[i].qty+'</td>';
