		...
		<div class="row">
			<div class="col-xs-12">
				...
				<div class="col-xs-12" style="padding: 0;overflow-x: scroll;">
					...
					<div class="col-xs-4" style="margin-top: 10px;padding-right: 0px;padding-left: 5px">
						<button class="btn btn-success" onclick="confirmAll()" style="width: 100%;font-size: 25px;font-weight: bold; display: none" id="btn_conf">
							SAVE
						</button>
					</div>
				</div>
	...
	function save_form() {
		...
		$.get('{{ url("fetch/fixed_asset/location/list") }}', data, function(result, status, xhr){
			if(result.status){
				...
				if ('{{ Request::segment(4) }}' == 'check2') {
					if (result.total_blm_cek > 0) {
						$("#btn_conf").hide();
					} else {
						$("#btn_conf").show();
						openSuccessGritter('Informasi','Mohon Save Pengecekan');
						return false;
					}
				} 
				...
				if (result.asset.length == 0 && '{{ Request::segment(4) }}' != 'check2') {
