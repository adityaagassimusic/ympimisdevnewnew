...
<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li>
			...
			<a data-toggle="modal" data-target="#createModal" class="btn btn-success btn-md" style="color:white"><i class="fa fa-plus"></i>Request Disposal</a>
		</li>
	</ol>
</section>
</section>
						...
						<form method="post" enctype="multipart/form-data" autocomplete="off" id="form_master">
							<div class="row">
								...
								<div class="col-xs-12" style="padding-bottom: 1%;">
									...
									<div class="col-xs-6">
										<select class="form-control select2" data-placeholder="select mode" onchange="pilihMode(this)" style="width: 100%" name="mode" id="mode">
											<option value=""></option>
							...
							<div class="row">
								<div class="col-xs-4 pull-right">
									<center><button class="btn btn-success" type="submit" id="create_btn"><i class="fa fa-check"></i> Request Disposal </button></center>
								</div>
							</div>
						</form>
					</div>
...
<script>
	...
	function drawData() {
		$.get('{{ url("fetch/fixed_asset/disposal") }}', function(result, status, xhr){
			$("#masterBody").empty();
			var body = "";
			$.each(result.datas, function(index, value){
				...
				if(value.status == "created"){
					...
					body += '<td><label class="label label-warning">Deputy General Manager</label></td>';
				}
				...
				if (value.status != 'hold' && value.status != 'reject' && value.status != 'new_pic') {
					...
					} else if (value.created_by.toUpperCase() == '{{ strtoupper(Auth::user()->username) }}' && value.status == 'created') {
						body += "<button class='btn btn-success btn-xs' onclick='openSendMail(\""+value.form_number+"\",\""+value.status+"\")'><i class='fa fa-send'></i> Send Mail</button>&nbsp;";
