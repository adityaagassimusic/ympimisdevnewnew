...
<section class="content">
						...
						<div class="row">
							<table id="tableSga" class="table table-bordered table-striped table-hover" style="margin-bottom: 0;">
								<thead style="" id="headTableSga">
									<tr>
										<th width="4%" style="background-color: rgb(126,86,134); color: #fff;">Category</th>
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header" style="margin-bottom: 20px">
					<center><h3 style="background-color: #f39c12; font-weight: bold; padding: 3px; margin-top: 0; color: white;">Update SGA Point Check</h3>
					</center>
				</div>
					...
					<div class="col-xs-12">
						<div class="form-group row" align="right">
							...
							<div class="col-sm-4">
								...
								<input type="text" class="form-control" id="edit_criteria_category" name="edit_criteria_category" placeholder="Criteria Category">
							</div>
						</div>
					</div>
					<div class="col-xs-12">
	...
	<div class="modal fade" id="modalAddPoint">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header" style="margin-bottom: 20px">
					<center><h3 style="background-color: #f39c12; font-weight: bold; padding: 3px; margin-top: 0; color: white;">Add SGA Point Check</h3>
					</center>
				</div>
				<div class="modal-body table-responsive no-padding" style="min-height: 90px;padding-top: 5px">
					<div class="col-xs-12">
						<div class="form-group row" align="right">
							<label for="" class="col-sm-2 control-label">Category<span class="text-red"> *</span></label>
							<div class="col-sm-4">
								<input type="text" class="form-control" id="add_criteria_category" name="add_criteria_category" placeholder="Criteria Category">
							</div>
						</div>
					</div>
					<div class="col-xs-12">
				...
				<div class="modal-footer" style="margin-top: 10px;">
					<div class="col-xs-12">
						<div class="row">
							<button type="button" class="btn btn-danger pull-left" data-dismiss="modal"><i class="fa fa-close"></i> Cancel</button>
							<button onclick="addPoint()" class="btn btn-success pull-right"><i class="fa fa-edit"></i> Update</button>
						</div>
					</div>
				</div>
			</div>
...
<script>
	...
	function fillList(){
		$('#loading').show();
		$.get('{{ url("fetch/sga/point_check") }}', function(result, status, xhr){
			if(result.status){
				...
				$.each(result.point, function(key, value) {
					tableDataBody += '<tr>';
					tableDataBody += '<td style="padding:10px;text-align:left">'+ value.criteria_category +'</td>';
					...
					tableDataBody += '<td style="padding:10px;text-align:center"><button onclick="editPoint(\''+value.id+'\',\''+value.criteria_category+'\',\''+value.criteria+'\',\''+value.result_1+'\',\''+value.result_2+'\',\''+value.result_3+'\',\''+value.result_4+'\')" class="btn btn-warning btn-sm">Edit</button><button style="margin-left:10px" class="btn btn-danger btn-sm" onclick="deletePoint(\''+value.id+'\')">Delete</button></td>';
					tableDataBody += '</tr>';
				})
	...
	function editPoint(id,criteria_category,criteria,result_1,result_2,result_3,result_4) {
		$('#id').val(id);
		$('#edit_criteria_category').val(criteria_category);
		$('#select_edit_criteria').select2({
	...
	function updatePoint() {
		$('#loading').show();
		var id = $('#id').val();
		var criteria_category = $('#edit_criteria_category').val();
		...
		$.post('{{ url("update/sga/point_check") }}',data, function(result, status, xhr){
			if(result.status){
				fillList();
	...
	function addPoint() {
		$('#loading').show();
		var criteria_category = $('#add_criteria_category').val();
		...
		$.post('{{ url("input/sga/point_check") }}',data, function(result, status, xhr){
			if(result.status){
				fillList();
	...
	function deletePoint(id) {
		if (confirm('Apakah Anda yakin akan menghapus data?')) {
			...
			$.get('{{ url("delete/sga/point_check") }}',data, function(result, status, xhr){
				if(result.status){
					fillList();
