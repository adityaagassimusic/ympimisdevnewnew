...
<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li><a class="btn btn-success pull-right" style="width: 100%;color: white" onclick="newData('new')"><i class="fa fa-plus"></i> &nbsp;Tambah Data Kecelakaan Lalu Lintas</a></li>
			...
			<div class="box box-solid" style="margin-bottom: 0px;margin-left: 0px;margin-right: 0px;margin-top: 10px">
				<div class="box-body">
					<div class="col-xs-12" style="margin-top: 0px;padding-top: 10px;padding: 0px">
						<div class="col-xs-12" style="background-color:  #bdbdbd ;padding-left: 5px;padding-right: 5px;height:40px;vertical-align: middle;color:white" align="center">
							<span style="font-size: 25px;color: black;width: 25%;">REKAP KECELAKAAN LALU LINTAS PT. YMPI </span>
	...
	<div class="modal-dialog modal-lg" style="width: 900px">
		<div class="modal-content">
			<div class="modal-header" style="padding-top: 0;">
				...
				<center><h3 style="font-weight: bold; padding: 3px;background-color: #bdbdbd;color: black;" id="modalNewTitle"></h3></center>
					<div class="row">
						...
						<div class="col-md-10">
							...
							<div class="col-md-6" style="margin-bottom: 5px;padding: 0">
								<label for="title" class="col-sm-12 control-label">Korban<span class="text-red">*</span></label>
								<div class="col-sm-12">
									<select class="form-control select3" id="employee" name="employee" data-placeholder='Identitas Korban' style="width: 100%">
										<option value="">&nbsp;</option>
										@foreach($employee as $emp)
										<option value="{{$emp->employee_id}}_{{$emp->name}}_{{$emp->department}}">{{$emp->employee_id}} - {{$emp->name}}</option>
										@endforeach
									</select>
								</div>
							</div>	
							...
							<div class="col-md-6" style="margin-bottom: 5px;padding: 0">
								...
								<div class="col-sm-12">
									...
									<input type="text" class="form-control" id="location" name="location" placeholder="Lokasi Kejadian">
								</div>
							</div>											

							<div class="col-md-6" style="margin-bottom: 5px;padding: 0">
								<label for="area" class="col-sm-12 control-label">Bagian<span class="text-red">*</span></label>
								<div class="col-sm-12">
									<input type="text" class="form-control" id="area" name="area" placeholder="Bagian">
								</div>
							</div>
							...
							<div class="col-md-6" style="margin-bottom: 5px;padding: 0">
								<label for="category" class="col-sm-12 control-label">Kategori<span class="text-red">*</span></label>
								<div class="col-sm-12">
									<select class="form-control select3" id="accident_number" name="accident_number" data-placeholder='Kategori' style="width: 100%">
										<option value="">&nbsp;</option>
										<option value="Waktu Pribadi">Waktu Pribadi</option>
										<option value="Waktu Kerja (Dihitung)">Waktu Kerja (Dihitung)</option>
										<option value="Waktu Kerja (Tidak Dihitung)">Waktu Kerja (Tidak Dihitung)</option>
									</select>
								</div>
							</div>
							...
							<div class="col-md-12" style="margin-bottom: 5px;padding: 0">
								...
								<div class="col-sm-12">
									<textarea type="text" class="form-control" id="detail_incident" name="detail_incident" placeholder="Detail Kejadian" rows="3" cols="50"></textarea>
								</div>
							</div>

							<div class="col-md-12" style="margin-bottom: 5px;padding: 0">
								<label for="poin_penting" class="col-sm-12 control-label">Poin Penting<span class="text-red">*</span></label>
								<div class="col-sm-12">
									<textarea type="text" class="form-control" id="poin_penting" name="poin_penting" placeholder="Poin Penting" rows="2" cols="50"></textarea>
								</div>
							</div>
							...
							<div class="col-md-4" style="margin-bottom: 5px;padding: 0">
								...
								<div class="col-sm-12">
									<input type="text" class="form-control" id="loss_time" name="loss_time" placeholder="E.g. : 2 Hari"> </textarea>
								</div>
							</div>
							...
							<div class="col-md-4" style="margin-bottom: 5px;padding: 0">
								...
								<div class="col-sm-12">
									<input type="text" class="form-control" id="recovery_time" name="recovery_time" placeholder="E.g. : 1 Hari"> </textarea>
								</div>
							</div>

							<div class="col-md-4" style="margin-bottom: 5px;padding: 0">

								<label for="loss_cost" class="col-sm-12 control-label">Kerugian</label>
								<div class="col-sm-12">
									<input type="text" class="form-control" id="loss_cost" name="loss_cost" placeholder="E.g. : $ 60"> </textarea>
								</div>
							</div>
							...
							<div class="col-md-5 ilustrasi" style="margin-bottom: 5px;padding: 0">
								...
								<div class="col-sm-12">
									<input type="file" class="form-control" id="illustration_image" name="illustration_image"> 
								</div>
							</div>
         	...
         	<table id="tableDetail" class="table table-striped table-bordered" style="width: 100%;">
		        <thead>
			        <tr style="border-bottom:3px solid black;border-top:3px solid black;background-color:#cddc39">
				        ...
				        <th style="padding: 5px;text-align: center;width: 1%">Kategori</th>
...
<script>
	...
	function newData(id){
		...
		if(id == 'new'){
			$('#modalNewTitle').text('Data Kecelakaan Lalu Lintas');
			$('#newButton').show();
			$('#updateButton').hide();
			clearNew();
			$('#modalNew').modal('show');
			...
			$.get('{{ url("detail/kecelakaan") }}', data, function(result, status, xhr){
				if(result.status){
					...
					// $('#employee').html('');
					// var employee = "";
					// if(result.accident.employee_id == ){
					// 	employee += '<option value="YMPI" selected>YMPI</option>';
					// 	employee += '<option value="Other">Other</option>';
					// }
					// else if (result.accident.employee == "IDR"){
					// 	employee += '<option value="YMPI">YMPI</option>';
					// 	employee += '<option value="Other" selected>Other</option>';
					// }
					$('#employee').val(result.accident.employee_id+"_"+result.accident.employee_name+"_"+result.accident.employee_department).trigger("change");
					...
					$('#accident_number').val(result.accident.accident_number).trigger("change");
					...
					$('#poin_penting').val(result.accident.illustration_detail);
                    ...
                    for (var i = 0; i < data_illustrasi_image.length; i++) { 
						isi_ilustrasi += '<div class="col-md-12" style="padding:0"><div class="col-md-5 ilustrasi" style="margin-bottom: 5px;padding: 0"><label for="illustration_image" class="col-sm-12 control-label">Foto kejadian</label><div class="col-sm-12"><img src="{{url("files/kecelakaan/kecelakaan_lalu_lintas")}}/'+data_illustrasi_image[i]+'" width="700"> </div></div><div class="col-md-7 ilustrasi" style="margin-bottom: 5px;padding: 0"></div>';
					}
					...
					$('#modalNewTitle').text('Update Data Kecelakaan Lalu Lintas');
					$('#loading').hide();
					$('#modalNew').modal('show');
				}
				else{
	...
	function Save(id){	
		...
		if(id == 'new'){
			if($("#submission_date").val() == "" || $('#position').val() == "" || $('#location').val() == "" || $('#area').val() == "" || $('#date_incident').val() == "" || $('#accident_number').val() == "" || $('#time_incident').val() == "" || $('#detail_incident').val() == ""  || $('#poin_penting').val() == "" || $('#condition').val() == "") {
			...
			var korban = employee.split("_");
			
			formData.append('lop', $("#lop").val());
			formData.append('submission_date', $("#submission_date").val());
			formData.append('emp_id', korban[0]);
			formData.append('emp_name', korban[1]);
			formData.append('emp_department', korban[2]);
			formData.append('accident_number', $("#accident_number").val());
			...
			formData.append('poin_penting', $("#poin_penting").val());
   ...
   //        	formData.append('illustration_image', $("#illustration_image").prop('files')[0]);
			...
			$.ajax({
				url:"{{ url('create/kecelakaan/lalu_lintas') }}",
				method:"POST",
				data:formData,
				dataType:'JSON',
				contentType: false,
			...
			if($("#submission_date").val() == "" || $('#position').val() == "" || $('#location').val() == "" || $('#area').val() == "" || $('#date_incident').val() == "" || $('#time_incident').val() == "" || $('#detail_incident').val() == "" || $('#poin_penting').val() == "" || $('#condition').val() == ""){
   ...
   //        	formData.append('illustration_image', $("#illustration_image").prop('files')[0]);
	...
	function fetchTable(){
		$('#loading').show();
		$.get('{{ url("fetch/kecelakaan/lalu_lintas") }}', function(result, status, xhr){
			if(result.status){
				...
				$.each(result.accident, function(key, value){
					...
					listTableBody += '<td style="width:0.1%;text-align:left">'+value.accident_number+'</td>';
					listTableBody += '<td style="width:1%;text-align:left">'+value.employee_id+' - '+value.employee_name+'</td>';
					listTableBody += '<td style="width:1%;text-align:left">'+value.location+'</td>';
					listTableBody += '<td style="width:2%;text-align:left">'+value.condition+'</td>';
	          ...
	          for (var i = 0; i < data.length; i++) { 
							listTableBody += '<a target="_blank" href="{{ url("files/kecelakaan/kecelakaan_lalu_lintas") }}/'+data[i]+'"><i class="fa fa-paperclip"></i>';
						}
					...
					if (value.status == null) {
						...
						listTableBody += '<a class="btn btn-md btn-danger" target="_blank" href="{{ url("index/kecelakaan/report") }}/'+value.id+'"><i class="fa fa-file-pdf-o"></i> </a> | <button class="btn btn-md btn-success" onclick="sendEmail(\''+value.id+'\')" title="Send Email To All Employee"><i class="fa fa-envelope"></i></button> <a class="btn btn-md btn-warning" onclick="ShowChart(\''+value.id+'\')"> <i class="fa fa-bar-chart"></i></a>';

						listTableBody += '</td>'
					}
					else {
						listTableBody += '<td style="width:2%;"><center><a class="btn btn-md btn-danger" target="_blank" href="{{ url("index/kecelakaan/report") }}/'+value.id+'"><i class="fa fa-file-pdf-o"></i> </a> <a class="btn btn-md btn-warning" onclick="ShowChart(\''+value.id+'\')"><i class="fa fa-bar-chart"></i> </a></center></td></button>';
					}
 ...
 //    }
	...
	// function drawChart() {
	// 	$.get('{{ url("fetch/monitoring/kecelakaan/lalu_lintas") }}', function(result, status, xhr) {
	// 		if(xhr.status == 200){
	// 			if(result.status){
	// 				var bulan = [], jumlah = [];
...
function tambah(id,lop) {
	...
	function sendEmail(id) {
      ...
      if (!confirm("Apakah anda yakin ingin mengirim Berita Kecelakaan Lalu Lintas Ke Semua Member yang memiliki email?")) {
        return false;
      }
      else{
      	$("#loading").show();
      ...
      $.get('{{ url("index/kecelakaan/sendemail/lalu_lintas") }}', data, function(result, status, xhr){
				...
				if(result.status){
					...
					$.each(result.detail, function(key2, value2){
						detail_modal.push({
							...
							accident_number:value2.accident_number,
    ...
    function ShowModal(bulan,status,tahun) {
			...
			for(var i = 0; i < detail_modal.length;i++){
				...
				if (getMonthName(new Date(detail_modal[i].date_incident)) === bulan && new Date(detail_modal[i].date_incident).getFullYear() == tahun) {
					...
					tableDetail += '<td style="width:2%;text-align:left">'+detail_modal[i].accident_number+'</td>';
					tableDetail += '<td style="width:2%;text-align:left">'+detail_modal[i].location+'</td>';
					tableDetail += '<td style="width:2%;text-align:left">'+detail_modal[i].area+'</td>';
					tableDetail += '<td style="width:10%;text-align:left">'+detail_modal[i].detail_incident+'</td>';
	...
	function ShowChart(id){
			...
			$.get('{{ url("chart/kecelakaan") }}',data,function(result, status, xhr){
				...
				$('#judul_chart').html('Detail Sosialiasi Kecelakaan Lalu Lintas '+result.accident.location+' '+result.accident.date_incident);
