		...
		function fetchChart(){
			$.get('{{ url("fetch/filosofi") }}',function(result, status, xhr){
				if(result.status){
					...
					periode = '';
					var filos = [];
					for(var u = 0; u < result.tr_filos_answers.length;u++){
						filos.push(result.tr_filos_answers[u].employee_id);
					}
					...
					for(var i = 0; i < result.departments.length;i++){
						...
						for(var k = 0; k < result.employees.length;k++){
							...
							if (result.employees[k].department == result.departments[i].department_name) {
								var employee_id = new RegExp(result.employees[k].employee_id, "g");
								if (filos.join().match(employee_id)) {
									count_sudah++;
									total_sudah++;
									training_detail.push({
										employee_id:result.employees[k].employee_id,
										name:result.employees[k].name,
										department_shortname:result.departments[i].department_shortname,
										department:result.departments[i].department,
						...
						sudah.push({y:parseInt(count_sudah),key:result.departments[i].department_name});
						belum.push({y:parseInt(count_belum),key:result.departments[i].department_name});
						xCategories.push(result.departments[i].department_shortname);
					}
...
function ShowModal(department_shortname,status_cek,department) {
	...
	for(var i = 0; i < training_detail.length;i++){
		if (training_detail[i].department === department) {
			if (training_detail[i].status_cek === status_cek) {
				tableDetail += '<tr>';
				tableDetail += '<td>'+training_detail[i].employee_id+'</td>';
				tableDetail += '<td>'+training_detail[i].name+'</td>';
				tableDetail += '<td>'+(training_detail[i].department_shortname || '')+'</td>';
				tableDetail += '<td>'+(training_detail[i].section || '')+'</td>';
				tableDetail += '<td>'+(training_detail[i].group || '')+'</td>';
				tableDetail += '<td>'+(training_detail[i].sub_group || '')+'</td>';
				tableDetail += '<td>'+(training_detail[i].status_cek || '')+'</td>';
				tableDetail += '</tr>';
			}
		}
	}
