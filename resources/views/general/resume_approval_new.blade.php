						...
						<table id="tableData" class="table table-bordered table-hover" style="margin-top: 10px;">
							<thead style="">
								<tr>
									...
									<th style="width: 0.01%; text-align: center; background-color: #605ca8; color: white;">Status</th>
									...
									<th style="width: 0.1%; text-align: center; background-color: #605ca8; color: white;">Approval</th>
								</tr>
							</thead>
			...
			if(result.status){
				...
				$.each(result.resumes, function(key, value){
					if (value.category == '-') {
						...
						tableDataBody += '<td style="width: 0.1%; text-align: left;">'+value.approval+'</td>';
						tableDataBody += '</tr>';
					}else{
						tableDataBody += '<tr>';
						if (value.status == "Fully Approved") {
				...
				var table = $('#tableData').DataTable({
					...
					initComplete: function () {
			            this.api()
			                ...
			                .every(function (dd) {
			                    var column = this;
			                    var theadname = $("#tableData th").eq([dd]).text();
			                    var select = $('<select><option value="" style="font-size:11px;">All</option></select>')
			                    ...
			                    column
			                        ...
			                        .each(function (d, j) {
			                        	var vals = d;
			                        	if ($("#tableData th").eq([dd]).text() == 'Category') {
			                        		vals = d.split(' ')[0];
			                        	}
